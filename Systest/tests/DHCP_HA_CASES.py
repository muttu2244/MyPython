...
TOPOLOGY DIAGRAM:
	...
	-------------------------------------------------------------------------
         ...
         -------------------------------------------------------------------------
HOW TO RUN:python2.5 DHCP_HA_CASES.py
...
class test_DHCP_HA_CASES(test_case):
    myLog = getLogger()

    def setUp(self):

        #Establish a telnet session to the SSX box.
        self.ssx = SSX(ssxha["ip_addr"])
    ...
    def test_DHCP_HA_CASES(self):

        self.myLog.output("\n**********start the test**************\n")
        print "Running Vgroup :~"
        vgroup_var1=topo4[0]+" "+topo4[1]
        vgroup_var2=topo5[0]+" "+topo5[1]
        #vgroup b/w SSX and linux1.
        vgroup_new(vgroup_var1)
        #vgroup b/w SSX and linux2.
        vgroup_new(vgroup_var2)

        
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_HA_CASES'])
	...
	Ip_out = get_ipaddr(self.linux2,p1_ssx_linux2[1])
        ...
        self.failUnless(is_healthy(hs, Card_Reset = 1,IMC_Switch=1), "Platform is not healthy")         

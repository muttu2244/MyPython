...
DESCRIPTION: Detecting & classifying protocol, Telnet
TEST PLAN: DPI Test Plan
TEST CASES: DPI_PD_13
...
How to run: "python2.5 DPI_FUN_013.py"
...
from misc import *
#import configs file
pexpect.run('perl  -pi -e \'s/proto_from_script = "\w+"/proto_from_script = "telnet"/\' ikev2_config.py')
from ikev2_config import *
from topo import *

class test_DPI_FUN_013(test_case):
    myLog = getLogger()
	...
	self.failUnless(pingOutput, "Ping is failed b/w ike client and remote host")
        ...
        telnetOutput = self.xpress_vpn.cmd("sudo nc -z 69.0.0.1 23")
	time.sleep(5)
	self.failUnless(telnetOutput, "telnet is failed b/w ike client and remote host")
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_DPI_FUN_013)

...
class test_log_fun_005_2_day_hd(test_case):
    myLog = getLogger()
    ...
    def test_log_fun_005_2_day_hd(self):
        """
        This test verifies the system will erase log files when the total count gets
        to be over 1000 files in either /hd/logs and the files are newer then 1 day
        
        This test will pass if there are 1000 files or less left in /hd/logs
        It generates files that are over 2 day old. All of those files should be deleted
        """
        debug = True
        ...
        # Setting the auto save value to 2
        
        command = 'logging auto-save max-retention-interval 2'
        ...
        if debug:
            self.myLog.debug("length of return value: %s" % len(retr))
        
        self.failUnless(len(retr) == 0)
        command = 'end'
        retr = self.ssx.configcmd(command)
        
        
        # Open the hidden shell
        ...
        # if the time delta is greater then 1 days or some user defined value
...
class test_log_fun_005_2_day_hdp(test_case):
    myLog = getLogger()
    ...
    def test_log_fun_005_2_day_hdp(self):
        """
        This test verifies the system will erase log files when the total count gets
        to be over 1000 files in either /hdp/logs and the files are newer then 1 day
        
        This test will pass if there are 1000 files or less left in /hdp/logs
        It generates files that are over 2 day old. All of those files should be deleted
        """
        debug = True
        # Extra "dummy" files will be generated to the system to get the system to
...
class test_log_fun_005_10_day_hd(test_case):
    myLog = getLogger()
    ...
    def test_log_fun_005_2_day_hd(self):
        ...
        # Setting the auto save value to 10
        
        command = 'logging auto-save max-retention-interval 10'
...
class test_log_fun_005_10_day_hdp(test_case):
    myLog = getLogger()
    ...
    def test_log_fun_005_10_day_hdp(self):
        """
        This test verifies the system will erase log files when the total count gets
        to be over 1000 files in either /hdp/logs and the files are newer then 1 day
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_log_fun_005_2_day_hd)
    suite.addTest(test_log_fun_005_2_day_hdp)
    suite.addTest(test_log_fun_005_10_day_hd)
    suite.addTest(test_log_fun_005_10_day_hdp)   

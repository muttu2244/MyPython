...
DESCRIPTION:Send UDP traffic flood before IKEv2 session is established and check that session is not dropped and re-key happens
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_077
...
HOW TO RUN:python2.5 ip_atk_077.py
...
class test_ip_atk_077(test_case):
    myLog = getLogger()	
	...
	time.sleep(25)
        ...
        self.ssx.cmd("clear port %s counters drop"%script_var['ssx_port2'])
        self.myLog.output("Port counters exist on SSX:%s"%self.ssx.cmd("show port %s counters drop"%script_var['ssx_port2']))
        ...
        self.xpressvpn1.cmd("sudo ./add_ip_takama_qos-ashu") #copy this file from takama if already not existing
        ...
        self.myLog.output("Port counters exist on SSX after flooding traffic:%s"%self.ssx.cmd("show port %s counters drop"%script_var['ssx_port2']))

...
DESCRIPTION: Verify that SSX allows deleting a NTP server and adding a new NTP server
TEST PLAN:NTP CLI Test plans
TEST CASES: NTP_CLI_010
...
HOW TO RUN: python2.5 NTP_CLI_010.py
...
class test_NTP_CLI_010(test_case):
    myLog = getLogger()
	...
	status = False
        ...
        if "server %s"%var_dict['infc_linux1_ip'] in op.splitlines()[key_index] :
	...
	# Push NTP configuration on NTP client
        ...
        self.ssx.cmd("context %s"%var_dict['context_name'])
        self.ssx.cmd("ntp profile")
        self.ssx.cmd("no server %s"%var_dict['infc_linux1_ip'])
	self.ssx.cmd("server %s"%var_dict['infc_r1_ip'])
        self.ssx.cmd("exit")
        self.ssx.cmd("end")
        ...
        if status == False:
                self.failIf(True, "No context configured")
        key_index = context_index + 2
        if "server %s"%var_dict['infc_r1_ip'] in op.splitlines()[key_index]:
                self.myLog.output("NTP server deleted and configured again")
        else:
                self.failIf(True, "NTP server not configured")
...
if __name__ == '__main__':
	...
	suite = test_suite()
	suite.addTest(test_NTP_CLI_010)

...
HOW TO RUN: python2.5 GLCR_SES_IKEv2_FUN_035.py
...
class test_GLCR_SES_IKEv2_FUN_035(test_case):
    myLog = getLogger()
	...
	self.sst.cmd("sst %s count - 1 rate 1 ike-protocol ikev2 transport ipv4"%ipsec_var['port_sst_slot2'])
        ...
        # Bring up back interface which is connected to client
        self.myLog.info("Bring up back Cisco interface which is connected to client")
        self.cisco.cmd("config ter")
        self.cisco.cmd("interface gigabit %s"%ipsec_var['port_cisco_sst_slot2'])
    ...
    def test_GLCR_SES_IKEv2_FUN_035(self):
	...
	if int(op['standby']) != 4:
        ...
        self.ssx.config_from_string(ipsec_var['GLCR_SES_IKEv2_FUN_035'])
        self.sst.config_from_string(ipsec_var['SST_GLCR_SES_IKEv2_FUN_035'])
	...
	# Bring down the interface which is connected to client
	self.myLog.info("Bring down the Cisco interface which is connected to client")
	self.cisco.cmd("config ter")
	self.cisco.cmd("interface gigabit %s"%ipsec_var['port_cisco_sst_slot2'])
	self.cisco.cmd("shutdown")
	self.cisco.cmd("end")
	...
	self.myLog.info("DPD info as per the config:\n")
	self.myLog.info("DPD Interval      : 60 Sec")
	self.myLog.info("DPD Retrt Interval: 60 Sec")
	self.myLog.info("Maximum Retries   : 5")
	sesOp = self.ssx.cmd("show ike-session detail handle %s"%ses_yn.splitlines()[-1].split()[1])
	reOp = re.search("Next DPD check after\s+:\s+(\d+)\s+",sesOp, re.I)
	letmeSleep = reOp.group(1)
	self.myLog.info("Waiting for 120 second for first retry")
	time.sleep(int(letmeSleep) + 5)

	for timeIndex in range(5):
		time.sleep(60)
		self.myLog.info("Verifying the session for %s(th) DPD retry"%(int(timeIndex) + 1))
		ses_yn = self.ssx.cmd("show ike-session 4 brief")
		self.failIf("ERROR:" in ses_yn, "Session went down before Max DPD retry")
		op1 = self.ssx.cmd("show session") + self.ssx.cmd("show ike-session 4 brief") + self.ssx.cmd("show ike-session 4 list") + self.ssx.cmd("show ike-session detail handle %s"%ses_yn.splitlines()[-1].split()[1])
		self.myLog.output("Session Details on 4th card: %s"%op1)
		self.myLog.info("Verifying the debug for the DPD Retries")
		debugOp = self.ssx.cmd('show log debug | grep "IKED_RETRANSMIT_DPDV2_PKT"')
		if debugOp == None:
			self.fail("No debug for the DPD RETRANSMIT")
		self.myLog.info("Verifying the number of DPD Retries logged")
		print debugOp
		print len(debugOp.splitlines())
		print (len(debugOp.splitlines()) != int(timeIndex) + 2)
		self.failIf(len(debugOp.splitlines()) != int(timeIndex) + 2, "Number of DPD retries logged are less")
		self.myLog.info("Verifying DPD Retries count in the session details")
		sesOp = self.ssx.cmd('show ike-session detail handle %s | grep "DPD Retries"'%ses_yn.splitlines()[-1].split()[1])
		print sesOp
		print int(sesOp.splitlines()[-1].split(':')[-1].strip())
		print (int(sesOp.splitlines()[-1].split(':')[-1].strip()) != int(timeIndex) + 1)
		print int(timeIndex)
		self.failIf(int(sesOp.splitlines()[-1].split(':')[-1].strip()) != int(timeIndex) , "DPD Retries count is not matching with the number of Retries")
		

	self.myLog.info("Given delay for last DPD retry")
	time.sleep(60)
	self.myLog.info("Verifying the session after all DPD Retries")
	ses_yn = self.ssx.cmd("show ike-session 4 brief")
	self.failIf("ERROR:" not in ses_yn, "Session is still up after all DPD Retries")

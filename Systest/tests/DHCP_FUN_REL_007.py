...
DESCRIPTION:To verify SSX (relay agent) relays the DHCP reply message from a DHCP server to the many DHCP subscribers. 
TEST PLAN: Sanity Test plans
TEST CASES: DHCP-FUN-REL-007
...
HOW TO RUN:python2.5 DHCP-FUN-REL-007.py
...
class test_DHCP_FUN_007(test_case):
    myLog = getLogger()
	...
	#self.linux4 = Linux(linux2["ip_addr"])
        #self.linux5 = Linux(linux3["ip_addr"])
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_007'])
	...
	#Removing the tethereal capture file if its present on client sides.      
	...
	self.linux1.cmd("sudo /usr/sbin/tethereal -i '%s' -q -w dhcp.pcap -R bootp &" %(p1_ssx_linux1[1]))

#	Infc_out = p1_ssx_linux2[1]
	...
	#chaging context and displaying Dhcp counters incremented due to relaying of dhcp messages.
        self.ssx.cmd("context %s"%script_var['context_name'])
	self.myLog.output("\ndisplaying Dhcp counters incremented due to relaying of dhcp messages\n%s\n"%self.ssx.cmd("show dhcp counters"))
	...
	self.myLog.info("*" *50)
        ...
        self.linux3.cmd("sudo rm -f dhcp.pcap")
	Infc_out = p1_ssx_linux3[1]

        #Capturing the packets on remote Linux and running dhcpclient on client side
        self.linux1.cmd("sudo /usr/sbin/tethereal -i '%s' -q -w dhcp.pcap -R bootp &" %(p1_ssx_linux1[1]))
        self.linux3.cmd("sudo ./dhclient %s"%Infc_out,timeout=100)

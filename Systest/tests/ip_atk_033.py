...
DESCRIPTION: Send TCP oversized packets
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_033
...
HOW TO RUN:python2.5 ip_atk_033.py
...
class test_ip_atk_033(test_case):
    myLog = getLogger()	
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send TCP Oversized packet to SSX.
	out = self.linux1.cmd("sudo /usr/sbin/hping -V  -d 65000 %s -p 22 -I %s &"%(script_var['ssx_ip_addr'],p1_ssx_linux1[1]))

...
TEST CASES:DHCP_CLI_002, DHCP_CLI_003
...
TOPOLOGY DIAGRAM:
	...
	-------------------------------------------------------------------------
       ...
       |         Trans  IP = 2.1.1.1/24	---------- > TransIP = 2.1.1.2/24	|
			eth1	                         port-4/0 
         ----------------------------------------------------------------------
HOW TO RUN:python2.5 DHCP_CLI_002.py
...
class test_DHCP_CLI_002(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_CLI_002'])
	...
	Ip_out = get_ipaddr(self.linux2,p1_ssx_linux2[1])
        ...
        # Verifying the test case DHCP_CLI_003
        ...
        self.failIf("dhcp relay" in a.split("\n"))

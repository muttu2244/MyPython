...
DESCRIPTION:To Verify that ACL allows all Protocol traffic when it is removed from port  
TEST PLAN: ACL Test plans
TEST CASES:GRED_ACL_TUN_FUN_013.py

HOW TO RUN:python2.5 GRED_ACL_TUN_FUN_013
...
class test_GRED_ACL_TUN_FUN_013(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	# Checking SSX Health before tearDown
	self.myLog.info("Checking SSX Health before tearDown")
    ...
    def tunnelUp(self):
        ...
        self.linux.write_to_file(script_var['fun_013_xpressvpn'],"autoexec.cfg","/xpm/")
        ...
        self.failUnless(op_ping,"Ping through tunnel failed before applying Acl")
	...
	#changing context and clearing ip counters
        ...
        #self.failIf(tun_cntrs!=0 , "Tunnel counters does not exist")
	...
	#print self.ssx.cmd("show ip access-list name %s"%(script_var['acl_name']))
        self.ssx.cmd("end")
	...
	tun_cntrs = verify_tunnel_counters_with_name(self.ssx,script_var['tunnel_name']) #tunnel api
        self.failIf(tun_cntrs != 0 , "Traffic is not via tunnel!!No tunnel counters")



    def test_GRED_ACL_TUN_FUN_013(self):
	...
	op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
        self.failIfEqual(op_ping,False,"Ping through tunnel failed during reload though the tunnel is on 4th card")
	...
	print "***********************************************AFTER RELOAD BEFORE SWITCHBACK****************\n"

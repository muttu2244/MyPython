...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Vlan enabled port when inbound and outbound ACLs applied to Vlan enabled port [Permit for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_210.py

HOW TO RUN:python2.5 ACL_FUNC_210
...
class test_ACL_FUNC_210(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['permit_out_icmp_port'])

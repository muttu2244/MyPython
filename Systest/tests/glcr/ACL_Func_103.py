...
DESCRIPTION: Verify ACL functionality for TCP traffic through port when inbound and outbound ACLs applied to port [Deny for inbound and Deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_103.py

HOW TO RUN:python2.5 ACL_FUNC_103
...
class test_ACL_FUNC_103(test_case):
    myLog = getLogger()
    def setUp(self):

...
DESCRIPTION: Verify ACL functionality for TCP traffic through port when inbound and outbound ACLs applied to port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_100.py

HOW TO RUN:python2.5 ACL_FUNC_100
...
class test_ACL_FUNC_100(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_tcp_port'])
	...
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,False,"Ping through tunnel must fail after acl applied")
	...
	self.ssx.config_from_string(script_var['permit_in_tcp_port'])

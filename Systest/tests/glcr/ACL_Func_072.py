...
DESCRIPTION:Verify ACL functionality for UDP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_072.py

HOW TO RUN:python2.5 ACL_Func_072
...
class test_ACL_Func_072(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#self.linux.cmd("!sudo ping %s -I %s -c 2"%(script_var['linux1'], script_var['xpVpnHstIp']))
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['deny_udp_out'])
	...
	#nemesis tool for generating igmp packets on linux
        #self.linux.cmd("!sudo /usr/local/bin/nemesis udp -S %s -D %s"%( script_var['xpVpnHstIp'], script_var['cisco_service_slot2_ip']))
	...
	tun_cntrs = verify_tunnel_counters_with_name(self.ssx,script_var['tunnel_name'])
        self.failIf(tun_cntrs == 0, "Traffic must not go via tunnel ")
	
	op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['cisco_service_slot2_ip'],"500","4")
        self.failIfEqual(op_ping,False,"Ping through tunnel failed after acl removed")
	
	time.sleep(10)	
        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0","0","0","4","5")
	#acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0","0","0","4","1")
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_udp_in'])
	...
	time.sleep(10)
        ...
        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"9","0","0","0","0")

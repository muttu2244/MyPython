...
DESCRIPTION:Verify ACL functionality for IGMP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_064.py

HOW TO RUN:python2.5 ACL_Func_064
...
class test_ACL_Func_064(test_case):
    myLog = getLogger()
    def setUp(self):

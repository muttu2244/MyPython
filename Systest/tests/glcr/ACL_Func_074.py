...
DESCRIPTION:Verify ACL functionality for UDP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Permit for inbound and Permit for outbound]  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_074.py

HOW TO RUN:python2.5 ACL_Func_074
...
class test_ACL_Func_074(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_udp_out'])
	...
	self.ssx.cmd("clear tunnel counters")
	#clearing all the acl's 
        self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
	
	self.linux1.cmd("sudo /usr/local/bin/nemesis udp -D %s -S %s"%( script_var['xpVpnHstIp'], script_var['linux1']))
        time.sleep(5)

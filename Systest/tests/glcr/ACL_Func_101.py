...
DESCRIPTION: Verify ACL functionality for TCP traffic through port when inbound and outbound ACLs applied to port [Deny for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_101.py

HOW TO RUN:python2.5 ACL_FUNC_101
...
class test_ACL_FUNC_101(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_in_tcp_port'])
	...
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,False,"Ping through tunnel failed after acl applied")
	...
	self.ssx.config_from_string(script_var['permit_out_tcp_port'])
	...
	# Reload the card
        ...
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['standby_slot4_ip'],"500","25")
	op_ping = self.linux.ping(script_var['standby_slot4_ip'])
        ...
        time.sleep(20)
        #ping operation after acl applied
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var[''],"500","25")
	op_ping = self.linux.ping(script_var['active_slot2_ip'])

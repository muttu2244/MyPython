...
DESCRIPTION:Verify permit/deny ICMP <src> <dest> [<msg-type>] [<code>] [precedence <level>] [tos <level>] - for all values of msg-type, when bound to session for both inbound and outbound.  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_126.py

HOW TO RUN:python2.5 ACL_Func_126
...
class test_ACL_Func_126(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	op_ping = self.linux2.ping_xpress_vpn(script_var['ip_pool'],script_var['linux1'],"500","4")
        self.failEqual(op_ping,True,"Ping through session must fail")
	...
	self.ssx.config_from_string(script_var['deny_icmp_sess_in_options'])

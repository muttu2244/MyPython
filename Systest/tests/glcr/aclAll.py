...
class test_ACL_Func(test_case):
	...
	#self.linux1.write_to_file(script_var['rad_users_acl_out'],"users","/usr/local/etc/raddb/")
	...
	op_client_cmd2 = self.linux2.cmd("start_ike")
        time.sleep(10)	
	...
	#self.linux1.cmd("sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['xpVpnHstIp']))
        #time.sleep(5)

	if 'permit_tcp' in rule:
            if 'in' in rule:
		self.linux.cmd("!sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['linux1']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux.cmd("!sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['linux1']))
		time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"5","0","0","0","0")
                self.failIf(acl_cntrs != True , "Acl counters are failing ")
		if acl_cntrs != True:
		    self.myLog.output("\n\n\n######################################################################\n")
		    ...
		    self.myLog.output("\n\n\n######################################################################\n")
            if 'out' in rule:
		self.linux1.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['xpVpnHstIp']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux1.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['xpVpnHstIp']))
        	time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0","0","0","5","0")
                self.failIf(acl_cntrs != True , "Acl counters are failing ")
		if acl_cntrs != True:
		    self.myLog.output("\n\n\n######################################################################\n")
	...
	if 'permit_udp' in rule:
            if 'in' in rule:
		self.linux.cmd("!sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['linux1']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux.cmd("!sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['linux1']))
                time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"5","0","0","0","0")
	...
	if 'permit_tcp' in rule:
            if 'in' in rule:
		self.linux2.cmd("!sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['ip_pool'],script_var['linux1']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux2.cmd("!sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['ip_pool'],script_var['linux1']))
                time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"5","0","0","0","0")
		...
		self.linux1.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['ip_pool']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux1.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['ip_pool']))
                time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0","0","0","5","0")
	...
	if 'permit_udp' in rule:
            if 'in' in rule:
		self.linux2.cmd("!sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['ip_pool'],script_var['linux1']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux2.cmd("!sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['ip_pool'],script_var['linux1']))
                time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"5","0","0","0","0")
		...
		self.linux1.cmd("sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['ip_pool']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		self.linux1.cmd("sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['linux1'] ,script_var['ip_pool']))
                time.sleep(20)
                acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0","0","0","5","0")
	...
	if 'permit' in rule:
		...
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
		output = verify_port_counters(self.ssx,cmd_op,4)
                self.failIfEqual(output,0,"icmp through interface did not pass through when permited")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
                    self.myLog.output("Port: permit_in_icmp_port : icmp did not pass through when permitted ")
                    self.myLog.output("\n\n\n######################################################################\n")
            if 'out' in rule:
		op_ping = self.linux1.cmd("ping -I %s %s -s 500 -c 4"%(script_var['linux1'],script_var['tnLpBkIp']))
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['sr_Active1_port']))
		output = verify_port_counters(self.ssx,cmd_op,4)
		self.failIfEqual(output,0,"icmp through interface did not pass through when permited")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
		...
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
		output = verify_port_counters(self.ssx,cmd_op,4)
                self.failIfEqual(output,0,"icmp through interface passed through when denied")
                if output == 1:
                    self.myLog.output("\n\n\n######################################################################\n")
	...
	if 'permit' in rule:
            #output = verify_port_counters(self.ssx,cmd_op,1)
            if 'in' in rule:
		self.linux.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['tnLpBkIp']))
                #time.sleep(20)
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
		self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
		output = verify_port_counters(self.ssx,cmd_op,5)
                self.failIfEqual(output,0,"tcp did not pass through when permited")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
                    self.myLog.output("Port: permit_in_tcp_port : tcp did not pass through when permitted ")
                    self.myLog.output("\n\n\n######################################################################\n")
            if 'out' in rule:
		self.linux1.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['linux1'],script_var['tnLpBkIp']))
                #time.sleep(20)
                cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['sr_Active1_port']))
                self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
                output = verify_port_counters(self.ssx,cmd_op,5)
		...
		self.linux.cmd("sudo /usr/sbin/hping2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['tnLpBkIp']))
                ...
                self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
                output = verify_port_counters(self.ssx,cmd_op,5)
                self.failIfEqual(output,0,"tcp  passed through when denied")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
	...
	#self.linux.cmd("sudo /usr/sbin/hping2 -2 -a %s %s -c 5"%(script_var['xpVpnHstIp'],script_var['tnLpBkIp']))
	#time.sleep(5)
	#self.ssx.cmd("context %s" %(script_var['context_name']))
        #cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
	...
	if 'permit' in rule:
		...
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
		output = verify_port_counters(self.ssx,cmd_op,5)
                self.failIfEqual(output,0,"udp did not pass through when permited")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
		...
		cmd_op = self.ssx.cmd("show  port %s counters "%(script_var['tr_Active1_port']))
		output = verify_port_counters(self.ssx,cmd_op,5)
                self.failIfEqual(output,0,"udp through interface passed through when denied")
                if output == 0:
                    self.myLog.output("\n\n\n######################################################################\n")
	...
	for bridge in bridges:
		...
		self.myLog.output("\n********** Configuring SSX Responder **************\n")
	    	...
	    	for rule in aclSessList:
	    	    if 'in' in rule:
	    	    	self.linux1.write_to_file(script_var['rad_users_acl_in'],"users","/usr/local/etc/raddb/")
	    	    elif 'out' in rule:
	    	    	self.linux1.write_to_file(script_var['rad_users_acl_out'],"users","/usr/local/etc/raddb/")
	    	    self.ssx.config_from_string(script_var[rule])

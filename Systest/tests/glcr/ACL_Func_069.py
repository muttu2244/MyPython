...
DESCRIPTION:Verify ACL functionality for TCP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Deny for inbound and Permit for outbound]   
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_069.py

HOW TO RUN:python2.5 ACL_Func_069
...
class test_ACL_Func_069(test_case):
    myLog = getLogger()
    def setUp(self):

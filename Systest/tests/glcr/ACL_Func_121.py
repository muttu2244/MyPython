...
DESCRIPTION: Verify permit/deny ICMP <src> <dest> [<msg-type>] [<code>] [precedence <level>] [tos <level>] - for all values of code, when bound to tunnel for both inbound and outbound  
TEST PLAN: ACL Test plans
TEST CASES:test_ACL_Func_121.py

HOW TO RUN:python2.5 test_ACL_Func_121
...
class test_ACL_Func_121(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#ping before acl applied
        ...
        # Push ACL config
        self.ssx.config_from_string(script_var['permit_icmp_in_options'])
	...
	self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
        ...
        # Push ACL config
        self.ssx.config_from_string(script_var['permit_icmp_out_options'])

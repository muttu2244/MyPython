...
DESCRIPTION: Verify ACL functionality for IGMP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Deny for inbound and Deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_215.py

HOW TO RUN:python2.5 ACL_FUNC_215
...
class test_ACL_FUNC_215(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,False,"Ping through failed after acl applied")

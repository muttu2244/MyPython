...
DESCRIPTION: Verify ACL functionality for IGMP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [ permit inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_214.py

HOW TO RUN:python2.5 ACL_FUNC_214
...
class test_ACL_FUNC_214(test_case):
    myLog = getLogger()
    def setUp(self):

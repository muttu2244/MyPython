...
DESCRIPTION: Verify ACL functionality for UDP traffic through native vlan enabled port when inbound and outbound ACLs applied to native vlan enabled port [Deny for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_237.py

HOW TO RUN:python2.5 ACL_FUNC_237
...
class test_ACL_FUNC_237(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_in_udp_port_untag'])
	...
	self.ssx.config_from_string(script_var['permit_out_udp_port_untag'])

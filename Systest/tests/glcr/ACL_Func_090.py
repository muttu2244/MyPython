...
DESCRIPTION: Verify ACL functionality for UDP traffic through session when inbound and outbound ACLs applied to session [Permit for inbound and Permit for outbound] 
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_090.py

HOW TO RUN:python2.5 ACL_Func_090
...
class test_ACL_Func_090(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.cmd("config")
        ...
        self.ssx.cmd("no 20 permit udp any any")
        self.ssx.cmd("exit")
        self.ssx.cmd("no session name %s"%(script_var['sess_name']))
        self.ssx.cmd("end")	
	self.ssx.config_from_string(script_var['permit_udp_sess_out'])

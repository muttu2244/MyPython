...
TEST CASES:GRED_ACL_TUN_FUN_008.py

HOW TO RUN:python2.5 GRED_ACL_TUN_FUN_008
...
class test_GRED_ACL_TUN_FUN_008(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#clear the acl counters before the traffic is sent
        ...
        self.failIf(tun_cntrs!=0 , "Traffic not going via tunnel!! No tunnel counters")

        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],'0','0','0','0','0')
    ...
    def test_GRED_ACL_TUN_FUN_008(self):
	...
	# Push ACL config
        self.ssx.config_from_string(script_var['permit_udp'])

...
DESCRIPTION:Verify permit/deny ICMP <src> <dest> [<msg-type>] [<code>] [precedence <level>] [tos <level>] - for all values of code, when bound to session for both inbound and outbound  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_125.py

HOW TO RUN:python2.5 ACL_Func_125
...
class test_ACL_Func_125(test_case):
    myLog = getLogger()
    def setUp(self):

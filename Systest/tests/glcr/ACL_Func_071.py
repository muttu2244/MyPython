...
DESCRIPTION:Verify ACL functionality for TCP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [deny for inbound and deny for outbound]   
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_071.py

HOW TO RUN:python2.5 ACL_Func_071
...
class test_ACL_Func_071(test_case):
    myLog = getLogger()
    def setUp(self):

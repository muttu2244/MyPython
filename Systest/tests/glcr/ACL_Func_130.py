...
DESCRIPTION:Verify permit/deny IGMP <src> <dest> [<msg-type>] [precedence <level>] [tos <level>] - for all values of ToS, when bound to tunnel for both inbound and outbound  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_130.py

HOW TO RUN:python2.5 ACL_Func_130
...
class test_ACL_Func_130(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.linux.cmd("!sudo ping %s -I %s -c 2"%(script_var['linux1'], script_var['xpVpnHstIp']))
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['deny_igmp_out_options'])
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_igmp_in_options'])

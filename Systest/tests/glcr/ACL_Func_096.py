...
DESCRIPTION: Verify ACL functionality for IGMP traffic through port when inbound and outbound ACLs applied to port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_096.py

HOW TO RUN:python2.5 ACL_FUNC_096
...
class test_ACL_FUNC_096(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['permit_in_igmp_port'])
        ...
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,True,"Ping failed after permit acl applied")
	time.sleep(15)
        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"5","0","0","0","0")
        self.failIf(acl_cntrs!=False , "Permit Acl counters are not incrementing ")

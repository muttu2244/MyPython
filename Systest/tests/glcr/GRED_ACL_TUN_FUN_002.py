...
DESCRIPTION:To Verify ACL functionality for IGMP traffic through tunnel when ACL bound to Tunnel(Permit/Deny)  
TEST PLAN: ACL Test plans
TEST CASES:GRED_ACL_TUN_FUN_002.py

HOW TO RUN:python2.5 GRED_ACL_TUN_FUN_002
...
class test_GRED_ACL_TUN_FUN_002(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.linux.cmd("!sudo ping %s -I %s -c 2"%(script_var['linux1'], script_var['xpVpnHstIp']))
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['deny_igmp'])
	...
	#verifying  coreespondig tunnel counters
        #tun_cnt = check_single_tunnel_counters_bytes_gte(self.ssx,script_var['tunnel_name'],'0','0')
        #self.failIfEqual(tun_cnt,True,"igmp traffic passed through tunnel when denied")
	
	tun_cntrs = verify_tunnel_counters_with_name(self.ssx,script_var['tunnel_name'])
        self.failIf(tun_cntrs != 0, "Traffic is not via tunnel ")
	
	#clear the ACL counters before sending the traffic
        self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
	
	#print "*************************************************************************\n"	
	#print self.ssx.cmd("show ip access-list name %s counters"%script_var['acl_name'])
	#print "*************************************************************************\n"
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_igmp'])
	...
	#clear the acl counters before the traffic is sent
        ...
        #nemesis tool for generating igmp packets on linux
        #self.linux.cmd("exit")
        #self.linux.cmd("sudo nemesis igmp -i %s"%script_var['linux1'])
	...
	self.linux.cmd("!sudo /usr/local/bin/nemesis igmp -S %s -D %s"%( script_var['xpVpnHstIp'], script_var['linux1']))
        ...
        #verifying  coreespondig tunnel counters
        #tun_cnt = check_single_tunnel_counters_bytes_gte(self.ssx,script_var['tunnel_name'],'0','0')
        #self.failIfEqual(tun_cnt,False,"igmp traffic not passed through tunnel when permitted")
	
	tun_cntrs = verify_tunnel_counters_with_name(self.ssx,script_var['tunnel_name']) #lanlan api
        self.failIf(tun_cntrs != 0, "Traffic is not via tunnel")

	#clearing all the acl's before the end of the script
        self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
    ...
    def test_GRED_ACL_TUN_FUN_002(self):
	...
	output_dut = check_tunnel_state(self.ssx ,tunnel = script_var['tunnel_name'])
        self.failIfEqual(output_dut,False , "Tunnels Have Gone Down After reload")
...
if __name__ == '__main__':
	...
	tt_faketest_2(testcase_id=1940, build_name='test_GRED_ACL_TUN_FUN_002',testplan_id=14,status=2,product_id=54,environment_id=12,manager_id=1217,version='6.0',tester=1217,build=None,run=None,testcase_run=None)

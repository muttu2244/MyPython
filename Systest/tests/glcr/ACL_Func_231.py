...
DESCRIPTION: Verify ACL functionality for IGMP traffic through native vlan enabled port when inbound and outbound ACLs applied to native vlan enabled port [Deny for inbound and Deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_231.py

HOW TO RUN:python2.5 ACL_FUNC_231
...
class test_ACL_FUNC_231(test_case):
    myLog = getLogger()
    def setUp(self):

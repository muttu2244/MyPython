...
DESCRIPTION: Verify ACL functionality for UDP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_220.py

HOW TO RUN:python2.5 ACL_FUNC_220
...
class test_ACL_FUNC_220(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_udp_port'])
	...
	self.ssx.config_from_string(script_var['permit_in_udp_port'])
	...
	self.ssx.cmd("config")
        ...
        self.ssx.cmd("no ip access-list name %s"%(script_var['acl_name1']))
        self.ssx.cmd("end")



    def test_ACL_FUNC_220(self):

...
DESCRIPTION:To Verify ACL functionality is maintained after imc-switchover 
TEST PLAN: ACL Test plans
TEST CASES:GRED_ACL_TUN_FUN_012.py

HOW TO RUN:python2.5 GRED_ACL_TUN_FUN_012
...
class test_GRED_ACL_TUN_FUN_012(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#ping before acl applied
        ...
        op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
        self.failIfEqual(op_ping,0,"Ping through tunnel failed after acl applied")
        ...
        #ping operation after acl applied
        op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
        self.failIfEqual(op_ping != 1,"Ping through tunnel failed after acl applied")

        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"4","0","0","0","0")
        self.failIf(acl_cntrs != True , "Acl counters are incrementing ")
    ...
    def test_GRED_ACL_TUN_FUN_012(self):

...
DESCRIPTION:Verify ACL functionality for ICMP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [deny for inbound and deny for outbound]  
TEST PLAN: ACL Test plans
TEST CASES:test_ACL_Func_063.py

HOW TO RUN:python2.5 test_ACL_Func_063
...
class test_ACL_Func_063(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#Removing the Acl applied
        ...
        # Push ACL config
        self.ssx.config_from_string(script_var['deny_icmp_in'])
	...
	#clear the ACL counters before sending the traffic
        ...
        #ping operation after acl applied
        op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['cisco_service_slot2_ip'],"500","4")
        self.failIfEqual(op_ping,True,"Ping through tunnel must fail after deny ICMP acl applied")

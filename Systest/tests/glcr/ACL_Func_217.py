...
DESCRIPTION: Verify ACL functionality for TCP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Deny for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_217.py

HOW TO RUN:python2.5 ACL_FUNC_217
...
class test_ACL_FUNC_217(test_case):
    myLog = getLogger()
    def setUp(self):

...
DESCRIPTION: Verify ACL functionality for ICMP traffic through port when inbound and outbound ACLs applied to port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_092.py

HOW TO RUN:python2.5 ACL_FUNC_092
...
class test_ACL_FUNC_092(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_icmp_port'])
	...
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,True,"Ping to the port must fail after acl applied")
	...
	self.ssx.config_from_string(script_var['permit_in_icmp_port'])

        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
        #self.failIfEqual(op_ping,False,"Ping through tunnel failed after permit acl applied")
	
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,False,"Not Pinging after permit  acl applied")
	...
	op_ping = self.linux.ping(script_var['standby_slot4_ip'])
        self.failIfEqual(op_ping,False,"Ping to port failed during reload")
        ...
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","25")
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,False,"Ping to port failed during switchback")

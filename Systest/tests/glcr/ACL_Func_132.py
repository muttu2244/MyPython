...
DESCRIPTION:Verify permit/deny IGMP <src> <dest> [<msg-type>] [precedence <level>] [tos <level>] - for all values of msg-type, when bound to tunnel for both inbound and outbound.  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_132.py

HOW TO RUN:python2.5 ACL_Func_132
...
class test_ACL_Func_132(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_igmp_out_options'])

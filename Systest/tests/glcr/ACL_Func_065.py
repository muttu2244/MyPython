...
DESCRIPTION:Verify ACL functionality for IGMP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Deny for inbound and Permit for outbound]  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_065.py

HOW TO RUN:python2.5 ACL_Func_065
...
class test_ACL_Func_065(test_case):
    myLog = getLogger()
    def setUp(self):

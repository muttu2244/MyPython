...
DESCRIPTION:To Verify that File Transfer(SCP,SFTP,TFTP) thru tunnel can be governed by ACL    
TEST PLAN: ACL Test plans
TEST CASES:GRED_ACL_TUN_FUN_014.py

HOW TO RUN:python2.5 GRED_ACL_TUN_FUN_014
...
class test_GRED_ACL_TUN_FUN_014(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#ping before acl applied
        ...
        self.failIfEqual(op_ping,0,"Ping through tunnel failed before acl applied")
	...
	time.sleep(10)
        ...
        #ftp  operation
        ping_op = self.linux.cmd("!sudo sftp regress@%s:/xpm/ike.log /home/regress"%(script_var['linux1']),timeout=500)

	print "**************PING VALUE %s*********************"%ping_op
	if 'Connection timed out' not in ping_op:
	    self.failIf(ping_op , "sftp copy must not happen as the deny acl is applied")
	
	ping_op = self.linux.cmd("!sudo scp /xpm/ike.log regress@%s:/home/regress"%(script_var['linux1']),timeout=500)
	if 'Connection timed out' not in ping_op:
            self.failIf(ping_op , "sftp copy must not happen as the deny acl is applied")	


	self.ssx.cmd("show port counters")

	tun_count = self.cmd("show tunnel counters | grep %s"%script_var['tunnel_name'])
	tun_break=tun_count.split()
	in_pkts = tun_break[2]
	out_pkts = tun_break[3]
	denied_pkts = in_pkts - out_pkts	

        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"0",denied_pkts,"0","0","0")
	...
	tun_count = self.cmd("show tunnel counters | grep %s"%script_var['tunnel_name'])
        ...
        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],out_pkts,"0","0","0","0")
        self.failIfEqual(acl_cntrs,False , "Permit Acl counters are not incrementing ")	
    ...
    def test_GRED_ACL_TUN_FUN_014(self):

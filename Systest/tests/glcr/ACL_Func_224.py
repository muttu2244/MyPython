...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Native Vlan enabled port when inbound and outbound ACLs applied to Native Vlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_224.py

HOW TO RUN:python2.5 ACL_FUNC_224
...
class test_ACL_FUNC_224(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_icmp_port_untag'])
	...
	self.ssx.config_from_string(script_var['permit_in_icmp_port_untag'])

...
DESCRIPTION:Verify ACL functionality for ICMP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Deny for inbound and Permit for outbound]  
TEST PLAN: ACL Test plans
TEST CASES:test_ACL_Func_061.py

HOW TO RUN:python2.5 test_ACL_Func_061
...
class test_ACL_Func_061(test_case):
    myLog = getLogger()
    def setUp(self):

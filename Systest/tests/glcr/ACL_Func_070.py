...
DESCRIPTION:Verify ACL functionality for TCP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Permit for inbound and Permit for outbound]   
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_070.py

HOW TO RUN:python2.5 ACL_Func_070
...
class test_ACL_Func_070(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	# Push ACL config
        self.ssx.config_from_string(script_var['permit_tcp_out'])

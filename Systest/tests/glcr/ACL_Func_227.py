...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Native Vlan enabled port when inbound and outbound ACLs applied to Native Vlan enabled port [Deny for inbound and deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_227.py

HOW TO RUN:python2.5 ACL_FUNC_227
...
class test_ACL_FUNC_227(test_case):
    myLog = getLogger()
    def setUp(self):

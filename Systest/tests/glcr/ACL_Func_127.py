...
DESCRIPTION: Verify permit/deny IGMP <src> <dest> [<msg-type>] [precedence <level>] [tos <level>] - for all values of ToS, when bound to session for both inbound and outbound. 
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_127.py

HOW TO RUN:python2.5 ACL_Func_127
...
class test_ACL_Func_127(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['permit_igmp_sess_in_options'])
	...
	self.ssx.config_from_string(script_var['deny_igmp_sess_out_options'])

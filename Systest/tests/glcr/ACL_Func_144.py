...
DESCRIPTION:Verify permit/deny TCP <src> <port> <dest> <port> [established] [precedence <level>] [tos <level>] - for all values of ToS, when bound to tunnel for both inbound and outbound.   
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_144.py

HOW TO RUN:python2.5 ACL_Func_144
...
class test_ACL_Func_144(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	time.sleep(5)
        ...
        self.ssx.config_from_string(script_var['deny_tcp_out_options'])
	time.sleep(10)
	...
	# Push ACL config
        self.ssx.config_from_string(script_var['permit_tcp_in_options'])

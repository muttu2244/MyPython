...
DESCRIPTION: Verify ACL functionality for UDP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Deny for inbound and Deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_223.py

HOW TO RUN:python2.5 ACL_FUNC_223
...
class test_ACL_FUNC_223(test_case):
    myLog = getLogger()
    def setUp(self):

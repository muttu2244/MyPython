...
DESCRIPTION:Verify ACL functionality for ICMP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC] 
TEST PLAN: ACL Test plans
TEST CASES:test_ACL_Func060.py

HOW TO RUN:python2.5 test_ACL_Func060.py
...
class test_ACL_Func_060(test_case):
    myLog = getLogger()
    def setUp(self):

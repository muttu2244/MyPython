...
DESCRIPTION: Verify permit/deny UDP <src> <port> <dest> <port> [precedence <level>] [tos <level>] - for all values of ToS, when bound to port for both inbound and outbound.
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_152.py

HOW TO RUN:python2.5 ACL_FUNC_152
...
class test_ACL_FUNC_152(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_udp_port_options'])
	...
	self.ssx.config_from_string(script_var['permit_in_udp_port_options'])

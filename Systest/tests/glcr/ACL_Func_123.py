...
DESCRIPTION:Verify permit/deny ICMP <src> <dest> [<msg-type>] [<code>] [precedence <level>] [tos <level>] - for all values of ToS, when bound to session for both inbound and outbound  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_123.py

HOW TO RUN:python2.5 ACL_Func_123
...
class test_ACL_Func_123(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	#op_ping = self.linux2.cmd("!ping %s -I %s"%(script_var['linux1'],script_var['ip_pool']))
	...
	self.linux1.write_to_file(script_var['rad_users_acl_in'],"users","/a/radius1/etc/raddb/")
        self.ssx.config_from_string(script_var['permit_icmp_sess_in_options'])
	...
	self.myLog.info("\nStep1: Checking session state After the second ACL applied\n")
        #output_dut2 = check_ike_session_brief(self.ssx)
        #self.failUnless(output_dut2 , "Sessions Have Gone Down")
        ses_yn = self.ssx.cmd("show ike-session brief")
        self.myLog.output("show ike-session brief : %s"%ses_yn)
	...
	#Push ACL config
	self.ssx.config_from_string(script_var['deny_icmp_sess_out_options'])	

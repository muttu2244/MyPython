...
DESCRIPTION: Verify permit/deny TCP <src> <port> <dest> <port> [established] [precedence <level>] [tos <level>] - for all values of ToS, when bound to port for both inbound and outbound.
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_142.py

HOW TO RUN:python2.5 ACL_FUNC_142
...
class test_ACL_FUNC_142(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_out_tcp_port_options'])
	...
	#op_ping = self.linux.ping(script_var['active_slot2_ip'])
        ...
        #self.failIfEqual(tun_cntrs,0 , "Tunnel counters must not exist")

	#self.linux.cmd("!sudo /usr/local/bin/nemesis tcp -S %s -D %s"%( script_var['xpVpnHstIp'], script_var['linux1']))

	
	#ftp  operation
        ping_op = self.linux.cmd("!sudo sftp regress@%s:/xpm/autoexec.cfg /home/regress"%(phy_linux['ip_addr']),timeout=500)
        if 'Connection timed out' not in ping_op:
            self.failIf(ping_op , "sftp copy must not happen as the deny acl is applied")

        ping_op = self.linux.cmd("!sudo scp /xpm/autoexec.cfg regress@%s:/home/regress"%(phy_linux['ip_addr']),timeout=500)
        if 'Connection timed out' not in ping_op:
            self.failIf(ping_op , "sftp copy must not happen as the deny acl is applied")
        ...
        self.failIfEqual(acl_cntrs,False , "Deny Acl counters are not incrementing ")
	...
	self.ssx.config_from_string(script_var['permit_in_tcp_port_options'])
	...
	#self.linux.cmd("!sudo /usr/local/bin/nemesis tcp -S %s -D %s"%( script_var['xpVpnHstIp'], script_var['linux1']))
	ping_op=self.linux.cmd("!sudo sftp regress@%s:/xpm/simtriplets.txt  /home/regress"%(phy_linux['ip_addr']),timeout=500)
        self.failUnless(ping_op , "sftp copy is failing")
        ping_op=self.linux.cmd("!sudo scp /xpm/simtriplets.txt regress@%s:/home/regress"%(phy_linux['ip_addr']),timeout=500)
        self.failUnless(ping_op , "sftp copy is failing")
        time.sleep(5)
        out=self.ssx.cmd("show ip counters tcp")
        time.sleep(5)
        self.myLog.output("the output of ftp is %s"%(out))
        self.failUnless("3"  in  out.split()[21]  ,"ftp through interface failed when acl applied")

        time.sleep(65)
	
	self.ssx.cmd("show ip counters tcp")

        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name'],"1","0","0","0","4")
        self.failIfEqual(acl_cntrs,False , "Permit Acl counters are not incrementing ")
	...
	# Reload the card
        ...
        op_ping = self.linux.ping(script_var['active_slot2_ip'])
	self.failIfEqual(op_ping,False,"Ping through tunnel failed during reload")
...
if __name__ == '__main__':
	filename = os.path.split(__file__)[1].replace('.py','.log')
        ...
        suite = test_suite()
        suite.addTest(test_ACL_FUNC_142)

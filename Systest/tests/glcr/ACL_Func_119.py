...
DESCRIPTION:Verify permit/deny ICMP <src> <dest> [<msg-type>] [<code>] [precedence <level>] [tos <level>] - for all values of ToS, when bound to tunnel for both inbound and outbound. 
TEST PLAN: ACL Test plans
TEST CASES:test_ACL_Func119.py

HOW TO RUN:python2.5 test_ACL_Func119.py
...
class test_ACL_Func_119(test_case):
    myLog = getLogger()
    def setUp(self):

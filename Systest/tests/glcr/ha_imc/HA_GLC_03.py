...
DESCRIPTION: Create many tunnels on a GLC and produce coredump on non restartable process
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_GLC_03
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_GLC_03.py
...
class test_HA_GLC_03(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        imcNonRestartProcesses=haimc_var['kill_imc_non_rest_process'].split()
        for imcNonRestartProcess in imcNonRestartProcesses :        
          self.myLog.info("\n\n########killing non restartable process on IMC.Process should reloade IMC card...\n")
          self.Resp.kill_process(imcNonRestartProcess,1,6)
          if imcNonRestartProcess == "NSM" : 
             self.myLog.info("\n\n########killing NSM process will also kill the DFN process")
             
             Crit_logs     = Crit_logs + 4     
          else :
             Crit_logs     = Crit_logs + 2
          IMC_Switch = IMC_Switch + 1
          Card_Reset = Card_Reset + 1
          out = self.Resp.cmd("show syscount")
          self.myLog.info("\n\n######## After killing non restartable process %s on IMC .\n show syscount : \n %s \n\n"%(imcNonRestartProcess,out))
          time.sleep(120)
          self.Resp.telnet()
          #===================  Verify tunnels
          self.myLog.info("\n\n######## Killing non restartable process %s should not have any impact on tunnels ##### \n"%imcNonRestartProcess)

          out = ver_tun_up()
          if out != 0 :
             self.myLog.info("\n\n######## Few tunnels gone down after Killing non restartable process %s"%imcNonRestartProcess)
             self.fail("Could not able to establish %s tunnels in a proper time"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
          self.myLog.info("\n\n##### Verified Killing non restartable process %s not effected on tunnels and all Tunnels %s are UP \n"%(imcNonRestartProcess,(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2']))))

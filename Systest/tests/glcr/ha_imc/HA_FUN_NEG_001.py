...
DESCRIPTION: Reload the Standby card. Execute .system imc-switchover. while standby card is still rebooting
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_FUN_NEG_001
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_FUN_NEG_001.py
...
class test_HA_FUN_NEG_001(test_case):
    myLog = getLogger()
	...
	while(1):
        ...
        for cardIndex in cardIndexList :
            ...
            self.myLog.info("\n\n######## Reload the standby %snd card and verify the %stunnels \n"%(cardIndex,int(haimc_var['max_tuns_slot2'])))
            self.Resp.cmd("reload card %s"%cardIndex)
            Card_Reset = Card_Reset + 1
            
            while 1 :
               cardState = self.Resp.cmd("show card | grep -i \"GLC1\" | grep -i \"4 \"")
               if "Booting" in cardState :
                  break
                  
            #================== Do imc-switchover
            self.myLog.info("\n\n######## Do IMC-switchover while standby card is in booting state##### \n")
            self.Resp.imc_switchover_mgmt("system imc-switchover")
            IMC_Switch = IMC_Switch + 1
            Card_Reset = Card_Reset + 1
            self.Resp.wait4cards()
            ...
            #===================  Verify tunnels
            self.myLog.info("\n\n######## Verify reloading standby card should not have any impact on tunnels ##### \n")
            cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
            if cntOp != True :
...
if __name__ == '__main__':
        ...
        suite = test_suite()
        suite.addTest(test_HA_FUN_NEG_001)

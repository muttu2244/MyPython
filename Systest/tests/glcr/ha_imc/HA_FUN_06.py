...
DESCRIPTION: Produce a core dump on  all restartable processes on the standby card. 

TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_FUN_006
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_FUN_006.py
...
class test_HA_FUN_006(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        #VERIFICATION  STEPS
        #===============================
        #==================== Verify SSX behaviour when restartable process kill on XGLC, which causes card dumping
        cardIndexList = [4]
        for cardIndex in cardIndexList :
            self.myLog.info("\n\n######## Verify SSX behaviour when restartable process kill on standby XGLC, which causes card dumping\n")
            glcRestartProcesses = haimc_var['kill_glc_resta_process'].split() 
            for glcRestartProcess in glcRestartProcesses :
              self.myLog.info("\n\n######## Kill the restartable process %s on standby XGLC, which causes card dumping "%glcRestartProcess)
              self.myLog.info("\n\n######## Stadby Card should come up without any issues")
              ...
              time.sleep(20)
              #================ Verify tunnels
              self.myLog.info("\n\n######## Verify tunnels on Active XGLCS not effected after Kill the restartable process %s on standby GLC##### \n"%glcRestartProcess)
              self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
              cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
              self.failIf(cntOp == False , "tunnels are on Active XGLCS effected after Kill the restartable process %s on GLC"%glcRestartProcess)
              self.Resp.wait4cards()

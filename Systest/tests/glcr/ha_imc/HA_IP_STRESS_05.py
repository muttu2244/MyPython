...
DESCRIPTION: Establish many tunnels and switchover several times
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_IP_STRESS_05
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_IP_STRESS_05.py
...
class test_HA_IP_STRESS_05(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        cardIndexList = [2,3]
        for iter in range(0,int(haimc_var['num_of_iter'])):
          for cardIndex in cardIndexList :
            ...
            ver_tun_up()
            self.myLog.info("\n\n##### Verified tunnels swithced back to Home slot and all Tunnels %s are UP \n"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
...
if __name__ == '__main__':
        ...
        suite = test_suite()
        suite.addTest(test_HA_IP_STRESS_05)

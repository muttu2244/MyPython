...
DESCRIPTION: Produce a core dump on non restartable process on GLC and while GLC restarting switch over IMC

TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_GLC_006
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_GLC_006.py
...
class test_HA_GLC_006(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        #VERIFICATION  STEPS
        #===============================
        #======================== Verify SSX behaviour when nonrestartable process kill on GLC,which doesnot effect card Restarting
        cardIndexList = ["2","3"]
        for cardIndex in cardIndexList :
            ...
            glcNonRestartProcesses = ["%s"%glcNonRestartProcesses[0]]
            for glcNonRestartProcess in glcNonRestartProcesses :
              self.myLog.info("\n\n######## Kill the nonrestartable process %s on GLC, which causes card Restarting and tunnel switched to standby card"%glcNonRestartProcess)
            ...
            while 1 :
               cardState = self.Resp.cmd("show card | grep -i \"GLC1\" | grep -i \" %s\""%cardIndex)
               if "Restarting" in cardState :
                  break

            #================== Do imc-switchover
            self.myLog.info("\n\n######## Do IMC-switchover while  card is in Restarting state##### \n")
            self.Resp.imc_switchover_mgmt("system imc-switchover")
            IMC_Switch = IMC_Switch + 1
            Card_Reset = Card_Reset + 1
            self.Resp.wait4cards()
            ...
            #================ Verify tunnels
            self.myLog.info("\n\n######## Verify tunnels during XGLC failover##### \n")
            self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
            cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
            self.failIf(cntOp == False , "tunnels are not go to standby card during XGLC failover ")
            ...
            while 1 :
               cardState = self.Resp.cmd("show card | grep -i \"GLC1\" | grep -i \" 4\"")
               if "Restarting" in cardState :
                  break
            ...
            self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
            cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
            self.failIf(cntOp == False , "tunnels are not swithced back to Home slot")

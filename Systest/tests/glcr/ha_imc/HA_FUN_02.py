...
HOW TO RUN : python2.5 HA_FUN_02.py
...
class test_HA_FUN_02(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        for imcRestartProcess in imcRestartProcesses :        
          ...
          self.Resp.kill_process(imcRestartProcess,1,6)      
          ...
          proc = self.Resp.cmd("show process")
          print "###############################################"
          print "%s not in %s"%(imcRestartProcess,proc)
          print "###############################################"
          self.failIf('%s'%imcRestartProcess not in '%s'%proc, "Restartable Process %s not got started after killing on IMC"%imcRestartProcess) 

          #===================  Verify tunnels
          self.myLog.info("\n\n######## Killing restartable process %s should not have any impact on tunnels ##### \n"%imcRestartProcess)
          out = ver_tun_up()
          if out != 0 :
             self.myLog.info("\n\n#####Few tunnels gone down after Killing restartable process %s"%imcRestartProcess)
             self.fail("Could not able to establish %s tunnels in a proper time"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
          self.myLog.info("\n\n##### Verified Killing restartable process %s not effected on tunnels and all Tunnels %s are UP \n"%(imcRestartProcess,(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2']))))

        #==================== Verify SSX behaviour when restartable process kill on , which causes card dumping
        cardIndexList = ["2","3"]
        for cardIndex in cardIndexList :
            self.myLog.info("\n\n######## Verify SSX behaviour when restartable process kill on GLC, which causes card dumping\n")
            glcRestartProcesses = haimc_var['kill_glc_resta_process_dump_card'].split() 
            for glcRestartProcess in glcRestartProcesses :
              self.myLog.info("\n\n######## Kill the restartable process %s on GLC, which causes card dumping and tunnel switched to standby card"%glcRestartProcess)
              ...
              pid = process_list[glcRestartProcess]['pid']
              # Kill the process
              self.myLog.info("About to kill the process %s on GLC-%s. PID:%s" %(glcRestartProcess,cardIndex,pid))
              ...
              time.sleep(20)
              #================ Verify tunnels
              self.myLog.info("\n\n######## Verify tunnels swithced back after Kill the restartable process %s on GLC##### \n"%glcRestartProcess)
              self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
              out = ver_tun_up()
              if out != 0 :
                 self.myLog.info("\n\n#####tunnels are not swithced back after Kill the restartable process %s on GLC"%glcRestartProcess)
                 self.fail("Could not able to establish %s tunnels in a proper time"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))

              cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
              self.failIf(cntOp == False , "tunnels are not swithced back after Kill the restartable process %s on GLC"%glcRestartProcess)
              ...
              out = ver_tun_up()
              if out != 0 :
                  self.fail("tunnels are not swithced back to home slot")
              
        #======================== Verify SSX behaviour when restartable process kill on GLC,which doesnot effect card dumping
        cardIndexList = ["2","3"]
        for cardIndex in cardIndexList :
            self.myLog.info("\n\n######## Verify SSX behaviour when restartable process kill on GLC, which causes only core\n")
            glcRestartProcesses = haimc_var['kill_glc_resta_process_dont_dump_card'].split()
            for glcRestartProcess in glcRestartProcesses :
              self.myLog.info("\n\n######## Kill the restartable process %s on GLC, which causes card dumping and tunnel switched to standby card"%glcRestartProcess)
              # Retrieve the process list
              process_list = show_process(self.Resp, "slot %s"%cardIndex)
              ...
              self.myLog.info("\n\n########Verify Process got Restarted ...\n")
              proc = self.Resp.cmd("show process")
              self.failIf('%s'%glcRestartProcess not in '%s'%proc, "Restartable Process not got started after killing on GLC")

              #================ Verify tunnels
              self.myLog.info("\n\n######## Verify tunnels not effected Kill the restartable process %s on GLC##### \n"%glcRestartProcess)
              self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
              out = ver_tun_up()
              if out != 0 :
                  self.fail("tunnels are not effected after Kill the restartable process %s on GLC"%glcRestartProcess)

...
DESCRIPTION: Switch over IMC when GLC card is in Loading Image state
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_GLC_009
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_GLC_009.py
...
class test_HA_GLC_009(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        for cardIndex in cardIndexList :
            ...
            #================ Verify tunnels
            self.myLog.info("\n\n######## Verify tunnels go to standby card after GLC failover ##### \n")
            self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
            cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
            self.failIf(cntOp == False , "tunnels not go to standby card after GLC failover")

            Card_Reset = Card_Reset + 1
            while 1 :
               cardState = self.Resp.cmd("show card | grep -i \"GLC1\" | grep -i \" %s\""%cardIndex)
               if "Loading Image" in cardState :
                  break

            #================== Do imc-switchover
            self.myLog.info("\n\n######## Do IMC-switchover while  card is in Loading Image state##### \n")
            self.Resp.imc_switchover_mgmt("system imc-switchover")
            IMC_Switch = IMC_Switch + 1
            Card_Reset = Card_Reset + 1
            self.Resp.wait4cards()
            ...
            #================ Verify tunnels
            self.myLog.info("\n\n######## Verify tunnels after Do IMC-switchover when  card is in Loading Image state##### \n")
            self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
            cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
            self.failIf(cntOp == False , "Active card tunnels are not swithced back to stand by after Do IMC-switchover when card is in Loading Image state")

...
DESCRIPTION: Establish    tunnels and clear them several times. Then switch over
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_IP_22
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_IP_22.py
...
class test_HA_IP_22(test_case):
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        #=================  clear tunnels several times and Perform imc-switchover
        self.myLog.info("\n\n######## execute the command \"clear tunnel all\" several times\n")
        for iter in range(0,haimc_var['num_clear_tun_cmds']) :
             self.Resp.cmd("clear tunnel all")
        ...
        itr = 0
        # Verify the all tunnels are up
        self.myLog.info("\n\n######## Wait for %s Tunnels Came UP after executing the command clear tune all after several times:\n"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
        while(1):
                cntOp = verify_ike_session_counters(self.Resp, count=(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))
                if cntOp == True :
                        break

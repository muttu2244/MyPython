...
DESCRIPTION: To Check for Port counters after IMC switchback
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_IMC_CLI_012
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_IMC_CLI_012.py
...
class test_HA_IMC_CLI_012(test_case):
    myLog = getLogger()
    ...
    def test_HA_IMC_CLI_012(self):
        ...
        # This Test Case needs only one tunnel config so adding variable in test case
	...
	while(1):
        ...
        self.Resp.cmd("clear port counters")      
        self.Resp.cmd("cont %s"%haimc_var['context2'])
        self.Resp.cmd("ping %s"%haimc_var['active_slot3_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_standby_4slot3_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_rad_intf_4slot3_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_bkp_rad_intf_4slot3_ip'])
        self.Resp.cmd("cont %s"%haimc_var['context1'])
        self.Resp.cmd("ping %s"%haimc_var['active_slot2_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_standby_4slot2_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_rad_intf_4slot2_ip'])
        self.Resp.cmd("ping %s"%haimc_var['cisco_bkp_rad_intf_4slot2_ip'])
        out = self.Resp.cmd("show port count detail")
        self.myLog.output("\n\n ########### show port counters before IMC switchover ")
        self.myLog.output("%s ########### \n\n"%out)
        ...
        out = self.Resp.cmd("show port count details")
        self.myLog.output("\n\n ########### show port counters after IMC switchback ")
        self.myLog.output("%s ########### \n\n"%out)
...
if __name__ == '__main__':
        ...
        suite = test_suite()
        suite.addTest(test_HA_IMC_CLI_012)

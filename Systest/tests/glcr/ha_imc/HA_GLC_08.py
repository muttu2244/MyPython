...
DESCRIPTION: Switch over IMC when GLC card is in dumping state
TEST MATRIX: 4.6B2_HA-IMC.xls
TEST CASE  : HA_GLC_008
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_GLC_008.py
...
class test_HA_GLC_008(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3%s"%respConfigFile)
        ...
        #VERIFICATION  STEPS
        #===============================
        #======================== Verify SSX behaviour when nonrestartable process kill on GLC,which doesnot effect card dumping
        cardIndexList = ["2","3"]
        for cardIndex in cardIndexList :
            ...
            while 1 :
               ...
               if "Dumping" in cardState :
                  break

            #================== Do imc-switchover
            self.myLog.info("\n\n######## Do IMC-switchover while  card is in dumping state##### \n")
            self.Resp.imc_switchover_mgmt("system imc-switchover")
            IMC_Switch = IMC_Switch + 1
            Card_Reset = Card_Reset + 1
            time.sleep(30)
            ...
            #================ Verify tunnels
            self.myLog.info("\n\n######## Verify tunnels are go to standby after Kill the nonrestartable process %s on GLC##### \n"%glcnonrestartProcess)
            self.myLog.info("ike-session counters info: %s"%self.Resp.cmd("show ike-session counters"))
            out = ver_tun_up()
            if out != 0 :
                 self.myLog.info("\n\n#####tunnels are go to standby after Kill the nonrestartable process %s on GLC"%glcnonrestartProcess)
                 self.fail("Could not able to establish %s tunnels in a proper time"%(int(haimc_var['max_tuns_slot3'])+int(haimc_var['max_tuns_slot2'])))

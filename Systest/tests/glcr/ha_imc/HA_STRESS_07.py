...
TEST CASE  : HA_STRESS_07
TOPOLOGY   : GLC-R Setup 

HOW TO RUN : python2.5 HA_STRESS_07.py
...
class test_HA_STRESS_07(test_case):
    myLog = getLogger()
	...
	loadResp = testIt(self.Resp, "/hd/Card3_resp.cfg")
        ...
        time.sleep(30)
        op = verfiy_rtr_state(self.Resp,rtrId=haimc_var['rtr_id1'])
        self.failIf(int(op) != 0 , "RTR ID - %s is not reachable" %haimc_var['rtr_id1'])
        self.Resp.cmd("cont %s"%haimc_var['context2'])
        op = verfiy_rtr_state(self.Resp,rtrId=haimc_var['rtr_id2'])
        self.failIf(int(op) != 0 , "RTR ID - %s is not reachable" %haimc_var['rtr_id2'])
...
if __name__ == '__main__':
        ...
        suite = test_suite()
        suite.addTest(test_HA_STRESS_07)

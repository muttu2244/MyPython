...
DESCRIPTION:To verify ACL functionality for ICMP traffic through tunnel when ACL bound to Tunnel(Permit/Deny)  
TEST PLAN: ACL Test plans
TEST CASES:test_GRED_ACL_TUN_FUN_001.py

HOW TO RUN:python2.5 test_GRED_ACL_TUN_FUN_001
...
class test_GRED_ACL_TUN_FUN_001(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.cmd("clear ip access-list name %s counters"%script_var['acl_name'])
        ...
        # Push ACL config
        self.ssx.config_from_string(script_var['permit_icmp'])
	...
	#clear the ACL counters before sending the traffic
        ...
        #ping operation after acl applied
        op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
        self.failIfEqual(op_ping,False,"Ping through tunnel failed after permit ICMP acl applied")
    ...
    def test_GRED_ACL_TUN_FUN_001(self):
...
if __name__ == '__main__':
	...
	#tt_faketest_2(testcase_id=1939, build_name='test_GRED_ACL_TUN_FUN_001',testplan_id=14,status=2,product_id=54,environment_id=12,manager_id=1217,version='6.0',tester=1217,build=None,run=None,testcase_run=None)

	#updateResults(2096,"Important Test",'Daily','',14,2,54,12,1217,'6.0',1217,run=None,testcase_run=None)
	#tcRunUpdate(30,54,2111,'Daily',None,12,None,6)

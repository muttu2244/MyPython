...
DESCRIPTION:Verify ACL functionality for ICMP traffic through Native Vlan enabled port when inbound and outbound ACLs applied to Native Vlan enabled port [Permit for inbound and Permit for outbound] 
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_226.py

HOW TO RUN:python2.5 ACL_FUNC_226
...
class test_ACL_FUNC_226(test_case):
    myLog = getLogger()
    def setUp(self):

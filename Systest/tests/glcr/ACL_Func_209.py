...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Vlan enabled port when inbound and outbound ACLs applied to Vlan enabled port [Deny for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_209.py

HOW TO RUN:python2.5 ACL_FUNC_209
...
class test_ACL_FUNC_209(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_in_icmp_port'])

...
DESCRIPTION:Verify permit/deny UDP <src> <port> <dest> <port> [precedence <level>] [tos <level>] - for all values of precedence, when bound to tunnel for both inbound and outbound.  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_151.py

HOW TO RUN:python2.5 ACL_Func_151
...
class test_ACL_Func_151(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.linux.cmd("!sudo ping %s -I %s -c 2"%(script_var['linux1'], script_var['xpVpnHstIp']))
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['deny_udp_out_options'])
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['permit_udp_in_options'])

...
DESCRIPTION: Verify ACL functionality for IGMP traffic through session when inbound and outbound ACLs applied to session [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC] 
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_080.py

HOW TO RUN:python2.5 ACL_Func_080
...
class test_ACL_Func_080(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['permit_igmp_sess_in'])
	...
	tunCnt = self.ssx.cmd("show session counters")
        ...
        rx_count  = tunCnt.splitlines()[-1].split()[2]
        tx_count = tunCnt.splitlines()[-1].split()[3]
        if not (rx_count == '10' and tx_count == '1'):
                self.fail("Session counters are not matching Rx Count = %s Tx Count = %s"%(rx_count,tx_count))
	...
	acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name1'],"0","0","0","1","4")
        self.failIf(acl_cntrs != True , "Acl counters are not incrementing ")
	...
	self.ssx.config_from_string(script_var['deny_igmp_sess_out'])

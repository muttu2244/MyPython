...
DESCRIPTION: Verify ACL functionality for UDP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [ permit inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_222.py

HOW TO RUN:python2.5 ACL_FUNC_222
...
class test_ACL_FUNC_222(test_case):
    myLog = getLogger()
    def setUp(self):

...
DESCRIPTION: Verify ACL functionality for TCP traffic through session when inbound and outbound ACLs applied to session [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC] 
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_084.py

HOW TO RUN:python2.5 ACL_Func_084
...
class test_ACL_Func_084(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.linux1.cmd("sudo /usr/local/bin/nemesis tcp -D %s -S %s"%( script_var['ip_pool'], script_var['linux1']))
        time.sleep(5)
	...
	self.ssx.cmd("config")
        ...
        self.ssx.cmd("end")
	self.ssx.config_from_string(script_var['permit_tcp_sess_in'])
	...
	tunCnt = self.ssx.cmd("show session counters")
        ...
        rx_count  = tunCnt.splitlines()[-1].split()[2]
        tx_count = tunCnt.splitlines()[-1].split()[3]
        if not (rx_count == '5' and tx_count == '1'):
                self.fail("Session counters are not matching Rx Count = %s Tx Count = %s"%(rx_count,tx_count))
	...
	self.ssx.config_from_string(script_var['deny_tcp_sess_out'])

...
DESCRIPTION: Verify ACL functionality for TCP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Deny for inbound and Deny for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_219.py

HOW TO RUN:python2.5 ACL_FUNC219
...
class test_ACL_FUNC_219(test_case):
    myLog = getLogger()
    def setUp(self):

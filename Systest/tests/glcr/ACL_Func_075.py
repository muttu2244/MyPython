...
DESCRIPTION:Verify ACL functionality for UDP traffic through tunnel when inbound and outbound ACLs applied to tunnel bound to Tunnel [deny for inbound and deny for outbound]  
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_075.py

HOW TO RUN:python2.5 ACL_Func_075
...
class test_ACL_Func_075(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.linux.cmd("!sudo ping %s -I %s -c 2"%(script_var['linux1'], script_var['xpVpnHstIp']))
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['deny_udp_in'])

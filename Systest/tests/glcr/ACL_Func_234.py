...
DESCRIPTION: Verify ACL functionality for TCP traffic through native vlan enabled port when inbound and outbound ACLs applied to native vlan enabled port [ permit inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_234.py

HOW TO RUN:python2.5 ACL_FUNC_234
...
class test_ACL_FUNC_234(test_case):
    myLog = getLogger()
    def setUp(self):

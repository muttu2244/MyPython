...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Vlan enabled port when inbound and outbound ACLs applied to Vlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound mapped to single port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_208.py

HOW TO RUN:python2.5 ACL_FUNC_208
...
class test_ACL_FUNC_208(test_case):
    myLog = getLogger()
    def setUp(self):

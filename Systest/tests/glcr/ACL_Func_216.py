...
DESCRIPTION: Verify ACL functionality for TCP traffic through vlan enabled port when inbound and outbound ACLs applied to vlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_216.py

HOW TO RUN:python2.5 ACL_FUNC_216
...
class test_ACL_FUNC_216(test_case):
    myLog = getLogger()
    def setUp(self):

...
DESCRIPTION: Verify ACL functionality for IGMP traffic through vlan enabled port when inbound and outbound ACLs applied tovlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_212.py

HOW TO RUN:python2.5 ACL_FUNC_212
...
class test_ACL_FUNC_212(test_case):
    myLog = getLogger()
    def setUp(self):
    ...
    def tunnelUp(self):
        ...
        op_client_cmd = self.linux.cmd("sudo ./iked")
        time.sleep(10)
	...
	self.ssx.config_from_string(script_var['permit_in_igmp_port'])
        ...
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,True,"Ping through tunnel must failed as igmp permit acl applied")

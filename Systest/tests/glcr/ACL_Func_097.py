...
DESCRIPTION:Verify ACL functionality for IGMP traffic through port when inbound and outbound ACLs applied to port [Deny for inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_097.py

HOW TO RUN:python2.5 ACL_FUNC_097
...
class test_ACL_FUNC_097(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['permit_out_igmp_port'])
        ...
        #op_ping = self.linux.ping_xpress_vpn(script_var['xpVpnHstIp'],script_var['linux1'],"500","4")
	op_ping = self.linux.ping(script_var['active_slot2_ip'])
        self.failIfEqual(op_ping,True,"Ping through must fail after acl applied")

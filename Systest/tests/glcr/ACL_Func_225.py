...
DESCRIPTION: Verify ACL functionality for ICMP traffic through Native Vlan enabled port when inbound and outbound ACLs applied to Native Vlan enabled port [Deny for inbound and Permit for outbound
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_225.py

HOW TO RUN:python2.5 ACL_FUNC_225
...
class test_ACL_FUNC_225(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	self.ssx.config_from_string(script_var['deny_in_icmp_port_untag'])
	...
	self.ssx.config_from_string(script_var['permit_out_icmp_port_untag'])

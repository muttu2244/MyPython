...
DESCRIPTION: Verify ACL functionality for IGMP traffic through native vlan enabled port when inbound and outbound ACLs applied to native vlan enabled port [Permit for inbound and Deny for outbound] [both inbound and outbound are mapped to same physical port on GLC]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_228.py

HOW TO RUN:python2.5 ACL_FUNC_228
...
class test_ACL_FUNC_228(test_case):
    myLog = getLogger()
    def setUp(self):
    ...
    def cisco_linux_config(self):
        ...
        self.ssx.config_from_string(script_var['test_ACL_untagged_port'])
	...
	self.ssx.config_from_string(script_var['deny_out_igmp_port_untag'])
	...
	self.ssx.config_from_string(script_var['permit_in_igmp_port_untag'])

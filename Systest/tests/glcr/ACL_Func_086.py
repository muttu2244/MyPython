...
DESCRIPTION: Verify ACL functionality for TCP traffic through session when inbound and outbound ACLs applied to session [Permit for inbound and Permit for outbound] 
TEST PLAN: ACL Test plans
TEST CASES:ACL_Func_086.py

HOW TO RUN:python2.5 ACL_Func_086
...
class test_ACL_Func_086(test_case):
    myLog = getLogger()
    def setUp(self):
	...
	tunCnt = self.ssx.cmd("show session counters")
        ...
        acl_cntrs = verify_access_list_counters(self.ssx,script_var['acl_name1'],"1","4","0","0","0")
	...
	self.ssx.cmd("config")
        ...
        self.ssx.cmd("no 20 permit tcp any any")
        self.ssx.cmd("exit")
	...
	tunCnt = self.ssx.cmd("show session counters")
        ...
        rx_count  = tunCnt.splitlines()[-1].split()[2]
        tx_count = tunCnt.splitlines()[-1].split()[3]
        if not (rx_count == '10' and tx_count == '2'):
                self.fail("Session counters are not matching Rx Count = %s Tx Count = %s"%(rx_count,tx_count))	

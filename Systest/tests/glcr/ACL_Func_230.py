...
DESCRIPTION: Verify ACL functionality for IGMP traffic through native vlan enabled port when inbound and outbound ACLs applied to native vlan enabled port [ permit inbound and Permit for outbound]
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUNC_230.py

HOW TO RUN:python2.5 ACL_FUNC_230
...
class test_ACL_FUNC_230(test_case):
    myLog = getLogger()
    def setUp(self):

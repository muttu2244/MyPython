...
DESCRIPTION:Send ICMP packets of more 65000bytes with both source and destination address as SSX IP.
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_086
...
HOW TO RUN:python2.5 ip_atk_086.py
...
class test_ip_atk_086(test_case):
    myLog = getLogger()	
   ...
   #     self.ssx.clear_config()
	...
	self.ssx.cmd("clear log debug")
 	...
 	#Send ICMP packets of more 65000bytes with both source and destination address as SSX IP.
	self.linux1.cmd("sudo /usr/sbin/hping -1 -p 23  -a %s %s  -d 65000 --flood &"%(script_var['ssx_ip_addr'],script_var['ssx_ip_addr']))
        time.sleep(40)

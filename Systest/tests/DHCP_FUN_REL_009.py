...
DESCRIPTION:To verify SSX (relay agent) relays the DHCP reply message from DHCP server to  many DHCP subscribers to.
 
TEST PLAN: Sanity Test plans
TEST CASES: DHCP-FUN-REL-009
...
HOW TO RUN:python2.5 DHCP-FUN-REL-009.py
...
class test_DHCP_FUN_009(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_009'])
	 ...
	 #configure ip interface on linux machine
        ...
        #adding route 
        self.linux2.add_route(script_var['ssx_client_ip_mask'],script_var['ssx_server2_ip_addr'],p1_ssx_linux2[1])

...
DESCRIPTION:Send IP header with option filled with option Type( 8 bit set) 136 and length( 8 bits) set to 4 and stream IP with any value to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_006
...
HOW TO RUN:python2.5 ip_atk_006.py
...
class test_ip_atk_006(test_case):
    myLog = getLogger()	
	...
	dst_ip_adr = script_var['linux1_ip_addr']
        ...
        invoke_scapy(self.linux1,dst_mac_adr = dst_mac_address,sudo_passwd=linux1['password'],dst_ip_adr = dst_ip_adr ,from_interface=p1_ssx_linux1[1], ip_options="\x88\x04\x80\x80",log = self.myLog,count = 1000)

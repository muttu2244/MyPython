...
DESCRIPTION:Send fragments of ICMP packets
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_088
...
HOW TO RUN:python2.5 ip_atk_088.py
...
class test_ip_atk_088(test_case):
    myLog = getLogger()	
   ...
   #     vg_output1 = vgroup_new(topo3[:])
	...
	Ping_out = self.ssx.ping(script_var['linux1_ip_addr'])
 	...
 	#Send fragments of ICMP packets
	self.linux1.cmd("sudo /usr/sbin/hping -V -p 22 -f %s -I %s --flood &"%(script_var['ssx_ip_addr'],p1_ssx_linux1[1]))
        time.sleep(40)

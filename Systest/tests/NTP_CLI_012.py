...
DESCRIPTION: Verify the configured NTP parameters can be deleted
TEST PLAN:NTP CLI Test plans
TEST CASES: NTP_CLI_012
...
HOW TO RUN: python2.5 NTP_CLI_012.py
...
class test_NTP_CLI_012(test_case):
    myLog = getLogger()
	...
	self.ssx.cmd("config")
        ...
        self.ssx.cmd("server %s key %s source %s version %s prefer"%(var_dict['infc_linux1_ip'],var_dict['key'],var_dict['infc_r1_ip'],var_dict['version']))
        self.ssx.cmd("exit")
        self.ssx.cmd("end")
	...
	self.myLog.output("==================Removing ntp configuration====================")

        self.ssx.cmd("config")
        self.ssx.cmd("context %s"%var_dict['context_name'])
        self.ssx.cmd("ntp profile")
        self.ssx.cmd("no server %s key %s source %s version %s prefer"%(var_dict['infc_linux1_ip'],var_dict['key'],var_dict['infc_r1_ip'],var_dict['version']))
        self.ssx.cmd("exit")
        self.ssx.cmd("end")
        ...
        if "server %s"%var_dict['infc_linux1_ip'] not in op and "key %s"%var_dict['key'] not in op and "source %s"%var_dict['infc_linux1_ip'] not in op and "version %s"%var_dict['version'] not in op and "prefer" not in op:
		self.myLog.output("NTP config deleted")
        else:
                self.failIf(True, "NTP config not deleted")
...
if __name__ == '__main__':
	...
	suite = test_suite()
	suite.addTest(test_NTP_CLI_012)

...
DESCRIPTION: Use hping to send packets with bad checksum
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_048
...
HOW TO RUN:python2.5 ip_atk_048.py
...
class test_ip_atk_048(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_040'])
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send TCP packets with SYN and FIN bits set to SSX.
	out = self.linux1.cmd("sudo /usr/sbin/hping -b %s -p 23 -i u1 & "%script_var['ssx_ip_addr'])
        time.sleep(40)

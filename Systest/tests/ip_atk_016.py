...
DESCRIPTION:Fragments-reassembly off by one IP header (Nestea Attack)

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_016
...
HOW TO RUN:python2.5 ip_atk_016.py
...
class test_ip_atk_016(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_016'])
	...
	out = self.linux1.cmd("sudo ./nestea %s %s -s 22 -t 22 -n 10000000 &"%(script_var['linux1_ip_addr'],script_var['ssx_ip_addr']))
	time.sleep(30)

	#killing hping process
        self.linux1.cmd("sudo pkill -9 nestea") 

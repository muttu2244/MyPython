...
DESCRIPTION:Flood of UDP traffic and then IMC switchover
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_082
...
HOW TO RUN:python2.5 ip_atk_082.py
...
class test_ip_atk_082(test_case):
    myLog = getLogger()	
   ...
   #     vg_output1 = vgroup_new(topo3[:])
	...
	#self.linux1.cmd("sudo ifconfig %s %s"%(p1_ssx_linux1[1],script_var['linux1_ip_addr/m'])) 
	#self.linux1.configure_vlan_interface(p1_ssx_linux1[1],script_var['linux1_ip_addr'],"1")
	#time.sleep(10)
	...
	self.linux1.cmd("sudo /usr/sbin/hping -2 -p 22 %s --flood "%script_var['ssx1_ip_addr'])
        time.sleep(40)
        self.linux1.cmd("sudo pkill -9 hping")

	#IMC-SWITCHOVER(STANDBY-MODE)
	self.ssx.cmd("system imc-switchover")

	time.sleep(100)	
	#IMC-SWITCHOVER(ACTIVE-MODE)
	self.ssx.cmd("system imc-switchover")
	...
	self.myLog.output("Displaying control plane rate limiter %s"%self.ssx.cmd("show dos slot %s counters"%p1_ssx_linux1_slot))
        ...
        self.failUnless(is_healthy(hs,IMC_Switch=2,Card_Reset=2Warn_logs = 100), "Platform is not healthy")

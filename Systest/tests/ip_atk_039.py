...
DESCRIPTION:Send TCP SYN packets with source and destination address as SSX IP address.
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_039
...
HOW TO RUN:python2.5 ip_atk_039.py
...
class test_ip_atk_039(test_case):
    myLog = getLogger()	
	...
	#Send TCP SYN packets with source and destination address as SSX IP address.
	out = self.linux1.cmd("sudo /usr/sbin/hping -S %s -a %s -p 23 -i u10 -I %s  &"%(script_var['ssx_ip_addr'],script_var['ssx_ip_addr'],p1_ssx_linux1[1]))
	time.sleep(40)

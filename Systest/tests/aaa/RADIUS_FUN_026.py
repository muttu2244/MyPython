...
Description: - Verify that the SSX  generates & forwards account-request
	 (stop) when the Phase 2 (XAUTH) Authentication fails after querying the local database.
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-026
...
How to run: "python2.5 RADIUS_FUN_026.py"
...
class test_RADIUS_FUN_026(test_case):
    ...
    def test_RADIUS_FUN_026(self):

        """
        Test case Id: -  RADIUS-FUN-026
        """

	self.myLog.output("\n**********start the test**************\n")

        self.myLog.output(" Step 1 - removing the file rad_fun_026.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_026.pcap -f")

        self.myLog.output("Step 2 -Start tethereal to capture the packets and store the result in a pcap file")
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_026.pcap  & '% topo.port_ssx_radius1[1])
	...
	self.ssx.config_from_string(script_var['rad_fun_026_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_026_ns5gt'])
	...
	time.sleep(5)
        ...
        self.failUnless("rad_fun_026.pcap" in ll_output,"Testcase FAILED because pcap file has not created")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_026.pcap")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_026.pcap  -R "radius.Acct_Status_Type"',timeout = 30)

...
How to run: "python2.5 RADIUS_FUN_014_1.py"
...
class test_RADIUS_FUN_014_1(test_case):
    ...
    def test_RADIUS_FUN_014_1(self):

        """
        Test case Id: -  RADIUS_FUN_014_1
        Description: - Verify that the SSX sends the access-request to 
			the recovered non-responsive Primary server.
        """

	#### Ethereal cap
        self.myLog.output ("Step 1 - removing the file rad_fun_014_1.pcap on pri and sec servers")
        self.ether_radius1.cmd("sudo rm rad_fun_014_1.pcap -f")
        self.ether_radius2.cmd("sudo rm rad_fun_014_1.pcap -f")
        self.myLog.output ("Step 2 -Start tethereal to capture the packets and store the result in file ")
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_014_1.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        self.ether_radius2.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_014_1.pcap -R "radius" & '% topo.port_ssx_radius2[1])

        #Push the SSX configuration
        self.ssx.config_from_string(script_var['common_ssx'])
        self.ssx.config_from_string(script_var['rad_fun_014_1_ssx'])


        # Push xpress vpn config
        self.xpress_vpn.write_to_file(script_var['rad_fun_014_1_xpressvpn'],"autoexec.cfg","/xpm/")
	...
	self.ssx.cmd("sh ike-session brief")
        ...
        self.failUnless( "rad_fun_014_1.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")
        ll_output = self.ether_radius2.cmd("ls -lrt *.pcap")
        self.failUnless( "rad_fun_014_1.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")


        self.myLog.output (" Step 4 - read the content of the file rad_fun_014_1.pcap")

        output_1=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_014_1.pcap -R "radius.code == 5" ',timeout = 30)
	...
	'''
        ...
        output_2=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_014_1.pcap -R "radius.code == 5" ',timeout = 30)
	...
	'''
   	...
   	#self.failUnless( count == 9, """ Expected - 9 packet with radius.code == 5,  Actual = %s packets"""% count)
        #self.failUnless("RADIUS" in output_2 ,'Expected - packet with  acc.accept ')

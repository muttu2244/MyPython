...
Description: -  Verify that a Accounting-request (stop) is sent 
		to the Radius when the session is terminated.
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-018
...
How to run: "python2.5 RADIUS_FUN_018.py"
...
class test_RADIUS_FUN_018(test_case):
    ...
    def test_RADIUS_FUN_018(self):

        """
        Test case Id: -  RADIUS-FUN-018
        """
	...
	self.ssx.config_from_string(script_var['rad_fun_018_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_018_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_018.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_018.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_018.pcap -R "radius" & '% topo.port_ssx_radius1[1])

        # Initiate IKEv1 session from a Netscreen to the SSX
	# ping from ssx to get ip resloved 
	...
	self.myLog.output (" Step 3 - stop tethereal by killing the tethereal application.")
        ...
        self.failUnless( "rad_fun_018.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_018.pcap")
	time.sleep(5)
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_018.pcap  -R "radius.Acct_Status_Type  == 2"',timeout = 30)

...
Description: -    Verify that SSX forwards the session accounting information to radius 
		when 'session accounting radius' is configured in AAA profile.
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: AAA-FUN-027
...
How to run: "python2.5 AAA_FUN_027.py"
...
class test_AAA_FUN_027(test_case):
    ...
    def test_AAA_FUN_027(self):

        """
        Test case Id: -  AAA-FUN-027
        """
	...
	self.ssx.config_from_string(script_var['fun_027_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['fun_027_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        #### Ethereal cap
        self.myLog.output(" Step 1 - removing the file aaa_fun_027.pcap")
        self.ether_radius1.cmd("sudo rm aaa_fun_027.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w aaa_fun_027.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        ...
        self.failUnless( "aaa_fun_027.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output  (" Step 4 - read the content of the file aaa_fun_027.pcap ")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r aaa_fun_027.pcap -R "radius.Acct_Status_Type  == 1"',timeout = 30)
        self.failUnless( "RADIUS" in output, " Expected - packet with radius.Acct.Status_Type == 1 ")

        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r aaa_fun_027.pcap -R "radius.code == 2"',timeout = 30)

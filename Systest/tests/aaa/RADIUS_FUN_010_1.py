...
Description: - Verify that session authentication credentials are 
		...
		Repeating the test with Radius Accounting profile
...
TOPOLOGY DIAGRAM:
  ...
  (Netscreen)                          |        |                            (Linux)
   ...
   |qa-ns1 | --------------------------|        |-------------------------| qa-svr3      |
    ------                             |        |                          --------------
                                        --------
How to run: "python2.5 RADIUS_FUN_010_1.py"
...
class test_RADIUS_FUN_010_1(test_case):
    myLog = getLogger()
    ...
    def test_RADIUS_FUN_010_1(self):
        """
        Test case Id: -  RADIUS_FUN_010_1
	"""
        self.myLog.output("\n**********start the test**************\n")

        # Push SSX config
        self.ssx.config_from_string(script_var['common_ssx'])
        self.ssx.config_from_string(script_var['rad_fun_010_1_ssx'])
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_010_1.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_010_1.pcap -f")
        self.ether_radius2.cmd("sudo rm rad_fun_010_1.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_010_1.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_010_1.pcap -R "radius" & '% topo.port_ssx_radius2[1])
        ...
        self.failUnless( "rad_fun_010_1.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output  (" Step 4 - read the content of the file rad_fun_010_1.pcap ")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_010_1.pcap -R "radius.code == 5" ',timeout = 30)
        self.failUnless("RADIUS" in output, """ Expected - packet with radius.code == 5,  Actual = %s"""% output)

        ll_output = self.ether_radius2.cmd("ls -lrt *.pcap")
	if "rad_fun_010_1.pcap" in ll_output :
	    output=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_010_1.pcap -R "radius.code == 5 "',timeout=30)
	    self.failUnless(not "RADIUS" in output,"Secondary radius sent the accounting response message which was not exected")

...
Description: - Verify that a Accounting-request (start) is sent 
		to the Radius when the session is established .
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-017
...
How to run: "python2.5 RADIUS_FUN_017.py"
...
class test_RADIUS_FUN_017(test_case):
    ...
    def test_RADIUS_FUN_017(self):

        """
        Test case Id: -  RADIUS-FUN-017
        """
	...
	self.ssx.config_from_string(script_var['rad_fun_017_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_017_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_017.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_017.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_017.pcap -R "radius" & '% topo.port_ssx_radius1[1])
	...
	time.sleep(5)
        ...
        self.failUnless( "rad_fun_017.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_017.pcap")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_017.pcap -R "radius.Acct_Status_Type  == 1"',timeout = 30)
        self.failUnless( output, " Expected - packet with radius.Acct.Status_Type == 1 ")
        #self.failUnless("RAdIUS" output, " Expected - packet with radius.Acct.Status_Type == 1 ")

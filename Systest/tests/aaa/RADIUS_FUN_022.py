...
Description: - Verify that the SSX can send session Accouting-request to
            RADIUS server when the session authentication is handled by local database .
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-022
...
How to run: "python2.5 RADIUS_FUN_022.py"
...
class test_RADIUS_FUN_022(test_case):
	...
	self.ssx.config_from_string(script_var['rad_fun_022_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_022_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_022.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_022.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_022.pcap & '% topo.port_ssx_radius1[1])
	...
	time.sleep(15)
        ...
        self.failUnless("rad_fun_022.pcap" in ll_output,"Testcase FAILED because pcap file has not created")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_022.pcap")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_022.pcap -R "radius.Acct_Status_Type  == 1"',timeout = 30)
        self.failUnless("RADIUS" in  output, " Expected - packet with radius.Acct.Status_Type == 1 ")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_022.pcap  -R "radius.Acct_Status_Type  == 2"',timeout = 30)
        ...
        self.failUnless(not sa_output,"SA not loaded even after clearing the session")

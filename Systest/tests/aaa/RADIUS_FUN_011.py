...
TOPOLOGY DIAGRAM:
  ...
  (Netscreen)                          |        |                            (Linux)
   ...
   |qa-ns1 | --------------------------|        |-------------------------| qa-svr3      |
    ------                             |        |                          --------------
                                        --------
How to run: "python2.5 RADIUS_FUN_011.py"
...
class test_RADIUS_FUN_011(test_case):
    myLog = getLogger()
    ...
    def test_RADIUS_FUN_011(self):
        """
        Test case Id: -  RADIUS_FUN_011
	"""

	'''
	self.xpress_vpn.cmd("cd /xpm")
	self.xpress_vpn.cmd("sudo ./start_ike")
        time.sleep(15)
	'''
        ...
        self.ssx.config_from_string(script_var['rad_fun_011_ssx'])
        # Push xpress vpn config
        #self.xpress_vpn.write_to_file(script_var['rad_fun_011_xpressvpn'],"autoexec.cfg","/xpm/")
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_011.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_011.pcap ")
        self.ether_radius2.cmd("sudo rm rad_fun_011.pcap ")
        ...
        self.ether_radius2.cmd('sudo /usr/sbin/tethereal -i %s  -w rad_fun_011.pcap -R "radius" & '% topo.port_ssx_radius2[1])
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s  -w rad_fun_011.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        ...
        self.failUnless( "rad_fun_011.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")
        ll_output = self.ether_radius2.cmd("ls -lrt *.pcap")
        self.failUnless( "rad_fun_011.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output  (" Step 4 - read the content of the file rad_fun_011.pcap ")

        output_1=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_011.pcap -R "radius.code == 2" ',timeout = 30)
	...
	output_2=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_011.pcap -R "radius.code == 2" ',timeout = 30)
	...
	count = count_1 + count_2
        ...
        self.failUnless( count == 4, """ Expected - 4 packet with radius.code == 2,  Actual = %s packets"""% count)
        
        #Consider 9 client & Check SAs clnt_ips from aaa_config file

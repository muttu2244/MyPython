...
How to run: "python2.5 RADIUS_FUN_008_1.py"
...
class test_RADIUS_FUN_008_1(test_case):
    ...
    def test_RADIUS_FUN_008_1(self):
        """
        ...
        #### Ethereal cap
        self.myLog.output(" Step 1 - removing the file rad_fun_008_1.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_008_1.pcap -f")

        self.myLog.output ("Step 2 -Start tethereal to capture the packets and store the result in a pcap file")
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_008_1.pcap -R "radius" & '% topo.port_ssx_radius1[1])

        #Push the SSX configuration        
	self.ssx.config_from_string(script_var['common_ssx1'])
        self.ssx.config_from_string(script_var['rad_fun_008_1_ssx'])
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_008_1_ns5gt'])
	...
	# Initiate IKEv1 session from a client to the SSX with valid X-auth credentials.
        ...
        self.myLog.output  (" Step 4 - read the content of the file rad_fun_008_1.pcap ")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_008_1.pcap -R "radius.code == 4 &&  ! icmp" ',timeout = 100)
	...
	for line in ether_op:
	    if "RADIUS" in line:
		count = count+1
  	self.failUnless(count in [5,6], """ Expected - packet with radius.code == 6 not found in 6 times  Actual = %d"""% count)
	time_stamps = []
	for line in ether_op:

...
Description: - Verify that the configured idle timeout for an session is applied after the session is successfully authenticated & established.
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: AAA-FUN-018
...
TOPOLOGY DIAGRAM:
  ...
  (Netscreen)                          |        |                            (Linux)
   ...
   |qa-ns | --------------------------|        |-------------------------| qa-svr3      |
    ------                             |        |                          --------------
                                        --------
How to run: "python2.5 AAA_FUN_018.py"
...
class test_AAA_FUN_001(test_case):
    myLog = getLogger()

### python libraries
import time

class test_AAA_FUN_018(test_case):
	...
	# Verifying the session authentication .
	...
	#time.sleep(idle_time_out+20)
	self.myLog.output('going to sleep 60 sec for  idle timeout+ 15 sec extra')
	time.sleep(idle_time_out+30)# test pass when idle timeout +20sec
	#time.sleep(idle_time_out+15)# test pass when idle timeout +20sec

	#Check out the SA status in SSX.
        self.ssx.cmd("context %s" %script_var['context'])
        sa_output= sa_check(self.ssx,script_var['ns_phy_ip'])
        self.failUnless(not sa_output,"session not terminated even after idle timeout")

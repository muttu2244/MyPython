...
Description: - Verify that the SSX DOES NOT generate any account-request
	 (stop) when the Phase1 Authentication fails after querying the local database  .
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-025
...
How to run: "python2.5 RADIUS_FUN_025.py"
...
class test_RADIUS_FUN_025(test_case):
    ...
    def test_RADIUS_FUN_025(self):

        """
        Test case Id: -  RADIUS-FUN-025
        """
	...
	self.ssx.config_from_string(script_var['rad_fun_025_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_025_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_025.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_025.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_025.pcap & '% topo.port_ssx_radius1[1])
	...
	time.sleep(5)
        ...
        self.failUnless("rad_fun_025.pcap" in ll_output,"Testcase FAILED because pcap file has not created")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_025.pcap")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_025.pcap  -R "radius.Acct_Status_Type  == 2"',timeout = 30)
        self.failUnless(not "RADIUS" in output, " Radius Accounting STOP sent even when Ph1 auth fails ")

...
Description: - Verify that the SSX sends all the essential & valid attributies
         in the	radius accounting -request(stop)message to the Radius server.
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-020
...
How to run: "python2.5 RADIUS_FUN_020.py"
...
class test_RADIUS_FUN_020(test_case):
    ...
    def test_RADIUS_FUN_020(self):

        """
        Test case Id: -  RADIUS-FUN-020
        """
	...
	self.ssx.config_from_string(script_var['rad_fun_020_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_020_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_020.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_020.pcap -f")
...
#        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -h')
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_020.pcap & '% topo.port_ssx_radius1[1])
	...
	session_id = re.search("session_handle:(\s+)(\w+)(\s+)",session_op,re.I)
        ...
        #Clear the sessions
        self.ssx.cmd("context %s" %script_var['context'])
        ssx_output=self.ssx.cmd("clear session all")
	self.myLog.output(ssx_output)
	...
	time.sleep(5)
        ...
        self.failUnless("rad_fun_020.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

	# Read the contents of the pcap file 
        self.myLog.output (" Step 4 - read the content of the file rad_fun_020.pcap")
        output=self.ether_radius1.cmd("""sudo /usr/sbin/tethereal -r rad_fun_020.pcap -R 'radius.User_Name == "user1@india-test"'""",timeout = 30)
        self.failUnless( "RADIUS" in output, " Expected - packet with radius.User_Name == user1@india-test")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_020.pcap -R "radius.Acct_Status_Type  == 2"',timeout = 30)
        self.failUnless( "RADIUS" in output, " Expected - packet with radius.Acct_Status_Type == 2")
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_020.pcap  -R "radius.Framed-IP-Address == 11.11.11.10"',timeout = 30)
        self.failUnless("RADIUS" in output, " Expected - packet with radius.Framed_IP_Address == 11.11.11.10")       
	output=self.ether_radius1.cmd("""sudo /usr/sbin/tethereal -r rad_fun_020.pcap -R 'radius.Acct_Session_Id  == "%s"'""" %session_id.group(2),timeout = 30)

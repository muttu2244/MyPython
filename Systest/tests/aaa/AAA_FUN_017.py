...
Description: -  Verify  session authentication with local database using in-valid X-auth credentials
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: AAA-FUN-017
...
How to run: "python2.5 AAA_FUN_017.py"
...
class test_AAA_FUN_017(test_case):
	...
	# With in-valid X-auth credentials
        op_debug =  aaa_verify_authentication(self.ssx,"invalidxauth@%s"%script_var['context'],"local")

...
TOPOLOGY DIAGRAM:
  ...
  (Netscreen)                          |        |                            (Linux)
   ...
   |qa-ns | --------------------------|        |-------------------------| qa-svr3      |
    ------                             |        |                          --------------
                                        --------
How to run: "python2.5 RADIUS_FUN_012_1.py"
...
class test_RADIUS_FUN_012_1(test_case):
    myLog = getLogger()
    ...
    def test_RADIUS_FUN_012_1(self):
        """
        Test case Id: -  RADIUS_FUN_012_1
	"""
	...
	#Configure two radius servers primary & secondary in the session authentication profile.
        ...
        self.ssx.config_from_string(script_var['rad_fun_012_1_ssx'])
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_012_1_ns5gt'])
	...
	# Stop the radius process
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_012_1.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_012_1.pcap -f")
        self.ether_radius2.cmd("sudo rm rad_fun_012_1.pcap -f")
        self.myLog.output("Step 2 -Start tethereal to capture the packets and store the result in a pcap file")
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_012_1.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        self.ether_radius2.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_012_1.pcap -R "radius" & '% topo.port_ssx_radius2[1])
	...
	time.sleep(50)
        ...
        #self.failUnless( "rad_fun_012_1.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")
        ll_output = self.ether_radius2.cmd("ls -lrt *.pcap")
        #self.failUnless( "rad_fun_012_1.pcap" in ll_output,"Testcase FAILED because pcap file has not created ")

        self.myLog.output  (" Step 4 - read the content of the file rad_fun_012_1.pcap ")
        output=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_012_1.pcap -R "radius.code == 4 " ',timeout = 30)
        self.failUnless("RADIUS" in output, """ Expected - packet with radius.code == 4,  Actual = %s"""% output)

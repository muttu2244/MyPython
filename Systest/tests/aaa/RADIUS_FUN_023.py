...
Description: - Verify that the SSX can send session Accouting-request &
             session authentication requests to different RADIUS servers .
TEST PLAN: AAA/RADIUS Test Plan
TEST CASES: RADIUS-FUN-023
...
How to run: "python2.5 RADIUS_FUN_023.py"
...
class test_RADIUS_FUN_023(test_case):
    ...
    def tearDown(self):
        ...
        # Close the telnet session of SSX and nsfgt ikev1 client 
        self.ssx.close()
        self.ns5gt.close()
        self.linux.close()
    ...
    def test_RADIUS_FUN_023(self):

        """
        Test case Id: -  RADIUS-FUN-023
        """

	self.myLog.output("\n**********start the test**************\n")
	# Configure SSX to forward session authentication credentials to 
	# radius(1) server and acctounting info to radius(2) server  .
	self.ssx.config_from_string(script_var['common_ssx'])
	self.ssx.config_from_string(script_var['rad_fun_023_ssx'])
	...
	# Ensure that the client is also configured with matching policies.
        ...
        self.ns5gt.config_from_string(script_var['rad_fun_023_ns5gt'])
	...
	# Flush the debug logs in SSX, if any
        ...
        self.myLog.output(" Step 1 - removing the file rad_fun_023.pcap")
        self.ether_radius1.cmd("sudo rm rad_fun_023.pcap -f")
        self.ether_radius2.cmd("sudo rm rad_fun_023.pcap -f")
        ...
        self.ether_radius1.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_023.pcap -R "radius" & '% topo.port_ssx_radius1[1])
        self.ether_radius2.cmd('sudo /usr/sbin/tethereal -i %s -q  -w rad_fun_023.pcap -R "radius" & '% topo.port_ssx_radius2[1])
	...
	time.sleep(5)
        ...
        self.failUnless("rad_fun_023.pcap" in ll_output,"Testcase FAILED because pcap file has not created")

        ll_output = self.ether_radius2.cmd("ls -lrt *.pcap")
        self.failUnless("rad_fun_023.pcap" in ll_output,"Testcase FAILED because pcap file has not created")

        self.myLog.output (" Step 4 - read the content of the file rad_fun_023.pcap")
	
	#Authentication Requset & Response
        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_023.pcap  -R "radius.code  == 1"',timeout = 30)
        self.failUnless("RADIUS" in output, " Expected - packet with radius.code == 1 ")

        output=self.ether_radius1.cmd('sudo /usr/sbin/tethereal -r rad_fun_023.pcap  -R "radius.code  == 2"',timeout = 30)
        self.failUnless("RADIUS" in output, " Expected - packet with radius.code == 2 ")

	#Accounting Requset & Response
        output=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_023.pcap -R "radius.Acct_Status_Type  == 1"',timeout = 30)
        self.failUnless("RADIUS" in  output, " Expected - packet with radius.Acct.Status_Type == 1 ")

        output=self.ether_radius2.cmd('sudo /usr/sbin/tethereal -r rad_fun_023.pcap -R "radius.Acct_Status_Type  == 2"',timeout = 30)

...
DESCRIPTION:Confirm SSX definition and loaded definition confirmed by "show config" are identical. [Bootup using configuration file in external CF]

HOW TO RUN:python2.5 NTT_1_10_1.py
...
class test_NTT_1_10_1(test_case):
    myLog = getLogger()
    ...
    def test_NTT_1_10_1(self):
        ...
        self.myLog.info("Go to hidden enable mode")
        ...
        self.ssx.cmd("del /cfext/%s nocon"%fileName)
        self.ssx.ftppasswd("copy sftp://regress@%s:%s/%s /cfext/%s"%(scriptServer,getPath,fileName,fileName))
        self.ssx.cmd("load conf /cfext/%s"%fileName,timeout=10000)
        self.ssx.cmd("boot config file /cfext/%s"%fileName,timeout=10000)
	...
	if int(tuncntOp.split()[-1]) < noOfTunnels:
        ...
        self.myLog.info("\n\n Reload the SSX with boot config file set to /cfext/%s \n\n"%fileName)
...
if __name__ == '__main__':
        ...
        log = buildLogger("NTT_1_10_1.log", debug=True,console=True)
        suite = test_suite()
        suite.addTest(test_NTT_1_10_1)

...
DESCRIPTION: Send ICMP Info Request messages to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_025
TOPOLOGY DIAGRAM:
        --------------------------------------------------------------------------------
...
HOW TO RUN:python2.5 ip_atk_025.py
...
class test_ip_atk_025(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_024'])
	...
	self.ssx.cmd("context %s"%script_var['context_name'])
	out = self.ssx.ping(script_var['linux1_ip_addr'])
        self.failUnless(out == 1,"linux is not reachable")
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send ICMP Info Request messages to SSX
	out = self.linux1.cmd("sudo /usr/local/bin/sing  -info %s &"%script_var['ssx_ip_addr'])

        time.sleep(40)

        #killing hping
        self.linux1.cmd("sudo pkill -9 sing")

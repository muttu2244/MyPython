...
DESCRIPTION: Send invalid offset to SSX.
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_044
...
HOW TO RUN:python2.5 ip_atk_044.py
...
class test_ip_atk_044(test_case):
    myLog = getLogger()	
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send TCP invalid offset set  to SSX.
	out = self.linux1.cmd("sudo /usr/sbin/hping -O 100 -S  %s -p 23 -i u1 & "%script_var['ssx_ip_addr'])
        time.sleep(40)

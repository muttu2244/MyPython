...
DESCRIPTION:Send ICMP flood to SSX from a LINUX machine and try to ping to LINUX machine connected to different port on same GLC 
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_083
...
HOW TO RUN:python2.5 ip_atk_083.py
...
class test_ip_atk_083(test_case):
    myLog = getLogger()	
   ...
   #     vg_output1 = vgroup_new(topo3[:])
	...
	self.ssx.cmd("clear log debug")
 	...
 	#Send ICMP flood to SSX from a LINUX machine and try to ping to LINUX machine connected to different port on same GLC 
	self.linux1.cmd("sudo /usr/sbin/hping -1  -p 22  %s -I %s --flood "%(script_var['ssx_ip_addr'],p1_ssx_linux1[1]))
        time.sleep(40)

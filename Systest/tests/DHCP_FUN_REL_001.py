...
DESCRIPTION:To verify SSX (relay agent) allows client to establish session with the DHCP server and maintains a table having IP address assigned associated with the MAC address.
TEST PLAN:DHCP Sanity Test plans
TEST CASES: DHCP-FUN-REL-001
...
TOPOLOGY DIAGRAM:
	...
	-------------------------------------------------------------------------
       ...
       |         Trans  IP = 2.1.1.1/24	---------- > TransIP = 2.1.1.2/24	|
			eth1	                         port-4/0 
         -------------------------------------------------------------------------
HOW TO RUN:python2.5 DHCP-FUN-REL-001.py
...
class test_DHCP_FUN_001(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_001'])

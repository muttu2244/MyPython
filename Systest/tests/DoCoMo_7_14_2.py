...
class testIt(Thread):
   ...
   def run(self):
	...
	self.threadLog.info("Loading the bulk configuration from the cosole")
	self.ssxHandle.cmd("load configuration /hd/DoCoMo_7_14_2.cfg", timeout=12000)
	self.status = 1
...
class test_DoCoMo_7_14_2(test_case):
	...
	ftpOp = ssxfdList[0].ftppasswd ("copy sftp://regress@%s:%s/DoCoMo_7_14_2.cfg /hd/DoCoMo_7_14_2.cfg noconfirm"%(scriptServer,getPath),Pword="gleep7")
	if ftpOp:
		self.fail("Copy of file 'DoCoMo_7_14_2.cfg' is failed")
	...
	loadBulk = testIt(ssxfdList[0]) # Loading the bulk config from Console.
	loadBulk.start()
	...
	setMe = 0
	while(cnt < 15):
		self.myLog.output("CPU Utilization: %s"%ssxfdList[1].cmd("show process cpu non-zero"))
		cpuUtil = ssxfdList[1].cmd('show process cpu non-zero | grep "CPU0 Utilization"')
		...
		if ((float(oneMin.group(1)) >= float(80.00)) or (float(fiveSec.group(1)) >= float(80.00))):
			...
			self.myLog.output("CPU Usage is more than 80 Precentage")
	                self.myLog.output("Current CPU Utilization: %s"%ssxfdList[1].cmd("show process cpu non-zero"))
	...
	if ((float(oneMin.group(1)) >= float(80.00)) or (float(fiveSec.group(1)) >= float(80.00))):
		...
		cpuUtil = ssxfdList[1].cmd('show process cpu non-zero | grep "CPU0 Utilization"')
                fiveSec = re.search("CPU0\s+Utilization\s+for\s+5\s+seconds:\s+(\d+\.\d+)", cpuUtil, re.I)
                oneMin = re.search("\s+1\s+Minute:\s+(\d+\.\d+)", cpuUtil, re.I)
		self.myLog.output("CurrentCPU Utilization: %s"%ssxfdList[1].cmd("show process cpu non-zero"))
		if not setMe:

...
DESCRIPTION:ph1 rekey timeout on existing session (on slot that's not failover)
TEST PLAN:
TEST CASE: DoCoMo IPsec

HOW TO RUN: python2.5 GLCR_SES_IKEv2_FUN_036.py
...
class test_GLCR_SES_IKEv2_FUN_036(test_case):
    myLog = getLogger()
    ...
    def test_GLCR_SES_IKEv2_FUN_036(self):
	...
	if int(op['standby']) != 4:
        ...
        self.ssx.config_from_string(ipsec_var['GLCR_SES_IKEv2_FUN_036'])
        self.sst.config_from_string(ipsec_var['SST_GLCR_SES_IKEv2_FUN_036'])
	...
	self.myLog.info("Phase1 life-time as per the config:\n")
	self.myLog.info("Phase1 soft life-time             : 60 Sec")
	self.myLog.info("Phase1  hard life-time		   : 360 Sec")

	# Verifying the Rekey...
	self.myLog.info("Verifying the Phase1 Rekey timeout")
	rekey1 = self.ssx.cmd('show ike-session detail handle %s | grep -i "IKE-SA Negotiation Count"'%ses_yn.splitlines()[-1].split()[1])
	rekey1 = re.search("IKE-SA Negotiation Count\s+:\s+(\d+)",rekey1,re.I)
	time.sleep(65)
	rekey2 = self.ssx.cmd('show ike-session detail handle %s | grep -i "IKE-SA Negotiation Count"'%ses_yn.splitlines()[-1].split()[1])
	rekey2 = re.search("IKE-SA Negotiation Count\s+:\s+(\d+)",rekey2,re.I)
	if rekey2 != None:
		rekey_val1 = int(rekey1.group(1))
		rekey_val2 = int(rekey2.group(1))
		print rekey_val1, rekey_val2
                if (rekey_val2 <= rekey_val1):
			self.fail( "Phase1 Rekey is not happened from new Active Card - 4th GLC")
                else:
                        self.myLog.output("\n Rekey happened successfully\n\n")

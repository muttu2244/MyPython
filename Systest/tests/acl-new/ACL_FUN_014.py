...
DESCRIPTION:To Verify permit (in) and Deny (in) counters when inbound filter is applied to a port.
TEST PLAN: ACL Test plans
TEST CASES:ACL_FUN_014

HOW TO RUN:python2.5 ACL_FUN_014.py
...
class test_ACL_FUN_014(test_case):
    myLog = getLogger()
    ...
    def test_ACL_FUN_014(self):


	# Push SSX config
	self.ssx.config_from_string(script_var['ACL_FUN_014'])
	...
	#self.linux.write_to_file(script_var['ACL_FUN_XPM_common'],"autoexec.cfg","/xpm/")
        ...
        #applying nemesis tool for generating tcp packets
        self.linux.cmd("sudo /usr/local/bin/nemesis tcp -S %s -D %s -d %s "%( script_var['xpress_phy_iface1_ip'], script_var['ssx_phy_iface1_ip'],p1_ssx_xpressvpn[1]))
        time.sleep(20)

	result=self.ssx.cmd("show ip counters tcp | grep Rx");
	need="Rx:    Total               1              Tx: Total               0"
	if result==need :
		output='1'
	else :
		output='0'

	self.failIfEqual(output,0,"TCP packet pass through failed")
        ...
        self.failIfEqual(output,0,"ICMP Deny Failed")

...
DESCRIPTION:To Verify that SSX allows traffic to only itself when ipv6 ACL is applied to a port 
TEST PLAN: Sanity Test plans
TEST CASES: ACL_FUN_048
...
HOW TO RUN : python ACL_FUN_048.py
...
class test_ACL_FUN_048(test_case):
    myLog = getLogger()
	...
	# Push SSX config
	self.ssx.config_from_string(script_var['ACL_FUN_048'])
	...
	#changing context and clearing ip counters
        ...
        ping_op=self.ssx.ping(dest=script_var['xpress_phy_iface1_ipv6'])
        time.sleep(5)
        self.failIfEqual(ping_op,0,"Ping Through Interface Failed When ACL was Applied")
	...
	self.failUnlessEqual(output,1,"ICMP Counters Increment Failed")
...
if __name__ == '__main__':
	...
	suite = test_suite()
	suite.addTest(test_ACL_FUN_048)

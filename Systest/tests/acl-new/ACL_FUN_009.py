...
DESCRIPTION:To Test deny rules with UDP when ACL is applied to port
TEST PLAN: Sanity Test plans
TEST CASES: ACL_FUN_009
...
HOW TO RUN : python ACL_FUN_009.py
...
class test_ACL_FUN_009(test_case):
    myLog = getLogger()
    ...
    def tearDown(self):
        ...
        self.ssx.close()
	self.linux.close()
    def test_ACL_FUN_009(self):
	  #vgroup b/w ssx and host
        #out = ssx["ip_addr"].split("-")[0]
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['ACL_FUN_009_2'])

	#configuring tunnel on linux machine
	self.linux.configure_ip_interface(topo.p1_ssx_linux1[1], script_var['xpress_phy_iface1_ip_mask'])

	# Push xpress vpn config
        self.linux.write_to_file(script_var['ACL_XPM_009_2'],"autoexec.cfg","/xpm/")
	...
	time.sleep(30)
	self.failIfEqual(op_client_cmd,1,"session was established")
        ...
        self.failIfEqual("ESTABLISHED" in ssx_show_op["session_state"],"failed to find SA")
	...
	time.sleep(5)
	output=ip_verify_ip_counters_udp(self.ssx,checksum='0',
                                  rx_total='0',tx_total='0',
                                  short_pkt='0',short_hdr='0',
                                 full='0',no_port_bcast='0' )

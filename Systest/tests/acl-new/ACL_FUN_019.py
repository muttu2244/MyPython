...
DESCRIPTION: To Verify SSX permits/drops packets based on tos option in the ACL and ACL is applied for filtering inbound packets
TEST PLAN :ACL Test plans
TEST CASES:ACL_FUN_019

HOW TO RUN : python2.5 ACL_FUN_019.ACL
...
class test_ACL_FUN_019(test_case):
    myLog = getLogger()
    ...
    def test_ACL_FUN_019(self):


	# Push SSX config
	self.ssx.config_from_string(script_var['ACL_FUN_019'])
	...
	self.linux.cmd("ping %s -c 5 -Q 0x16 "%(script_var['ssx_phy_iface1_ip']),timeout=40)
        time.sleep(5)

...
DESCRIPTION:To Verify that SSX retains the IPv6 ACL states after imc-switchover
TEST PLAN: Sanity Test plans
TEST CASES: ACL_FUN_052
...
HOW TO RUN : python ACL_FUN_052.py
...
class test_ACL_FUN_052(test_case):
    myLog = getLogger()
	...
	# Push SSX config
	self.ssx.config_from_string(script_var['ACL_FUN_052'])
	...
	self.ssx.cmd("context %s" %(script_var['context_name']))
        ...
        #ping operation   
	self.linux.ping6(script_var['ssx_phy_iface1_ipv6'])
	self.ssx.cmd("context local")
	self.ssx.imc_switchover_mgmt("system imc-switchover")

	self.ssx.cmd("context %s" %(script_var['context_name']))
	self.linux.ping6(script_var['ssx_phy_iface1_ipv6'])
	result=self.ssx.cmd("show ipv6 counters icmp | grep Rx")
	expected=" Rx:    Total               0              Tx: Total               0  "
	output='0'
	if result == expected :
		output='1'
	
	self.failIfEqual(output,0,"IPv6 ACL States were not maintained with Imc Switchover occers")
...
if __name__ == '__main__':
	...
	suite = test_suite()
	suite.addTest(test_ACL_FUN_052)

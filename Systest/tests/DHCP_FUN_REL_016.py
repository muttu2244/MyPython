...
DESCRIPTION:To verify SSX (relay agent)does not contain  relay information option in the reply sent to the subscriber. 
TEST PLAN: DHCP Test plans
TEST CASE ID: DHCP-FUN-REL-016
...
TOPOLOGY DIAGRAM:
	...
	--------------------------------------------------------------------------------
       ...
       |                     ETH1                                   Port 3/0		|
         --------------------------------------------------------------------------------
HOW TO RUN:python2.5 DHCP-FUN-REL-016.py
...
class test_DHCP_FUN_016(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_016'])
	 ...
	 #configure ip interface on linux machine
        ...
        outstr=self.linux2.cmd("sudo /usr/sbin/tethereal -i '%s' -q -w dhcp.pcap  -R bootp &" %p1_ssx_linux2[1])
        ...
        outstr=self.linux2.cmd("sudo /usr/sbin/tethereal -r dhcp.pcap -V")

        self.failUnless("Option: (82)"not in outstr)

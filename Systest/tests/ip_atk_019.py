...
DESCRIPTION: Sends oversized fragmented IGMP packets to SSX.

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_019
...
HOW TO RUN:python2.5 ip_atk_019.py
...
class test_ip_atk_019(test_case):
    myLog = getLogger()	
	...
	#Send oversized fragmented IGMP packets to SSX

	out = self.linux1.cmd("sudo ./fawx %s %s 1000000 & "%(script_var['linux1_ip_addr'],script_var['ssx_ip_addr']))
	time.sleep(50)

	#killing hping process
        self.linux1.cmd("sudo pkill -9 fawx") 

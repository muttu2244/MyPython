...
DESCRIPTION: Configure the time-zone on SSX as IST and NTP server as PST and obtain time from NTP server. Verify if the time is correct as per the IST
TEST PLAN: FUN Test plans
TEST CASES: NTP_FUN_018
...
HOW TO RUN:python2.5 NTP_FUN_018.py
...
class test_NTP_FUN_018(test_case):
    myLog = getLogger()
	...
	self.ssx.config_from_string(script_var['NTP_FUN_018'])
	...
	#Changing Timezone on NTP Client to IST
        self.ssx.configcmd("clock  timezone IST 5 30")
        Clock_out = self.ssx.cmd("show clock")
        #Changing timezone on server to IST.
        self.linux.cmd("sudo ln -sf ../usr/share/zoneinfo/PST8PDT /etc/localtime")
        Date_out = self.linux.cmd("date")
	...
	#Displaying Date and Clock output after synchronization of NTP server with client.
        self.myLog.output("======Date and Clock output after synchronization==============")
        self.linux.cmd("sudo ln -sf ../usr/share/zoneinfo/Asia/Calcutta /etc/localtime")
	Date_out = self.linux.cmd("date")
        Clock_out = self.ssx.cmd("show clock")
...
if __name__ == '__main__':
	...
	suite = test_suite()
	suite.addTest(test_NTP_FUN_018)

...
DESCRIPTION:Send IP packets with protocol field filled with unknown number (undefined protocol number>137) to SSX

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_003
...
HOW TO RUN:python2.5 ip_atk_003.py
...
class test_ip_atk_003(test_case):
    myLog = getLogger()	
	...
	#Sending IP packets with protocol field filled with unknown number (undefined protocol number>137) to SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping -H 355 %s -p 23 &"%script_var['ssx_ip_addr'])
	time.sleep(30)

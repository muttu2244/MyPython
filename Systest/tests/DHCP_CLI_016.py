...
Description: Verify that SSX allows configuring same DHCP parameters in different context.
TEST PLAN:DHCP CLI Test plans
TEST CASES: DHCP_CLI_016
...
TOPOLOGY DIAGRAM:
       ...
       |         Trans  IP = 2.1.1.1/24 ---------- > TransIP = 2.1.1.2/24       |
                        eth1                             port-4/0
         -------------------------------------------------------------------------
HOW TO RUN:python2.5 DHCP_CLI_016.py
...
class test_DHCP_CLI_016(test_case):
    myLog = getLogger()
    ...
    def test_DHCP_CLI_016(self):
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['DHCP_CLI_016'])
        ...
        self.failIf("%s"%script_var['context_name1'] not in a.split())

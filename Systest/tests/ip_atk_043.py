...
DESCRIPTION: Send TCP packet with large sequence number to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_043
...
HOW TO RUN:python2.5 ip_atk_043.py
...
class test_ip_atk_043(test_case):
    myLog = getLogger()	
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send TCP packet with large sequence number to SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping -S  -M 5000  %s -p 23 -i u1 -I %s & "%(script_var['ssx_ip_addr'], p1_ssx_linux1[1]))

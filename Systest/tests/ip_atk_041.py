...
DESCRIPTION: Send TCP fragmentation packets to SSX.
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_041
...
HOW TO RUN:python2.5 ip_atk_041.py
...
class test_ip_atk_041(test_case):
    myLog = getLogger()	
	...
	self.ssx.cmd("clear ip counters ")
        ...
        #Send TCP fragmentation packets to SSX.
	out = self.linux1.cmd("sudo /usr/sbin/hping -V  -f %s -p 22 -I %s &"%(script_var['ssx_ip_addr'],p1_ssx_linux1[1]))
        time.sleep(40)

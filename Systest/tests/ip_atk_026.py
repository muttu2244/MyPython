...
DESCRIPTION:Sending invalid IP fragments at it. The jolt attack sends very large, fragmented ICMP packets to a SSX. 

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_026
...
HOW TO RUN:python2.5 ip_atk_026.py
...
class test_ip_atk_026(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_026'])
	...
	#Spoofed broadcast ping messages to flood to SSX
	out = self.linux1.cmd("sudo ./jolt %s %s 2000000000 & "%(script_var['ssx_ip_addr'],script_var['linux1_ip_addr']))
	time.sleep(100)

	#killing hping process
        self.linux1.cmd("sudo pkill -9 jolt") 

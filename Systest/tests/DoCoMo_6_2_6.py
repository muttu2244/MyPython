...
TOPOLOGY DIAGRAM:
  ...
  (Netscreen)                          |        |                            (Linux)
   ...
   |qa-ns1 | --------------------------|        |-------------------------| qa-svr3      |
    ------                             |        |                          --------------
                                        --------
How to run: "python2.5 DoCoMo_6_2_6.py"
...
class test_DoCoMo_6_2_6(test_case):
    myLog = getLogger()
    global pskFlag
    pskFlag = 1
    ...
    def test_DoCoMo_6_2_6(self):
        """
        Test case Id: -  DoCoMo_6_2_6
	"""
	...
	vgroup_new("%s:%s %s:%s"%(ssx_name,vportssx2,radius1['ip_addr'],vportl2))
        ...
        # Push SSX config
        self.ssx.config_from_string(docomo_var['DoCoMo_6_2_6'])
	...
	self.sniffer.cmd("sudo /usr/sbin/tethereal -i %s -R esp -q -w esp.pcap &"% port_ssx_linux1[1])
	...
	time.sleep(5)
	strcap = self.sniffer.cmd("sudo /usr/sbin/tethereal -r esp.pcap")
	self.failIf("ICMP" in strcap , "ICMP Traffic is encrypted via tunnel")

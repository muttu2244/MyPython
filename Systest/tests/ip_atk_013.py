...
DESCRIPTION:Send  fragments with overlapped offsets to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_013
...
HOW TO RUN:python2.5 ip_atk_013.py
...
class test_ip_atk_013(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_013'])
	...
	#Sending fragments with overlapped offsets to SSX.

	out = self.linux1.cmd("sudo ./teardrop %s %s -n 10000 & "%(script_var['linux1_ip_addr'],script_var['ssx_ip_addr']))
	time.sleep(100)
	#killing hping process
        self.linux1.cmd("sudo pkill -9 teardrop") 
	...
	self.myLog.output("IP counters exist on SSX after flooding traffic \n:%s \n"%self.ssx.cmd("show ip counters"))

        self.myLog.output("System counters exist on SSX after flooding traffic \n:%s\n"%self.ssx.cmd("show syscount"))

        self.myLog.output("Port counters exist on SSX after flooding traffic\n:%s\n"%self.ssx.cmd("show port %s counters drop"%script_var['ssx_port']))

...
DESCRIPTION: Send TCP packet with FIN bit set but no ACK bit to SSX

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_034
...
HOW TO RUN:python2.5 ip_atk_034.py
...
class test_ip_atk_034(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_034'])
	...
	self.ssx.cmd("clear ip counters ")
        ...
        # Sending TCP packet with FIN bit set but no ACK bit to SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping -F %s -p 23 --flood &"%script_var['ssx_ip_addr'])

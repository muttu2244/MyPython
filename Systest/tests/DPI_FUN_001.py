...
DESCRIPTION: Detecting & classifying protocol, HTTP 
TEST PLAN: DPI Test Plan
TEST CASES: DPI_PD_01
...
How to run: "python2.5 DPI_FUN_001.py"
...
from misc import *
#import configs file
pexpect.run('perl  -pi -e \'s/proto_from_script = "\w+"/proto_from_script = "http"/\' ikev2_config.py')
from ikev2_config import *
from topo import *

class test_DPI_FUN_001(test_case):
    myLog = getLogger()
	...
	self.radius.cmd("sudo apachectl -k start")
        httpOutput = self.xpress_vpn.cmd("sudo curl http://69.0.0.1/index.html -i")
        #httpOutput = self.xpress_vpn.cmd("sudo nc -z 69.0.0.1 23")
	time.sleep(5)
	self.failUnless(httpOutput, "http is failed b/w ike client and remote host")
	counters = get_ses_counters_qos(self.ssx)
	self.myLog.info("Rcv Pkts:%s  Xmit Pkts:%s" %(counters[0], counters[1]))
	self.failUnless(counters[0] and counters[1], "http traffic is not bi-directional")
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_DPI_FUN_001)

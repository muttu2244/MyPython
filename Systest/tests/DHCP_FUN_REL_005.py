...
DESCRIPTION:To verify SSX (relay agent) relays the DHCP reply message from a DHCP server to the DHCP subscriber. 
TEST PLAN: DHCP Sanity Test plans
TEST CASE ID: DHCP-FUN-REL-005
...
HOW TO RUN:python2.5 DHCP-FUN-REL-005.py
...
class test_DHCP_FUN_005(test_case):
    myLog = getLogger()
	...
	self.linux2.telnet()
        ...
        # Flush th Ip of client.
        self.linux2.cmd("sudo /sbin/ip addr flush dev eth1")
	...
	#vgroup b/w SSX and linux.
        ...
        var1=topo2[0]+" "+topo2[1]
        #self.linux5.cmd("vgroup %s" % var1)


	   # Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_005'])
	...
	outstr=self.linux1.cmd("sudo /usr/sbin/tethereal -r dhcp.pcap")
       ...
       #Verifying the required DHCP relay messages from tethereal capture file on dhcp server side.
        #self.failUnless("Offer" in outstr.split())
        self.failUnless("ACK"  in outstr.split())

        #Greping dhcp counters of server side
        Client_out = self.ssx.cmd("show dhcp counters | grep server")
        self.failUnless((int(Client_out.split()[3]) == 1 ) | (int(Client_out.split()[5]) ==1))

...
DESCRIPTION:Run nmap tool onto SSX to send 0 bytes UDP packets on all ports
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_069
...
HOW TO RUN:python2.5 ip_atk_069.py
...
class test_ip_atk_069(test_case):
    myLog = getLogger()	
	...
	#Run nmap tool onto SSX to send 0 byte UDP packets on all ports
	out = self.linux1.cmd("sudo /usr/bin/nmap -sU %s  & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

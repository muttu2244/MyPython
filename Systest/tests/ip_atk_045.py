...
DESCRIPTION:Run nmap tool onto SSX to send scan attack
TEST PLAN:IP-ATTACK test plan
TEST CASEID:ip_atk_045
...
HOW TO RUN:python2.5 ip_atk_045.py
...
class test_ip_atk_045(test_case):
    myLog = getLogger()	
	...
	#Run nmap tool onto SSX to send scan attack
	out = self.linux1.cmd("sudo /usr/bin/nmap -p1-1024 %s  & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

...
class cli_cntx_ip(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_ip_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local -f %(drvr-data_ip)s -c %(ssx_system)s " %script_var)    	
...
#	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_ip_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_ip_pos_%(ssx_ver)s.log" %script_var )	
...
#	self.failUnless(not diff_out ,"Test Fail due to mismatch in logs")	
    ...
    def test_ip_002_rae(self):
        #closing of SSX connection
        self.ssx.close()
 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_ip_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local -f %(drvr-data_ip)s -c %(ssx_system)s " %script_var)
    ...
    def test_ip_003_ere(self):

        #closing of SSX connection
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_ip_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local -f %(drvr-data_ip)s -c %(ssx_system)s " %script_var)
    ...
    def test_ip_004_rm2(self):

        #closing SSX connection
        self.ssx.close()
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_ip_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local -f %(drvr-data_ip)s -c %(ssx_system)s " %script_var)
    ...
    def test_ip_005_all(self):

        # closing of SSX connection
        self.ssx.close() 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_ip_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local -f %(drvr-data_ip)s -c %(ssx_system)s " %script_var)

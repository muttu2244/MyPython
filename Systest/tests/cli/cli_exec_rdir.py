...
class cli_exec_rdir(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/rdir_exec_%(ssx_ver)s.log  %(ssx_type)s -C rmdir -f %(drvr-data)s %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_rdir_exec_%(ssx_ver)s.log %(cli_fldr)s/rdir_exec_%(ssx_ver)s.log" %script_var )	

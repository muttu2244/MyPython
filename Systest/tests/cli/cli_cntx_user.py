...
"""
DESCRIPTION             : This script will invoke "context local -> user name local" on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : user_001_pos, 
                          user_002_rae,
                          user_003_ere,
                          user_004_rm2,
                          user_005_all 
...
class cli_cntx_user(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_user_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local::user_name_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
	...
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_user_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_user_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_user_002_rae(self):

        #closing SSX connection
        self.ssx.close()
  
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_user_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local::user_name_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_user_003_ere(self):

        #closing SSX connection 
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_user_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local::user_name_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_user_004_rm2(self):

        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_user_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local::user_name_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_user_005_all(self):

        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_user_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local::user_name_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)

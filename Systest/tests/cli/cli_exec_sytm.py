...
class cli_exec_sytm(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/sytm_exec_%(ssx_ver)s.log  %(ssx_type)s -C system -f %(drvr-data)s %(ssx_mc)s " %script_var)    	

        # Opening SSX Connection Again
        self.ssx = SSX(topo.ssx_con['ip_addr'])
        self.ssx.telnet()
 
	#self.ssx.cmd("clear confi")
        # Compairing  the log file against the golden file
	self.myLog.output("Comparing the log files")
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_sytm_exec_%(ssx_ver)s.log %(cli_fldr)s/sytm_exec_%(ssx_ver)s.log" %script_var )	

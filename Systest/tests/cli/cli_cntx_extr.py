...
"""
DESCRIPTION             : This script will invoke "context local -> domain
						                    interface 
								     
						  on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : extr_001_pos, 
                          extr_002_rae,
                          extr_003_ere,
                          extr_004_rm2,
                          extr_005_all 
...
class cli_cntx_extr(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_intf_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local -f %(drvr-data_intf)s -c %(ssx_system)s " %script_var)    	
	...
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_intf_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_intf_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_extr_002_rae(self):
        # closing of SSX connection
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_intf_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local -f %(drvr-data_intf)s -c %(ssx_system)s " %script_var)
    ...
    def test_extr_003_ere(self):
        #closing of SSX connection
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_intf_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local -f %(drvr-data_intf)s -c %(ssx_system)s " %script_var)
        # Opening SSX Connection Again
        self.ssx = SSX(topo.ssx_con['ip_addr'])
        self.ssx.telnet()
    ...
    def test_extr_004_rm2(self):
        #closing SSX connection
        self.ssx.close() 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_intf_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local -f %(drvr-data_intf)s -c %(ssx_system)s " %script_var)
    ...
    def test_extr_005_all(self):
        # closing of SSX connection
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_intf_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local -f %(drvr-data_intf)s -c %(ssx_system)s " %script_var)

...
"""
DESCRIPTION             : This script will invoke "context local -> random-early-detect name local " on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : rand_001_pos, 
                          rand_002_rae,
                          rand_003_ere,
                          rand_004_rm2,
                          rand_005_all 
...
class cli_cntx_rand(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rand_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local::random-early-detect_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
	...
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_rand_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_rand_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_rand_002_rae(self):
        #closing SSX connection
        self.ssx.close()
 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rand_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local::random-early-detect_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rand_003_ere(self):
        #closing SSX connection
        self.ssx.close()
 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rand_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local::random-early-detect_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rand_004_rm2(self):
        #closing SSX connection
        self.ssx.close()
  
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rand_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local::random-early-detect_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rand_005_all(self):
     
        #closing SSX connection
        self.ssx.close()    
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rand_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local::random-early-detect_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
        # Opening SSX Connection Again
        self.ssx = SSX(topo.ssx_con['ip_addr'])
        self.ssx.telnet()

...
class cli_pos_boot(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/boot_pos_%(ssx_ver)s.log -u %(ssx_user)s -p %(ssx_pw)s -C boot -f %(drvr-data)s %(ssx)s " %script_var)    	

        # Compairing  the log file against the golden file
	self.myLog.output("Diffing the log files")
	diff_out = os.system("diff %(gldn_fldr)s/gld_boot_pos_%(ssx_ver)s.log %(cli_fldr)s/boot_pos_%(ssx_ver)s.log" %script_var )	

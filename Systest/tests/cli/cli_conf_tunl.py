...
"""
DESCRIPTION             : This script will invoke "tunl global profile" on SSX CLI (Configuration mode)
REFERENCE               : SSX-cli Test and Automation Strategy
TEST CASES              : tunl_001_pos,
                          tunl_002_rae,
                          tunl_003_ere,
                          tunl_004_rm2,
                          tunl_005_all
...
class cli_conf_tunl(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/conf_tunl_pos_%(ssx_ver)s.log %(ssx_type)s -i tunnel_local_type_ipsec_protocol_ip44_context_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_conf_tunl_pos_%(ssx_ver)s.log  %(cli_fldr)s/conf_tunl_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_tunl_002_rae(self):

        #closing SSX connection
        self.ssx.close()
  
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/conf_tunl_rae_%(ssx_ver)s.log %(ssx_type)s -n -i tunnel_local_type_ipsec_protocol_ip44_context_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_tunl_003_ere(self):

        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/conf_tunl_ere_%(ssx_ver)s.log %(ssx_type)s -a -i tunnel_local_type_ipsec_protocol_ip44_context_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_tunl_004_rm2(self):

         #closing SSX connection
        self.ssx.close()
  
         # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/conf_tunl_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i tunnel_local_type_ipsec_protocol_ip44_context_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_tunl_005_all(self):

        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/conf_tunl_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i tunnel_local_type_ipsec_protocol_ip44_context_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)

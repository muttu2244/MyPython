...
"""
DESCRIPTION             : This script will invoke "context local -> radius " on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : rdis_001_pos, 
                          rdis_002_rae,
                          rdis_003_ere,
                          rdis_004_rm2,
                          rdis_005_all 
...
class cli_cntx_rdis(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rdis_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local -f %(drvr-data_rdis)s -c %(ssx_system)s " %script_var)    	
	...
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_rdis_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_rdis_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_rdis_002_rae(self):
        # closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rdis_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local -f %(drvr-data_rdis)s -c %(ssx_system)s " %script_var)
        # Opening SSX Connection Again
        self.ssx = SSX(topo.ssx_con['ip_addr'])
        self.ssx.telnet()
    ...
    def test_rdis_003_ere(self):
        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rdis_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local -f %(drvr-data_rdis)s -c %(ssx_system)s " %script_var)
    ...
    def test_rdis_004_rm2(self):
        # closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rdis_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local -f %(drvr-data_rdis)s -c %(ssx_system)s " %script_var)
    ...
    def test_rdis_005_all(self):
        #closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rdis_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local -f %(drvr-data_rdis)s -c %(ssx_system)s " %script_var)

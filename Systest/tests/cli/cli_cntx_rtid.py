...
"""
DESCRIPTION             : This script will invoke "context local -> router-id 1.1.1.1 " on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : rtid_001_pos, 
                          rtid_002_rae,
                          rtid_003_ere,
                          rtid_004_rm2,
                          rtid_005_all 
...
class cli_cntx_rtid(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rtid_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local::router-id_1.1.1.1 -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
	...
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_rtid_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_rtid_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_rtid_002_rae(self):
        #closing SSX connection
        self.ssx.close()
     
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rtid_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local::router-id_1.1.1.1 -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rtid_003_ere(self):
        # closing SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rtid_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local::router-id_1.1.1.1 -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rtid_004_rm2(self):

        #closing SSX connection
        self.ssx.close() 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rtid_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local::router-id_1.1.1.1 -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_rtid_005_all(self):
        #closing SSX connection
        self.ssx.close()
 
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_rtid_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local::router-id_1.1.1.1 -f %(drvr-data)s -c %(ssx_system)s " %script_var)

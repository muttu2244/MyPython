...
class cli_conf_rtr(test_case):
	...
	self.myLog.info("Executing the command \"cli-driver.pl  -C rtr -i context_local -z  -a -c %s\" ......" %topo.ssx_con['ip_addr'])
	cliOutput = pexpect.run("cli-driver.pl  -C rtr -i context_local -z -a -c %s" %topo.ssx_con['ip_addr'])

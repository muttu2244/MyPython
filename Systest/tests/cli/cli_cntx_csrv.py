...
"""
DESCRIPTION             : This script will invoke "context local -> class-of-service local " on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : csrv_001_pos, 
                          csrv_002_rae,
                          csrv_003_ere,
                          csrv_004_rm2,
                          csrv_005_all 
...
class cli_cntx_csrv(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_csrv_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local::class-of-service_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
...
#	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_csrv_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_csrv_pos_%(ssx_ver)s.log" %script_var )	
...
#	#self.failUnless(not diff_out ,"Test Fail due to mismatch in logs")	
    ...
    def test_csrv_002_rae(self):
        self.ssx.close()  
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_csrv_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local::class-of-service_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_csrv_003_ere(self):
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_csrv_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local::class-of-service_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_csrv_004_rm2(self):
        self.ssx.close()   
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_csrv_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local::class-of-service_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_csrv_005_all(self):
        self.ssx.close()
        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_csrv_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local::class-of-service_local -f %(drvr-data)s -c %(ssx_system)s " %script_var)

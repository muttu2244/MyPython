...
class cli_exec_drve(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/drve_exec_%(ssx_ver)s.log  %(ssx_type)s -C drive -f %(drvr-data)s %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_drve_exec_%(ssx_ver)s.log %(cli_fldr)s/drve_exec_%(ssx_ver)s.log" %script_var )	

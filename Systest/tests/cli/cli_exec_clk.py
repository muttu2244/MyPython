...
# All Rights Reserved.
#
# This code is execidential and proprietary to Stoke, Inc. and may only
# be used under a license from Stoke.
#
#######################################################################

"""
DESCRIPTION             : This script will invoke "clk global profile" on SSX CLI (Configuration mode)
REFERENCE               : SSX-cli Test and Automation Strategy
TEST CASES              : clk_001_pos,
                          clk_002_rae,
                          clk_003_ere,
                          clk_004_rm2,
                          clk_005_all
...
class cli_exec_clk(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/clk_pos_%(ssx_ver)s.log  %(ssx_type)s  -C clock -f %(drvr-data_clk)s %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_clk_pos_%(ssx_ver)s.log  %(cli_fldr)s/clk_pos_%(ssx_ver)s.log" %script_var )	

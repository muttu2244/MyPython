...
class cli_exec_no(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/no_exec_%(ssx_ver)s.log  %(ssx_type)s -2 -C no -f %(drvr-data)s %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_no_exec_%(ssx_ver)s.log %(cli_fldr)s/no_exec_%(ssx_ver)s.log" %script_var )	

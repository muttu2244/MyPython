...
"""
DESCRIPTION             : This script will invoke "context local -> cdr " on SSX CLI (Configuration mode)
REFERENCE               : SSX-CLI Test and Automation Strategy
TEST CASES              : cdr_001_pos, 
                          cdr_002_rae,
                          cdr_003_ere,
                          cdr_004_rm2,
                          cdr_005_all 
...
class cli_cntx_cdr(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_cdr_pos_%(ssx_ver)s.log %(ssx_type)s -i context_local::cdr -f %(drvr-data)s -c %(ssx_system)s " %script_var)    	
	
        # Comparing the log file against the golden file
	#self.myLog.output("Comparing the log files")
	#diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_cntx_cdr_pos_%(ssx_ver)s.log  %(cli_fldr)s/cntx_cdr_pos_%(ssx_ver)s.log" %script_var )	
    ...
    def test_cdr_002_rae(self):

        # closing of SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_cdr_rae_%(ssx_ver)s.log %(ssx_type)s -n -i context_local::cdr -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_cdr_003_ere(self):

        # closing of SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_cdr_ere_%(ssx_ver)s.log %(ssx_type)s -a -i context_local::cdr -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_cdr_004_rm2(self):

        # closing of SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_cdr_rm2_%(ssx_ver)s.log %(ssx_type)s -2 -a -i context_local::cdr -f %(drvr-data)s -c %(ssx_system)s " %script_var)
    ...
    def test_cdr_005_all(self):

        # closing of SSX connection
        self.ssx.close()

        # Running the cli-driver commnad
        os.system(" cli-driver.pl -l %(cli_fldr)s/cntx_cdr_all_%(ssx_ver)s.log %(ssx_type)s -2 -a -n -i context_local::cdr -f %(drvr-data)s -c %(ssx_system)s " %script_var)

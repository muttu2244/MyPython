...
class cli_conf_clk(test_case):
	...
	os.system(" cli-driver.pl -l %(cli_fldr)s/conf_clk_pos_%(ssx_ver)s.log %(ssx_type)s  -f %(drvr-data_clk)s -c %(ssx_system)s " %script_var)    	
	...
	diff_out = os.system("diff %(gldn_fldr)s/%(ssx_ver)s/gld_conf_clk_pos_%(ssx_ver)s.log  %(cli_fldr)s/conf_clk_pos_%(ssx_ver)s.log" %script_var )	

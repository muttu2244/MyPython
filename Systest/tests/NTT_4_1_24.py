...
DESCRIPTION: Confirm route is added when add node in same area
TEST PLAN:
TEST CASES: NTT_4_1_24

HOW TO RUN: python2.5 NTT_4_1_24.py
...
class test_NTT_4_1_24(test_case):
    myLog = getLogger()
	...
	ospf_op1 = self.ssx1.cmd("show ip ospf database")
        if "99.1.1.1" not in ospf_op1:
           self.fail("Confirmed route is not added when add node in same area")
...
if __name__ == '__main__':
        ...
        log = buildLogger("NTT_4_1_24.log", debug=True,console=True)
        suite = test_suite()
        suite.addTest(test_NTT_4_1_24)

...
DESCRIPTION:Send TCP packets with bad checksum value to SSX.

TEST PLAN:IP-ATTACK test plans
TEST CASE ID:ip_atk_036
...
HOW TO RUN:python2.5 ip_atk_036.py
...
class test_ip_atk_036(test_case):
    myLog = getLogger()	
	...
	out = self.linux1.cmd("sudo /usr/sbin/hping  --badcksum  -p 23  %s  &"%(script_var['ssx_ip_addr']))
	time.sleep(30)

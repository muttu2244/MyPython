...
DESCRIPTION:Send UDP traffic flood before IKEv1 session is established 
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_079
...
HOW TO RUN:python2.5 ip_atk_079.py
...
class test_ip_atk_079(test_case):
    myLog = getLogger()	
   ...
   #     self.ssx.clear_config()
	...
	self.myLog.output("\n**********start the test**************\n")
	 ...
	 #Push the SSX configuration     
        # commenting line belo as we dont need this conf again --Ashu
	...
	#Flooding traffic after establishing ikev1 session
        self.linux1.cmd("/usr/sbin/hping -2 -p 22 %s --flood &"%script_var['ssx_ip_addr'])
        time.sleep(40)

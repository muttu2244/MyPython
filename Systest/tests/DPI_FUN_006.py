...
DESCRIPTION: Detecting & classifying protocol, passive-mode FTP
TEST PLAN: DPI Test Plan
TEST CASES: DPI_FUN_006
...
How to run: "python2.5 DPI_FUN_006.py"
...
class test_DPI_FUN_006(test_case):
    myLog = getLogger()
	...
	ftpPasvOutput = self.xpress_vpn.cmd("curl --ftp-pasv --user regress:gleep7 ftp://69.0.0.1/dpi.html")
	time.sleep(10)
	self.failUnless(ftpPasvOutput, "ftp is failed b/w ike client and remote host")
	counters = get_ses_counters_qos(self.ssx)
	self.myLog.info("Rcv Pkts:%s  Xmit Pkts:%s" %(counters[0], counters[1]))
	self.failUnless(counters[0] and counters[1], "ftp traffic is not bi-directional")
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_DPI_FUN_006)

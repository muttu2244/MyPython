...
DESCRIPTION: Detecting & classifying protocol, SMTP
TEST PLAN: DPI Test Plan
TEST CASES: DPI_PD_03
...
How to run: "python2.5 DPI_FUN_003.py"
...
from misc import *
#import configs file
pexpect.run('perl  -pi -e \'s/proto_from_script = "\w+"/proto_from_script = "smtp"/\' ikev2_config.py')
from ikev2_config import *
from topo import *

class test_DPI_FUN_003(test_case):
    myLog = getLogger()
	...
	self.radius.cmd("sudo /etc/init.d/sendmail start")
        smtpOutput = self.xpress_vpn.cmd("sudo nc -z 69.0.0.1 25")
	time.sleep(10)
	self.failUnless(smtpOutput, "smtp is failed b/w ike client and remote host")
	counters = get_ses_counters_qos(self.ssx)
	self.myLog.info("Rcv Pkts:%s  Xmit Pkts:%s" %(counters[0], counters[1]))
	self.failUnless(counters[0] and counters[1], "smtp traffic is not bi-directional")
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_DPI_FUN_003)

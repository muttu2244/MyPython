...
DESCRIPTION: Confirm neighbor is established, and confirm it shows adjency status. (9/21/11 NOS Review - Broadcast Network).
TEST PLAN: 
TEST CASES: NTT_4_1_5

HOW TO RUN: python2.5 NTT_4_1_5.py
...
class test_NTT_4_1_5(test_case):
    myLog = getLogger()
    ...
    def test_NTT_4_1_5(self):

        self.myLog.output("==================Starting The Test====================")
	
        # Push SSX config
	self.ssx1.config_from_string(route_var['NTT_4_1_5'])
	...
	ospf_op1 = self.ssx1.cmd('show ip ospf neighbor | grep "%s"'%route_var['ospf_intf1_ip'])
        ...
        if "BROADCAST" not in ospf_op1 :
           self.fail("Network Type BROADCAST is not there is output , actual output returned by command is %s"%ospf_op1)
...
if __name__ == '__main__':
        ...
        log = buildLogger("NTT_4_1_5.log", debug=True,console=True)
        suite = test_suite()
        suite.addTest(test_NTT_4_1_5)

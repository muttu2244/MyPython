...
DESCRIPTION:Flood SYN Fragment packets to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_038
...
HOW TO RUN:python2.5 ip_atk_038.py
...
class test_ip_atk_038(test_case):
    myLog = getLogger()	
	...
	#Flood SYN Fragment packets to SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping -V -S -f %s -P 22 -I %s --flood &"%(script_var['ssx_ip_addr'],p1_ssx_linux1[1]))
	time.sleep(40)

	#killing hping process
        self.linux1.cmd("sudo pkill -p hping") 

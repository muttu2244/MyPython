...
DESCRIPTION:Run nmap tool onto SSX to send raw IP Packets without any protocol header
TEST PLAN:IP-ATTACK test plan
TEST CASEID:ip_atk_070
...
HOW TO RUN:python2.5 ip_atk_070.py
AUTHOR:Ashu (agupta@stoke.com)
REVIEWER:Srinivas(srinivas@stoke.com)
...
class test_ip_atk_070(test_case):
    myLog = getLogger()	
	...
	#Run nmap tool onto SSX to send raw IP Packets without any protocol header
	out = self.linux1.cmd("sudo /usr/bin/nmap -sO %s  & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

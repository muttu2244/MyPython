...
class test_SAN_ACL_013(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['SAN_ACL_013_OUT'])
	...
	#changing context and clearing ip counters
        ...
        self.failUnlessEqual(output,1,"ip_counters_icmp failed")
	       # Push SSX config
        self.ssx.config_from_string(script_var['SAN_ACL_013_IN'])
        #changing context and clearing ip counters
        self.ssx.cmd("context %s" %(script_var['context_name']))
        time.sleep(5)
        self.ssx.cmd("clear ip counters")

...
class test_ACL_FUN_003(test_case):
	...
	#self.ssx.cmd("show ip counters icmp")     
        output=ip_verify_ip_counters_icmp(self.ssx,total_tx='1',total='0',echo_request='0', echo_reply='1', unreachable='0', \
        mask_request='0', mask_reply='0', source_quench= '0' , param_problem='0', timestamp='0',\
        redirects='0', info_reply='0', ttl_expired='0', other='0')
        print output

        self.failIfEqual(output,0,"Icmp Permit for Type 8 Failed")
	
	self.ssx.cmd("clear ip counters")
        time.sleep(5)
        
	#applying nemesis tool for generating icmp packets of not TYPE-8
        self.linux.cmd("sudo /usr/local/bin/nemesis icmp -c 7 -S %s -D %s -d %s "%( script_var['xpress_phy_iface1_ip'], script_var['ssx_phy_iface1_ip'],p1_ssx_xpressvpn[1]))
        time.sleep(20)

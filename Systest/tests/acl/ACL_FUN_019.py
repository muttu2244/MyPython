...
HOW TO RUN:python2.5 ACL_FUN_019.py
...
class test_SAN_ACL_019(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['SAN_ACL_019_OUT'])
	...
	output=ip_verify_ip_counters_icmp(self.ssx,total_tx='5',total='0',echo_request='5')
        self.failUnless(output == 1,"ip_counters_icmp failed")
	       ...
	       # Push SSX config
        self.ssx.config_from_string(script_var['SAN_ACL_019_IN'])
        #changing context and clearing ip counters
        self.ssx.cmd("context %s" %(script_var['context_name']))
...
if __name__ == '__main__':
	...
	filename = os.path.split(__file__)[1].replace('.py','.log')
        ...
        suite = test_suite()
        suite.addTest(test_SAN_ACL_019)

...
HOW TO RUN:python2.5 ACL_FUN_007.py
...
class test_ACL_FUN_007(test_case):
	...
	# Push SSX config
	self.ssx.config_from_string(script_var['SAN_ACL_007'])
	...
	ping_op=self.ssx.ping(dest=script_var['linux_phy_iface1_ip'])
        self.failUnless(ping_op == 0,"Ping through interface passed when acl with deny icmp rule applied")
	...
	# Push SSX config
	self.ssx.config_from_string(script_var['SAN_ACL_007_01'])

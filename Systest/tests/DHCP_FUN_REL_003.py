...
HOW TO RUN:python2.5 DHCP-FUN-REL-003.py
...
class test_DHCP_FUN_003(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['DHCP_FUN_003'])
	...
	#configure ip interface on linux machine
        ...
        op_client_cmd = self.linux2.cmd("sudo ./dhclient %s"%p1_ssx_linux2[1],timeout = 250)
	...
	self.failUnless("Discover" not in outstr.split())

...
DESCRIPTION: Send TCP packets with SYN and FIN bits set to SSX.
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_042
...
HOW TO RUN:python2.5 ip_atk_042.py
...
class test_ip_atk_042(test_case):
    myLog = getLogger()	
	...
	out = self.linux1.cmd("sudo /usr/sbin/hping -S  -F %s -p 23 -i u1 & "%script_var['ssx_ip_addr'])
        time.sleep(40)

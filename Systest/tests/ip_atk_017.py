...
DESCRIPTION:Two parts of fragment are send to SSX. 1. Payload 32 bytes long is the intial offset zero formate of the SYN packet. The second fragment of the SYN packet is also same in size, but is set to an offset of 64800bytes into a datagram

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_017
...
HOW TO RUN:python2.5 ip_atk_017.py
...
class test_ip_atk_017(test_case):
    myLog = getLogger()	
	...
	#Sending Two parts of fragment are send to SSX.
	out = self.linux1.cmd("sudo ./rose 1 %s  23 & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

	#killing rose process on Linux.
        self.linux1.cmd("sudo pkill -9 rose") 

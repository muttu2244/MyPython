...
DESCRIPTION:Flood ICMP echo request messages to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_029
...
HOW TO RUN:python2.5 ip_atk_029.py
...
class test_ip_atk_029(test_case):
    myLog = getLogger()	
	...
	out = self.linux1.cmd("sudo ./trash %s %s 10000000 &"%(script_var['ssx_ip_addr'],script_var['linux1_ip_addr']))
        time.sleep(30)

	#killing hping process
        self.linux1.cmd("sudo kill -9 trash") 

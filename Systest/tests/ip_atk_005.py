...
DESCRIPTION:Send IP header with option filled with source route. Source route with the following options : -          
	1.  Unreachable route             
	2.  Reachable route 
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_005
...
HOW TO RUN:python2.5 ip_atk_005.py
...
class test_ip_atk_005(test_case):
    myLog = getLogger()	
	...
	dst_ip_adr = script_var['linux1_ip_addr']
        ...
        #sending packets with reachable root.
        invoke_scapy(self.linux1,dst_mac_adr = dst_mac_address,sudo_passwd=linux1['password'],dst_ip_adr = dst_ip_adr ,from_interface=p1_ssx_linux1[1],log = self.myLog,count = 100)

        #displaying ip,port and syscounters and Cpu utilization
	self.myLog.output("\nSending packets with reachable ip address \n")
	self.myLog.output("\n Displaying control plane rate limiter \n%s\n"%self.ssx.cmd("show dos slot %s counters"%p1_ssx_linux1_slot))
	...
	#sending packets with unreachable root.
	#clearing ip,port and syscounters.
	self.ssx.cmd("clear ip counters ")
	self.ssx.cmd("clear syscount")
	self.ssx.cmd("clear port %s counters drop"%script_var['ssx_port'])

	dst_ip_adr = "0.0.0.0"
        invoke_scapy(self.linux1,dst_mac_adr = dst_mac_address,sudo_passwd=linux1['password'],dst_ip_adr = dst_ip_adr ,from_interface=p1_ssx_linux1[1],ip_hdr_len = 10,log = self.myLog,count = 100)

	 #displaying ip,port and syscounters and Cpu utilization
        self.myLog.output("\nSent packets with unreachable ip address \n")   
        self.myLog.output("\n Displaying control plane rate limiter \n%s\n"%self.ssx.cmd("show dos slot %s counters"%p1_ssx_linux1_slot))

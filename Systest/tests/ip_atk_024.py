...
DESCRIPTION: Send ICMP oversized packet (>65499) to SSX
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_024
...
HOW TO RUN:python2.5 ip_atk_024.py
...
class test_ip_atk_024(test_case):
    myLog = getLogger()	
	...
	self.ssx.cmd("clear ip counters ")
        ...
        # Sending ICMP oversized packet (>65499) to SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping %s -s 65500 -c 10 --flood &"%script_var['ssx_ip_addr'])

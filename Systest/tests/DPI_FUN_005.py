...
DESCRIPTION: Detecting & classifying protocol, IMAP
TEST PLAN: DPI Test Plan
TEST CASES: DPI_PD_05
...
How to run: "python2.5 DPI_FUN_005.py"
...
from misc import *
#import configs file
pexpect.run('perl  -pi -e \'s/proto_from_script = "\w+"/proto_from_script = "imap"/\' ikev2_config.py')
from ikev2_config import *
from topo import *

class test_DPI_FUN_005(test_case):
    myLog = getLogger()
	...
	self.radius.cmd("sudo service dovecot start")
        imapOutput = self.xpress_vpn.cmd("sudo fetchmail -u regress -v -p IMAP %s" %ipRad2Ssx)
	time.sleep(5)
	self.failUnless(imapOutput, "imap is failed b/w ike client and remote host")
...
if __name__ == '__main__':
    ...
    suite = test_suite()
    suite.addTest(test_DPI_FUN_005)

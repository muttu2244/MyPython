...
DESCRIPTION: Confirm OSPF priority is defineable and DR/BDR is selectable based on it 
TEST PLAN: 
TEST CASES: NTT_4_1_12

HOW TO RUN: python2.5 NTT_4_1_12.py
...
class test_NTT_4_1_12(test_case):
    myLog = getLogger()
    ...
    def test_NTT_4_1_12(self):

        self.myLog.output("==================Starting The Test====================")
	
        # Push SSX config
	self.ssx1.config_from_string(route_var['NTT_4_1_12'])
	self.myLog.info("Configured the OSPF, \nConfiguring in the Cisco")
        ...
        self.cisco.cmd("no int vlan 100")
	...
	self.cisco.cmd("ip address %s"%route_var['cisco_ospf_intf1_ip/mask'])
        self.cisco.cmd("ip ospf priority 10")
	...
	time.sleep(100)
        ...
        op = self.ssx1.cmd("show ip  ospf interface vr1")
        if "Priority 100" not in op :
           self.fail("DR is not selectable based on priority")
        op = self.cisco.cmd("sh ip os interface ten %s"%p_ssx1_cisco[1])
        if "Priority 10" not in op :
           self.fail("BDR is not selectable based on priority")
...
if __name__ == '__main__':
        ...
        log = buildLogger("NTT_4_1_12.log", debug=True,console=True)
        suite = test_suite()
        suite.addTest(test_NTT_4_1_12)

...
DESCRIPTION:To Verify CDR provides valid logs for invalid ip address configured for the upload server(FTP,TFTP and SFTP)
TEST PLAN: CDR Test plans
TEST CASES: CDR_NEG_003
...
class test_CDR_NEG_003(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_NEG_003'])
        ...
        if "Can't connect or login to host" in ssx_log:
          self.myLog.info(" CDR generation failed due to invalid ip address")
        else:
          self.myLog.info(" CDR generation passed due to invalid ip address")
        ...
        self.failUnless(' ' not in asn_file , "TTLV files are not generated when invalid ip address configured for upload server")
        self.failUnless(' ' not in xml_file , "XML files are not generated when invalid ip address configured for upload server")

...
DESCRIPTION:To Verify  CDR data gets uploaded in XML format to the configured server with SFTP protocol.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_034
...
class test_CDR_FUN_034(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_032'])
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        linuxip = self.linux.cmd("ls -rth ~krao | grep \".xml\" | tail -n 1")
        ...
        self.myLog.info("*" *50)
        self.myLog.output("XML files were generated using SFTP protocol")
        self.myLog.info("*" *50)

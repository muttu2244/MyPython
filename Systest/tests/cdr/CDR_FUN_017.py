...
DESCRIPTION:To verify CDR intermediate ttlv files are getting deleted after the disk-commit interval time
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_017
...
class test_CDR_FUN_017(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 1 min to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        ...
        self.ssx.shellcmd("ls -rth /hdp/cdr/%s/%s " %(script_var['context_name'],script_var['upload_profile4']))
        temp = self.ssx.shellcmd("ls -rth /hdp/cdr/%s/%s " %(script_var['context_name'],script_var['upload_profile4']))

        if not temp:
         self.myLog.info("TEMP FILES ARE NOT DELETD AFTER UPLOAD UNDER /hdp/cdr/cdr-1/temp")
        else:
         self.myLog.info("TEMP FILES ARE DELETD AFTER UPLOAD UNDER /hdp/cdr/cdr-1/temp")

        self.failUnless(temp , "TEMP FILES ARE NOT DELETD AFTER UPLOAD UNDER /hdp/cdr/cdr-1/temp")

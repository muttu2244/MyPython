...
class test_CDR_FUN_006(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_015'])
	...
	self.ssx.configcmd("exit")
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
        self.myLog.info("Waiting for 3 mins to change the upload interval time to 10 mins for CDR files to upload on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        time.sleep(180)
	...
	self.ssx.config_from_string(script_var['CDR_FUN_006'])
        ...
        self.myLog.info("Checking the Intermediate TTLV files on SSX-%s ...... " % ssx['ip_addr'])
        ls_temp = self.ssx.shellcmd("ls /hdp/cdr/%s/%s" %(script_var['context_name'],script_var['upload_profile4']))
	self.ssx.shellcmd("exit")
        self.myLog.output(ls_temp)


        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
        self.myLog.info("Waiting for 10 mins to get the final files generated on the linux machine-%s after change of cdr conf...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        ...
        self.linux.cmd("ls -rth /tftpboot/ | grep \".asn\" ")

        linuxip = self.linux.cmd("ls -rth /tftpboot/ | grep \".ttlv\" | tail -n 1")

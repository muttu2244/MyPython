...
DESCRIPTION:To Verify CDR generated files have naming conventions according to the 3GPP standard.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_053
...
class test_CDR_FUN_053(test_case):
    myLog = getLogger()
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        #3GPP Standard
        standard_3GPP = "<NodeID>_-_<RC>. <date>_-_<time> [.<PI>][.<FE>]"
        regular_3GPP =  ".*\_\-\_\d+\.\d+\_\-\_\d+\+\d+\.\w+\.\w+"
        compare_3GPP_CDR = re.match(regular_3GPP , linuxip1)
        compare_3GPP_CDR1 = compare_3GPP_CDR.group(0) 
       
        self.failUnless(compare_3GPP_CDR1,"The genrated CDR File is not in 3GPP standard")
        self.myLog.info("The genrated CDR File is in 3GPP standard")

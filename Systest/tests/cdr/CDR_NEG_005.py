...
DESCRIPTION:To Verify CDR provides valid logs for incorrect password configured for the upload server(FTP and SFTP).
TEST PLAN: CDR Test plans
TEST CASES: CDR_NEG_005
...
class test_CDR_NEG_005(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_NEG_005'])
...
#        ssx_log = self.ssx.cmd("sh log | grep \"WARN\" ")
        time.sleep(5)
        self.myLog.output(ssx_log)
        if "Can't connect or login to host" in ssx_log:
          self.myLog.info(" CDR generation failed due to incorrect password")
        else:
          self.myLog.info(" CDR generation passed due to incorrect password")
        ...
        if not asn_file:
         self.myLog.info("ASN files are generated when  incorrect password configured for upload server")
        else:
         self.myLog.info("ASN files are not generated when incorrect password configured for upload server")

        if not xml_file:
         self.myLog.info("XML files are generated when incorrect password configured for upload server")
        else:
         self.myLog.output("XML files are not generated when incorrect password configured for upload server")

        self.failUnless(' ' not in asn_file , "TTLV files are not generated when incorrect password configured for upload server")
        self.failUnless(' ' not in xml_file , "XML files are not generated when incorrect password configured for upload server")

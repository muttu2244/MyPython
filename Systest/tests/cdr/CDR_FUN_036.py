...
DESCRIPTION:To Verify  CDR data gets uploaded in ASN.1 format to the configured server with TFTP protocol.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_036
...
class test_CDR_FUN_036(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_036'])
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        linuxip = self.linux.cmd("ls -rth /tftpboot | grep \".asn\" | tail -n 1")
        linuxip1 = linuxip.strip()
        self.myLog.output(linuxip1)
 
        self.failUnless(linuxip1,"Failed to generate asn files using TFTP protocol")
        self.myLog.output("CDR data generation passed with disk-commit-interval as 1 min")
        self.myLog.info("*" *50)
        self.myLog.output("ASN files were generated using TFTP protocol")
        self.myLog.info("*" *50)

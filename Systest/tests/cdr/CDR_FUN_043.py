...
class test_CDR_FUN_043(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 30 seconds to change the cdr configuration with change in include filter on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        ...
        self.ssx.cmd("context %s" %(script_var['context_name']))
        self.ssx.cmd("cdr")
        self.ssx.cmd("upload-profile customer")
        self.ssx.cmd("include 4")
        self.ssx.cmd("exit")
        self.ssx.cmd("exit")
        self.ssx.cmd("exit")
        self.ssx.cmd("exit")
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
        self.myLog.info("Checking of ttlv files with change in include filter which are generated on SSX-%s ...... " % ssx['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
	...
	res = self.ssx.ftppasswd("copy sftp://root@%s:/tftpboot/%s /hd/%s noconfirm" %(script_var['upload_server_mgmt_ip'],linuxtftp2,linuxtftp2))
        ...
        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 0 | grep \"Record type(4)=SESSION_START\"" % linuxtftp2)

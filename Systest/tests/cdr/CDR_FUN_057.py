...
DESCRIPTION: To Verify no crash occurs during deleting configured  CDR context when CDRs are still pending to be written to final format     
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_057
...
class test_CDR_FUN_057(test_case):
    myLog = getLogger()
	...
	#if linuxip: 
	#self.myLog.output("No CDR files Generated")	


        #self.failUnless(linuxip,"Test case: PASS , no CDR's generated when no CDR configured")
        self.myLog.info("*" *50)

...
DESCRIPTION:To Verify  CDR data generation with disk-commit-interval as minimum (1 min).
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_010
...
class test_CDR_FUN_010(test_case):
    myLog = getLogger()
	...
	res = self.ssx.ftppasswd("copy sftp://root@%s:/tftpboot/%s /hd/%s noconfirm" %(script_var['upload_server_mgmt_ip'],linuxtftp2,linuxtftp2))
        ...
        #ttl1= self.ssx.shellcmd("cdrdump -f CDR_FUN_010.ttlv -r 0 | grep \"icmp_policy\" ")

...
DESCRIPTION:To Verify  CDR data generation(ASN.1) at path /hdp/cdr/<ctx-name>/<profile-name>/after the disk-committed time.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_015
...
class test_CDR_FUN_015(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_012'])
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        self.failUnless(ls_customer,"Failed to generate ASN Files")
        self.failUnless(ls_backup, "Failed to generate ASN files")
        self.failUnless(ls_stoke_log,"Failed to generate ASN Files")
        self.myLog.info("*" *50)

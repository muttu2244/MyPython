...
DESCRIPTION:To Verify  CDR data generation with disk-commit-interval as default.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_012
...
class test_CDR_FUN_012(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 5 mins to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        ...
        self.linux.cmd("ls -rth ~krao | grep \".asn\"")
        
        linuxip = self.linux.cmd("ls -rth ~krao | grep \".asn\" | tail -n 1 ")
        ...
        self.myLog.output("CDR data generation passed with disk-commit-interval as default")
        self.myLog.info("*" *50)

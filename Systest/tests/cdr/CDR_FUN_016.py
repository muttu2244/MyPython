...
DESCRIPTION:To Verify  CDR data generation(XML) at path /hdp/cdr/<ctx-name>/<profile-name> after the disk-committed time.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_016
...
class test_CDR_FUN_016(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_016'])
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 8 min to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        ...
        self.failUnless(linuxip1,"Failed to generate XML files using SFTP protocol")
        self.myLog.output("CDR data generation passed with disk-commit-interval as 8 mins")
        self.myLog.info("*" *50)
        self.myLog.output("XML files were generated after disk-committed time")
        self.myLog.info("*" *50)

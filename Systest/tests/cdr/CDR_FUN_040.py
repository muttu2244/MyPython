...
DESCRIPTION:To Verify  CDR data gets uploaded with series of  include filters.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_040
...
class test_CDR_FUN_040(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
        self.myLog.info("Pushing configuration with multiple include filters 0,1,3")
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
	...
	#self.failUnlessEqual(ttl1,"[0] Record type(0)=SESSION_START", "SESSION_START Record not found in ttlv file generated")

        #Checking for Record type(3)=SESSION_INTERIM_UPDATE in  ttlv files using CDR dump tool

        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 1 | grep \"Record type(3)=SESSION_INTERIM_UPDATE\"" % linuxtftp2)
        self.myLog.output(ttl1)
	self.failUnless('Record index too high' not in ttl1, "Record is not generated")
...
#        if res == -1:
#                self.failUnless(0,"CLASS START record not found on files generated on the Linux machine")


#        self.failUnless('Record type(4)=CLASS_START' in ttl1, "Record is not generated")

#        Out = Verify_class_name_in_cdr_records(self.ssx,record = ttl1,class_name ="icmp_class",test_string="Record type(4)=CLASS_START")
#        self.failUnless(Out == 1,"Test case failed due to incorrect class_name parameter in cdr record of class")   Record type(3)=SESSION_INTERIM_UPDATE
        ...
        time.sleep(10)
        self.ssx.shellcmd("cd /hd")
        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 0 | grep \"Record type(1)=SESSION_STOP\"" % linuxtftp2)
        self.myLog.output(ttl1)
	self.failUnless('Record index too high' not in ttl1, "Record is not generated")

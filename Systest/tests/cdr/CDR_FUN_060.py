...
DESCRIPTION: To Verify  CDR data generation with disk-commit-interval as max (1440 Min)
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_060
...
class test_CDR_FUN_060(test_case):
    myLog = getLogger()
	...
	self.ssx.config_from_string(script_var['CDR_FUN_002C'])

        #Show the running configuration of CDR
	self.ssx.cmd("show conf cdr")
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 1440 mins to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
	...
	if mib == -1:
        	self.myLog.output("Test case: PASS , CDR's not yet commited to disk")
	else:
        	self.failUnless(0,"Test case: FAIL , CDR's are commited to disk")
	...
	time.sleep(120*60)
        ...
        if mib == -1:
                self.failUnless(0,"Test case: FAIL , CDR's are not commited to disk")
        else:
                self.myLog.output("Test case: PASS , CDR's are commited to disk")
...
if __name__ == '__main__':
	...
	filename = os.path.split(__file__)[1].replace('.py','.log')
        ...
        suite = test_suite()
        suite.addTest(test_CDR_FUN_060)

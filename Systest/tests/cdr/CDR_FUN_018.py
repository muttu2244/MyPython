...
DESCRIPTION: To Verify  CDR data gets uploaded to the servers with max upload profiles (3) bound to the management port
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_018
...
class test_CDR_FUN_018(test_case):
    myLog = getLogger()
	...
	self.ssx.config_from_string(script_var['CDR_FUN_018'])
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        self.myLog.info("*" *50)
        self.myLog.output("XML files were generated")
        self.myLog.info("*" *50)

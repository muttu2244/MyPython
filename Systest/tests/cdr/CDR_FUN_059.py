...
DESCRIPTION: To Verify CDR data generation and upload  when SSX is booted with a new configuration having CDR enabled
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_059
...
class test_CDR_FUN_059(test_case):
    myLog = getLogger()
	...
	# Push SSX config
        ...
        self.ssx.cmd("save configuration mycdr")
        time.sleep(2)
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        #Saving the configuration.
        self.ssx.cmd("boot configuration mycdr")
        time.sleep(80)
	...
	if mib == -1:
        	self.failUnless(0,"Test case: FAIL , CDR's not generated after boot")
	else:
        	self.myLog.output("Test case: PASS , CDR's generated even after boot")

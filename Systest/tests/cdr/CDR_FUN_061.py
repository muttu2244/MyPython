...
DESCRIPTION: To verify CDR data gets uploaded (TFTP) successfully for the huge ( 1GB) data generated
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_061
...
class test_CDR_FUN_061(test_case):
    myLog = getLogger()
	...
	self.ssx.config_from_string(script_var['CDR_FUN_002D'])
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 2hrs to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
	...
	if mib == -1:
        	self.failUnless(0,"Test case: FAIL , CDR's are not uploaded")
	else:
        	self.myLog.output("Test case: PASS , CDR's are uploaded")
...
if __name__ == '__main__':
	...
	filename = os.path.split(__file__)[1].replace('.py','.log')
        ...
        suite = test_suite()
        suite.addTest(test_CDR_FUN_061)

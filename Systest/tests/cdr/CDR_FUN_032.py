...
DESCRIPTION:To Verify  CDR data gets uploaded in XML format to the configured server with FTP protocol.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_032
...
class test_CDR_FUN_032(test_case):
    myLog = getLogger()
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        self.failUnless(linuxip1,"Failed to generate XML files using FTP protocol")
        self.myLog.output("CDR data generation passed with disk-commit-interval as 1 min")
        self.myLog.info("*" *50)
        self.myLog.output("XML files were generated using FTP protocol")
        self.myLog.info("*" *50)

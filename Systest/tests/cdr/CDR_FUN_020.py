...
DESCRIPTION:To Verify  CDR data gets uploaded to the servers with max upload profiles (3) bound to 3 different slots.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_020
...
class test_CDR_FUN_020(test_case):
    myLog = getLogger()
	...
	self.linux.configure_ip_interface(p1_ssx_xpressvpn1[1], script_var['xpress_phy_iface1_ip_mask'])
        ...
        self.ssx.config_from_string(script_var['CDR_FUN_020'])

        # Push xpress vpn config
        self.linux.write_to_file(script_var['autoexec_config_EAP'],"autoexec.cfg","/xpm/")
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        linuxip = self.linux.cmd("ls -rth cdr* |  grep \".xml\" | tail -n 1 ")
        linuxip1 = linuxip.strip()
        self.myLog.output(linuxip1) 
        self.failUnless(linuxip1,"CDR data is not uploaded to the servers with max upload profiles bound to 3 slots")
        self.myLog.output("CDR data got uploaded to the servers with max upload profiles bound to 3 slots ")
	...
	self.myLog.info("Checking of xml files which are generated on Linux machine-%s ...... " % linux['ip_addr'])
        ...
        self.linux.cmd("cd")
        self.linux.cmd("cd /tftpboot/")
        linuxtftp = self.linux.cmd("ls -rth | tail -n 1")
	...
	res = self.ssx.ftppasswd("copy sftp://root@%s:/tftpboot/%s /hd/%s noconfirm" %(script_var['upload_server_mgmt_ip'],linuxtftp2,linuxtftp2))
        ...
        #ttl1= self.ssx.shellcmd("cdrdump -f CDR_FUN_020.ttlv -r 0 | grep \"icmp_policy\" ")

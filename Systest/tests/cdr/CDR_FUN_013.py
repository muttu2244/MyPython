...
DESCRIPTION:To Verify  CDR data generation for default-disk-commit interval with no upload profile set
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_013
...
class test_CDR_FUN_013(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_013'])
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
	self.myLog.info("Waiting for 5 mins seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")
        
        time.sleep(300) 

        ls_customer = self.ssx.shellcmd("ls /hdp/cdr/%s/%s" %(script_var['context_name'],script_var['upload_profile1']))
        self.myLog.output(ls_customer)
        ls_backup = self.ssx.shellcmd("ls /hdp/cdr/%s/%s" %(script_var['context_name'],script_var['upload_profile2']))
        self.myLog.output(ls_backup)
        ls_stoke_log = self.ssx.shellcmd("ls /hdp/cdr/%s/%s" %(script_var['context_name'],script_var['upload_profile3']))
        self.myLog.output(ls_stoke_log)
        ls_temp = self.ssx.shellcmd("ls /hdp/cdr/%s/%s" %(script_var['context_name'],script_var['upload_profile4']))
        self.myLog.output(ls_temp)
        ...
        asn_file = self.linux.cmd("ls -rth ~krao | grep \".asn \"")

        if not ttlv_file:
         self.myLog.info("TTLV FILES ARE GENERATED FOR NO UPLOAD PROFILE")
        else:
         self.myLog.info("TTLV FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")

        if not xml_file:
         self.myLog.info("XML FILES ARE GENERATED FOR NO UPLOAD PROFILE")
        else:
         self.myLog.output("XML FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")

        if not asn_file:
         self.myLog.info("ASN FILES ARE GENERATED FOR NO UPLOAD PROFILE")
        else:
         self.myLog.output("ASN FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")

        self.failUnless(' ' not in ttlv_file , "TTLV FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")
        self.failUnless(' ' not in xml_file , "XML FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")
        self.failUnless(' ' not in asn_file , "ASN FILES ARE NOT GENERATED FOR NO UPLOAD PROFILE")

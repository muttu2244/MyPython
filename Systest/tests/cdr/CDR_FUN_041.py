...
DESCRIPTION:To Verify  CDR data gets uploaded with All include filters.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_041
...
class test_CDR_FUN_041(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.myLog.info("\n\n")
        self.myLog.info("*" *50)
        self.myLog.info("Pushing configuration with multiple include filters 4,5,6")
        self.myLog.info("*" *50)
        self.myLog.info("\n\n")

        self.ssx.config_from_string(script_var['CDR_FUN_040'])
        self.ssx.config_from_string(script_var['CDR_FUN_041'])
	...
	res = self.ssx.ftppasswd("copy sftp://root@%s:/tftpboot/%s /hd/%s noconfirm" %(script_var['upload_server_mgmt_ip'],linuxtftp2,linuxtftp2))
        ...
        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 1 | grep \"Record type(4)=CLASS_START\"" % linuxtftp2)
	...
	self.failUnless('Record index too high' not in ttl1, "Record is not generated")
        ...
        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 2 | grep \"Record type(3)=SESSION_INTERIM_UPDATE\"" % linuxtftp2)
        self.myLog.output(ttl1)
        self.failUnless('Record index too high' not in ttl1, "Record is not generated")
	...
	self.failUnless('Record index too high' not in ttl1, "Record is not generated")
        ...
        #res = self.ssx.cmd("copy tftp://17.1.2.1/%s /hd" % linuxtftp2)
        ttl1= self.ssx.shellcmd("cdrdump -f %s -r 1 | grep \"Record type(1)=SESSION_STOP\"" % linuxtftp2)
        self.myLog.output(ttl1)
        self.failUnless('Record index too high' not in ttl1, "Record is not generated")

...
DESCRIPTION:To Verify no CDR process crashes if TTLV files deleted manually from standby IMC before IMC switch over 
TEST PLAN: CDR Test plans
TEST CASES: CDR_HA_FUN_003
...
class test_CDR_HA_FUN_003(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        self.ssx.config_from_string(script_var['CDR_FUN_033A'])
	...
	self.ssx.shellcmd("cd /hdp/cdr/cdr-1/temp/")
	self.ssx.shellcmd("rm -rf *")

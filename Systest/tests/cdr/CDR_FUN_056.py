...
DESCRIPTION:To Verify the date field in the CDR file name changes acording to the changed date
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_056
...
class test_CDR_FUN_056(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        #Date in old clock
        old_clock = self.ssx.cmd("show clock")
        self.myLog.output(old_clock)

        #Changing the date filed in SSX
        self.ssx.cmd("clock set 2015:01:25:22:44:34")
        new_clock = self.ssx.cmd("show clock")      
        changed_clock = change_clock_ssx(self,new_clock)
        self.myLog.output(changed_clock)
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        self.linux.cmd("ls -rth /tftpboot/cdr-1*")	
        self.linux.cmd("ls -rth ~krao/cdr-1*")
        
        self.linux.cmd("cd ~krao")
        linuxip = self.linux.cmd("ls -rth cdr-1* | tail -n 1 ")
        linuxip1 = linuxip.strip()
        self.myLog.output(linuxip1)
        self.failUnless(linuxip1,"Failed to generate asn files using SFTP protocol")
        self.myLog.output("CDR data generation passed with disk-commit-interval as 1 min")
        self.myLog.output("ASN files were generated using SFTP protocol")
     
        #Grepping the date from generated CDR File. 
        linux_cdr_date=re.search("\w+\_\-\_\d+\.(\d+)\_\-\_\.*", linuxip1)
        linux_cdr_date1 = linux_cdr_date.group(1)
        self.myLog.output(linux_cdr_date1)
   
        #Comparing the date field of CDR file with the changed date on SSX
        self.failUnless(changed_clock in linuxip1 ,"Date field in CDR  does not changed according to changed date on SSX")
        self.myLog.info("*" *50)
        self.myLog.info("Date field in CDR changed according to changed date on SSX")
        self.myLog.info("*" *50)

...
DESCRIPTION:To Verify  CDR data gets uploaded in TTLV format to the configured server with SFTP protocol.
TEST PLAN: CDR Test plans
TEST CASES: CDR_FUN_031
...
class test_CDR_FUN_031(test_case):
    myLog = getLogger()
	...
	# Push xpress vpn config on linux
        ...
        # Push SSX config
        self.ssx.config_from_string(script_var['CDR_FUN_031'])
	...
	self.myLog.info("Waiting for 60 seconds to get the files generated on the linux machine-%s ...... " % linux['ip_addr'])
        ...
        ttlv_files = self.linux.cmd("ls -rth ~krao | grep \".ttlv\" ")	
        self.failUnless(ttlv_files,"TTLV files are not generated using SFTP protocol")
        self.myLog.output("TTLV files are generated using SFTP protocol")

        linuxip = self.linux.cmd("ls -rth ~krao | grep \".ttlv\" | tail -n 1 ")
        ...
        linuxtftp = self.linux.cmd("ls -rth ~krao | grep \".ttlv\" | tail -n 1")
	...
	res = self.ssx.ftppasswd("copy sftp://root@%s:/home/krao/%s /hd/%s noconfirm" %(script_var['upload_server_mgmt_ip'],linuxtftp2,linuxtftp2))
        time.sleep(10)

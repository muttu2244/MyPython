...
DESCRIPTION:Run nmap tool onto SSX to send tcp packets with ACK bit set
TEST PLAN:IP-ATTACK test plan
TEST CASEID:ip_atk_071
...
HOW TO RUN:python2.5 ip_atk_071.py
...
class test_ip_atk_071(test_case):
    myLog = getLogger()	
	...
	#Run nmap tool onto SSX to send ACk packets
	out = self.linux1.cmd("sudo /usr/bin/nmap -sA %s  & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

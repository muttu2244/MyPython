...
DESCRIPTION:Run nmap tool onto SSX to send OS fingerprinting attack
TEST PLAN:IP-ATTACK test plan
TEST CASEID:ip_atk_073
...
HOW TO RUN:python2.5 ip_atk_073.py
...
class test_ip_atk_073(test_case):
    myLog = getLogger()	
	...
	#Run nmap tool onto SSX to send OS fingerprinting attack
	out = self.linux1.cmd("sudo /usr/bin/nmap -O %s  & "%(script_var['ssx_ip_addr']))
	time.sleep(30)

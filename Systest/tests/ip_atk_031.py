...
DESCRIPTION:ICMP spoofed unrechable flood smack/puke attack
TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_031
...
HOW TO RUN:python2.5 ip_atk_031.py
AUTHOR:agupta@stoke.com
...
class test_ip_atk_031(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_027'])
	...
	#Flood icmp echo request messages to SSX.
	out = self.linux1.cmd("sudo ./bloop %s %s 10000 &"%(script_var['ssx_ip_addr'],script_var['linux1_ip_addr']))
        time.sleep(100)

	#killing hping process
        self.linux1.cmd("sudo kill -9 hping") 

...
DESCRIPTION: Send 1000 byte of IP fragment with an offset of 65500 passing through SSX

TEST PLAN:IP-ATTACK test plans
TEST CASEID:ip_atk_087
...
HOW TO RUN:python2.5 ip_atk_087.py
...
class test_ip_atk_087(test_case):
    myLog = getLogger()	
	...
	self.ssx.config_from_string(script_var['DOS_FUN_087'])
	...
	#Sending 1000 byte of IP fragment with an offset of 65500 passing through SSX
	out = self.linux1.cmd("sudo /usr/sbin/hping -d 1000 -g 65000 %s &"%(script_var['ssx_ip_addr']),timeout = 350)
        time.sleep(30)

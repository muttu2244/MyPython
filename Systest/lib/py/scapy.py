...
#
# $Log: scapy.py,v $
# Revision 1.3  2007/04/27 23:05:44  don
# AutoPR:
# DefectId:
# Reviewer:
# Tested:
# Description:
# fixing annoying:
# /volume/labtools/systest/tests/precommit/../../lib/py/scapy.py:3233: Warning: 'with' will become a reserved keyword in Python 2.6
# messages
#
# Revision 1.2  2007/04/26 23:03:42  don
# AutoPR:
# DefectId:
# Reviewer:
# Tested:
# Description:
# Collapsing branch_refactor_dev into mainline.  this includes:
# new test framework
# new precommit tests
# sip tests
# ike tests
...
# umatt test tool
#
# Revision 1.1.2.1  2007/04/19 22:05:58  morris
# AutoPR:
# Reviewer:
# Tested: Yes, ran framework unit tests. Some SSX tests failed,
#  but they don't appear related to these changes. Will look into them
#  seperately.
# Description: Add Stoke copyright, and shebang magic, #!/usr/bin/env python2.5.
#
# Revision 1.1  2006/07/25 21:12:53  sam
# put python libs under cvs
#
...
RCSID="$Id: scapy.py,v 1.3 2007/04/27 23:05:44 don Exp $"
...
#                loctrace.append((ip,loc)) # no labels yet
        ...
        tr = map(lambda x: Gnuplot.Data(x,withfoo="lines"), trt.values())
        g = Gnuplot.Gnuplot()
        world = Gnuplot.File(conf.gnuplot_world,withfoo="lines")
        g.plot(world,*tr)
        return g
...
class L2Socket(SuperSocket):
    def __init__(self, iface = None, type = ETH_P_ALL, filter=None, nofilter=0):
        ...
        self.outs = self.ins
        self.outs.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 2**30)
        self.outs.setblocking(0) #sam devlin <06/07/13 23:47 sam> 
...
def sendpraw(x, inter=0, loop=0, iface=None, iface_hint=None, verbose=None, *args, **kargs):
    """Send packets at layer 2
send(packets, [inter=0], [loop=0], [verbose=conf.verb]) -> None"""
    if iface is None and iface_hint is not None:
        iface = conf.route.route(iface_hint)[0]
    __send_raw(conf.L2socket(iface=iface, *args, **kargs), x, inter=inter, loop=loop, verbose=verbose)

def __send_raw(s, x, inter=0, loop=0, verbose=None, *args, **kargs):
    if verbose is None:
        verbose = conf.verb
    n = 0
    try:

...
class CISCO:
    ...
    def telnet(self, host, username="stoke", password="stoke"):
        ...
        """
	log.output("Trying to connect to CISCO host %s " % host)
	...
	log.output("Trying to connect to CISCO host %s " % host)
        ...
        try:
            while True:
                ...
                self.flush_child()
                self.ses.sendline()
                index = self.ses.expect(["Enter your option :", "sername:", config_prompt_regex, enable_prompt_regex, shell_prompt_regex,presRetPrompt,escapeCharPrompt])
...
#           print "child.before: %s" % self.ses.before
    ...
    def init_vars(self,hostname):
        """API is to initial variable for the telnet session to Cisco box
        This API is internally used telnet API. Modified by Jameer to make sure that terminal
        monitor is disabled, this is to make sure smooth run of the script. 
        """
...
#        self.name = blahblahblah
	...
	# Added code to handle one gigabit and ten gigabit commands without changing the scripts
	if (("Invalid input") in retstr and ("gig") in command):
                command = command.replace("gig","tengig")
                self.flush_child()
                self.ses.sendline(command)
                self.ses.expect(re.escape(command)) #clean this out of output buffer
                retstr = ""
    ...
    def configure_ipv4_native_vlan_interface(self,ip_addr="1.1.1.1 255.255.255.0",intf="5/10",vlan=1,vrf=None,mtu="1500"):
        """ API to configure native vlans in CISCO device
        Usage: configure_ipv4_native_vlan_interface(self.cisco, ip_address, interface, vlan, vrf)
        """
        ip1 = ip_addr.split()[0]
        out = self.cmd("sh ip interface brief | inc %s"%intf).strip()
        intType=re.search("(\w+Ethernet)",out).group(0)

        self.cmd("config t")
        self.cmd("interface %s %s"%(intType,intf))
        ...
        for i in range(len(ip_op.splitlines())-1):
                ...
                if cmp_op == 0:
                        ...
                        self.cmd("config t")
                        self.cmd("interface "%(ip_op.splitlines()[i+1].split()[0]))
                        self.cmd("no ip address")
                        self.cmd("end")
        self.cmd("config t")
        self.cmd("interface vlan %s"%vlan)
        ...
        if "Ten" not in intType:
            self.cmd("switchport trunk encapsulation dot1q")
        self.cmd("switchport trunk native vlan %s"%vlan)
    ...
    def configure_ipv4_interface(self,ip_addr="1.1.1.1 255.255.255.0",intf="5/10",vrf=None,vlan=1,mtu="1500"):
	ip1 = ip_addr.split()[0]
        out = self.cmd("sh ip interface brief | inc %s"%intf).strip()
        intType=re.search("(\w+Ethernet)",out).group(0) 
	...
	for i in range(len(ip_op.splitlines())-1):
		cmp_op = compare2ipv4(ip1,ip_op.splitlines()[i+1].split()[1],index=3)
                if cmp_op == 0:
                        if 'Vlan' in ip_op.splitlines()[i+1].split()[0]:
                           self.cmd("config t")
                           print ip_op.splitlines()[i+1].split()[0]
                           self.cmd("interface  %s"%ip_op.splitlines()[i+1].split()[0])
                           self.cmd("no ip address")
                           self.cmd("end")
                        else:
                           obj = re.compile("\d+\/\d+")
                           m= obj.search(ip_op.splitlines()[i+1].split()[0])
			   #if obj.search(ip_op.splitlines()[i+1].split()[0]):
			#	self.clear_interface_config(intf=m.group())
			   #print m
                           self.clear_interface_config(intf=m.group())
	self.cmd("config t")
	self.cmd("interface %s %s"%(intType,intf))
    ...
    def configure_ipv4_interface_no(self,ip_addr="1.1.1.1 255.255.255.0",intf="5/10",vrf=None,vlan=1):
        ip1 = ip_addr.split()[0]
        out = self.cmd("sh ip interface brief | inc %s"%intf).strip()
        intType=re.search("(\w+Ethernet)",out).group(0)
    ...
    def configure_ipv4_vlan_interface(self,ip_addr="1.1.1.1 255.255.255.0",intf="5/10",vlan=1,vrf=None,mtu="1500"):
	ip1 = ip_addr.split()[0]
        out = self.cmd("sh ip interface brief | inc %s"%intf)
        intType=re.search("(\w+Ethernet)",out).group(0)
		...
		cmp_op = compare2ipv4(ip1,ip_op.splitlines()[i+1].split()[1],index=3)
                if cmp_op == 0:
                        if 'Vlan' in ip_op.splitlines()[i+1].split()[0]:
                           ...
                           obj = re.compile("\d+\/\d+")
                           m= obj.search(ip_op.splitlines()[i+1].split()[0])
                           self.clear_interface_config(self,intf="%s" %m.group())
	self.cmd("config t")
	self.cmd("interface vlan %s"%vlan)
	self.cmd("no shutdown")
        if vrf != None:
    ...
    def bind_ipv4_vlan_interface(self,intf="5/10",vlan=1,vrf=None):
        ...
        self.cmd("end")
        self.cmd("config t")
        self.cmd("no spanning-tree vlan %s" %vlan)
        self.cmd("interface %s %s"%(intType,intf))
        self.cmd("switchport")
        self.cmd("switchport access vlan %s"%vlan)
        self.cmd("no shutdown")
    ...
    def bind_ipv4_native_vlan_interface(self,intf="5/10",vlan=1,vrf=None,vlanType = "untagged"):

        out = self.cmd("sh ip interface brief | inc %s"%intf)
        intType=re.search("(\w+Ethernet)",out).group(0)
        self.cmd("config t")
        self.cmd("no spanning-tree vlan %s" %vlan)
        self.cmd("no spanning-tree vlan 1")
        self.cmd("interface %s %s"%(intType,intf))
        self.cmd("no ip address")
        self.cmd("exit")
        self.cmd("interface vlan %s"%vlan)
        ...
        self.cmd("no spanning-tree vlan %s" %vlan)
        self.cmd("interface %s %s"%(intType,intf))
        if vlanType == "untagged":
            self.cmd("switchport trunk native vlan %s"%vlan)
        self.cmd("switchport trunk allowed vlan %s"%vlan)
        self.cmd("switchport mode trunk")
        self.cmd("no cdp enable")
        self.cmd("no spanning-tree bpdufilter")
        self.cmd("no spanning-tree bpduguard")
        self.cmd("no shutdown")
        self.cmd("end")
        return 0


    def configure_vrf(self,vrfName = "vrf1", id = "1:2",imp = "1:2", exp = "1:2"):

        self.cmd("config t")
        self.cmd("ip routing")
        self.cmd("ip vrf %s" %(vrfName))
        self.cmd("rd  %s" %(id))
        self.cmd("route-target export %s" %exp)
        self.cmd("route-target import %s" %imp)
        self.cmd("end")
        return 0

    def clear_vlan_interface_config(self,vlan_id = "500"):
        self.cmd("conf t")
        self.cmd("no interface vlan %s" %vlan_id)
        self.cmd("end")
        return 0


    def configure_interface_status(self,intf="5/10",status="no shut"):
       out = self.cmd("sh ip interface brief | inc %s"%intf)
       intType=re.search("(\w+Ethernet)",out).group(0)
    ...
    def configure_only_vlan_interface(self,ip_addr="1.1.1.1 255.255.255.0",vlan=1,vrf=None,mtu="1500"):
        ip1 = ip_addr.split()[0]
        self.cmd("config t")
        self.cmd("interface vlan %s"%vlan)
        self.cmd("no ip address")
        ...
        for i in range(len(ip_op.splitlines())-1):
                ...
                if cmp_op == 0:
                        if 'Vlan' in ip_op.splitlines()[i+1].split()[0]:
                           ...
                           obj = re.compile("\d+\/\d+")
                           m= obj.search(ip_op.splitlines()[i+1].split()[0]) 
                           self.clear_interface_config(intf="%s" %m.group())
    ...
    def configure_multi_vlan_interface(self,ips="1.1.1.1 255.255.255.0,3.1.1.1 255.255.255.0",vlans="1,2",intf="5/10",mtu="1500"):
        out = self.cmd("sh ip interface brief | inc %s"%intf)
        intType=re.search("(\w+Ethernet)",out).group(0)
    ...
    def configure_multi_vlan_interface_no(self,ips="1.1.1.1 255.255.255.0,3.1.1.1 255.255.255.0",vlans="1,2",intf="5/10"):
        out = self.cmd("sh ip interface brief | inc %s"%intf)
        intType=re.search("(\w+Ethernet)",out).group(0)
        VlanList = vlans.split(',')
        vlans_op=""
    ...
    def clear_interface_counters(self,intf):
        ...
        self.ses.sendline("clear counters % %s"%(intType,intf))
	...
	inFlag = outFlag = 0
	cntOp = self.cmd("show interface %s %s counters | begin InUcastPkts"%(intType,intf))
    ...
    def chk_ip_route_detail(self,route="",admin="",proto="",metric="",next_hop=""):
        """API is to verify the route details in Cisco.
           Usage Ex: chk_ip_route_detail(self.cisco,route="",admin="",proto="",metric="",next_hop="")
           Returns: returns two lists:
		passed - this list has the passed parameters
		failed - this list has the failed parameters
        """

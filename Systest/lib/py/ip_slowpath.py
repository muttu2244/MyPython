...
def ip_verify_ip_counters_arp(self,unknown_proto='none',length='none',
                                  rx_request='none',tx_request='none',
                                  target_null='none',src_ip_null='none',
                                  local_hw_addr='none',other='none',
				  loacl_ip_addr='none' ):


        expected={'unknown_proto':unknown_proto,'length':length,
                  'rx_request':rx_request,'tx_request':tx_request,
                  'target_null':target_null,'src_ip_null':src_ip_null,
                  'local_hw_addr':local_hw_addr,'other':other, 'loacl_ip_addr':loacl_ip_addr}
        regex_list=['\s+Rx:\s+Request\s+(?P<rx_request>\d+)',
                    '\s+Tx:\s+Request\s+(?P<tx_request>\d+)',
		    '\s+Reply\s+(?P<rx_reply>\d+)',
		    '\s+Reply\s+(?P<tx_reply>\d+)',
 	            '\s+Other\s+(?P<rx_other>\d+)',
                    '\s+Error:\s+Unknown\s+Proto\s+(?P<unknown_proto>\d+)',
                    '\s+Length\s+(?P<length>\d+)',
                    '\s+Target\s+Null\s+(?P<target_null>\d+)',
                    '\s+Src\s+IP\s+Null\s+(?P<src_ip_null>\d+)',
                    '\s+Local\s+HW\s+Addr\s+(?P<local_hw_addr>\d+)',
                    '\s+Other\s+(?P<other>\d+)',
	            '\s+Local\s+IP\s+Addr\s+(?P<local_ip_addr>\d+)']

        actual={}
        cli="show ip counters arp"
        ret_str=self.cmd(cli)
        for regex in regex_list:
            obj=re.compile(regex,re.I)
            m=obj.search(ret_str)
...
def ip_verify_ip_counters_arp_dict(self,unknown_proto='none',length='none',
                                  rx_request='none',tx_request='none',
                                  target_null='none',src_ip_null='none',
                                  local_hw_addr='none',other='none',
                                  loacl_ip_addr='none' ):
	...
	if large_pkt==2:
        ...
        if ip_options == 'none':
            for key in dict.keys():
                if dict[key] != 'none':
                    value=int(dict[key])
		    log.output("ip_options==none")
                    scapy_cmd='sendp(Ether(dst="%s")/IP(src="%s",dst="%s",%s=%d,proto="tcp")/"%s",iface="%s")' %(str1,src_ip_adr,dst_ip_adr,key,value,payload,from_interface)
                    break
                else:
                    scapy_cmd='sendp(Ether(dst="%s")/IP(src="%s",dst="%s",proto="tcp")/"%s",iface="%s")' %(str1,src_ip_adr,dst_ip_adr,payload,from_interface)
        else:
            if ip_ttl == 'none' and ip_flags == 'none':
		#changing this line introducing proto in Ip field
                scapy_cmd='sendp(Ether(dst="%s")/IP(src="%s", dst="%s",options="%s",proto="tcp")/"%s",iface="%s")' %(str1,src_ip_adr,dst_ip_adr,ip_options,payload,from_interface)
            elif ip_flags != 'none':
                scapy_cmd='sendp(Ether(dst="%s")/IP(src="%s",dst="%s",flags=%d,options="%s",proto="tcp")/"%s",iface="%s")' %(str1,src_ip_adr,dst_ip_adr,ip_flags,ip_options,payload,from_interface)
         
            else:
                value=int(ip_ttl)
                scapy_cmd='sendp(Ether(dst="%s")/IP(src="%s",dst="%s",options="%s",ttl=%d,proto="tcp")/"%s",iface="%s")' %(str1,src_ip_adr,dst_ip_adr,ip_options,value,payload,from_interface)
...
def get_ip_counters_arp(self,context="local") :
        """
        Description : API used to get the ARP counters
        Arguments   : context : specify context name otherwise it will take default context name i.e local
        Example     : self.ssx.get_ip_counters_arp(context="cont1")
        ReturnValue : Returns the ARP counter names and values
        """

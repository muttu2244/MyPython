...
class Objects(object):
    ...
    def click(self, client, *args):
        ...
        try:
            logger.info("Navigated to: " + comment)
    ...
    def element_get_property(self, client, *args):
        ...
        try:
            ...
            logger.info("Got " + properties + " from screen " + " : " + value)
            return value
        except InternalException, msg:
    ...
    def element_send_text(self, client, *args):
        ...
        zone, element, index, text, comment, logger_name = args
        logger = logging.getLogger(logger_name)
        try:
            client.elementSendText(zone, element, index, text)
            logger.info("sending text to " + comment + " :: Element Send Text --> Passed")
        except InternalException, msg:
            logger.error("sending text to " + comment + " :: Element Send Text --> Passed;" +
                         str(msg))
        except RuntimeException, msg:
            logger.error("sending text to " + comment + " :: Element Send Text --> Passed;" +
                         str(msg))
    ...
    def verify_element_found(self, client, *args):
        """To Verify an element is found"""
        zone, element, index, comment, logger_name = args
        logger = logging.getLogger(logger_name)
        try:
            client.verifyElementFound(zone, element, index)
            logger.info("verifyElementFound-->Passed" + comment)
            return True
        except InternalException, msg:
            logger.error(" :: " + str(msg) + "Element Not Found-->Failed")
            return False
        except RuntimeException, msg:
            logger.error(" :: " + str(msg) + "Element Not Found-->Failed")
            return False
    ...
    def touch_down(self, client, *args):
    ...
    def element_list_pick(self, client, *args):
        """Select an element from the Object Repository in a list"""
        list_zone_name, list_locator, element_zone_name, element_locator, index, click = args
        try:
    ...
    def is_element_found(self, client, *args):
    ...
    def get_element_count(self, client, zone, element):
        ...
        try:
            ...
            logging.info(str(
                time.asctime(time.localtime())) + element + " count is: " + str(value) + "--> Passed")

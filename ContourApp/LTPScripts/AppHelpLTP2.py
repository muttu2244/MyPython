...
class Apphelp(unittest.TestCase):
    ...
    def main_screen(self):
        ...
        text_from_xml, ids, eng_list = self.util.get_text_from_xml(self.string_xml, "mainScreen",
                                                                   "trans-unit",
                                                                   Config.selected_language.strip())
        ...
        while loop_index < len(xpath):
            print xpath[loop_index]
            if loop_index == 1:
                self.place_holder(xpath, loop_index, text_from_xml, text_index, ids, eng_list)
                text_index += 1
                loop_index += 1
                continue
    ...
    def blood_sugar_reading_meal_marker(self):
        ...
        actual_text, ids, eng_list = self.util.get_text_from_xml(self.string_xml,
        ...
        for loop_index in range(max(len(actual_text), len(string_list)) - 1):
            ...
            if actual_text2[text_index] and string_list[loop_index]:
                self.logger.info("Testing StringID == " + str(ids[text_index]))
                self.logger.info("English Text == " + eng_list[text_index])
                self.util.text_compare2(self.common, actual_text2[text_index], string_list[loop_index], ids[text_index],
                                        self.logger_name)
            text_index += 1
            # except:
            #     "value error"
        ...
        for loop_index in range(max(len(actual_text), len(string_list)) - 1):
            ...
            if actual_text2[text_index] and string_list[loop_index]:
                ...
                self.util.text_compare2(self.common, actual_text2[text_index], string_list[loop_index],
                                        ids[text_index], self.logger_name)
            text_index += 1
            # except:
            #     "value error"
    ...
    def target_range_critical_values(self):
        ...
        text_from_xml, ids, eng_list = self.util.get_text_from_xml(self.string_xml, "TargetRangeandCriticalValues",
                                                                   "trans-unit", Config.selected_language.strip())
    ...
    def reminders_and_patterns(self):
        ...
        actual_text, ids, eng_list = self.util.get_text_from_xml(self.string_xml, "Reminders",
                                                                 "trans-unit", Config.selected_language.strip())
        actual_text2 = []
        for str1 in actual_text:
            if "<br>" in str1:

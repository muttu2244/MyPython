...
def readDataFromDB():
    ...
    #SQLCommand = ("SELECT top 1 Transformed FROM MessageQueue Where ProductId = 'Dev' ")
    SQLCommand = ("SELECT top 3 Transformed FROM (SELECT ROW_NUMBER() OVER (ORDER BY QueueId ASC) AS RowNumber,*FROM MessageQueue Where ProductId = 'Dev') AS foo WHERE RowNumber = 1")

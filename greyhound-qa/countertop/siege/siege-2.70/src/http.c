...
http_get(CONN *C, URL *U)
{
  ...
  rlen = snprintf(
    ...
    (ifmod!=NULL)?ifmod:"",
    (ifnon!=NULL)?ifnon:"",
    my.encoding, my.uagent, my.extra,
    gen_zid(&C->rand, my.zid_start, my.zid_end, NULL),
    keepalive 
  );
...
static char *expand_suffix(char *buffer, size_t size, char *orig, char *format, ...)
{
  char *p;
  size_t prefix;
  va_list ap;
  ...
  tmp = strdup(p + strlen(orig));

  va_start(ap, format);
  offset = vsnprintf(p, size - prefix - 1, format, ap);
  va_end(ap);
  
  snprintf(p+offset, size - prefix - offset - 1, "%s", tmp);
  free(tmp);

  return buffer;
}

static char *expand_zid(char *buffer, long int zid)
{
  int i;
  for(i = -10; i < 10; i++) {
    char pattern[128];
    sprintf(pattern, "{zid%+d}", i);
    char *s = expand_suffix(buffer, 4096, pattern, "%ld", zid + i);
    if(s) {
      return s;
    }
  }
...
http_post(CONN *C, URL *U)
{
  ...
  char fullpath[4096];
  long int zid;
  char *zauth = gen_zid(&C->rand, my.zid_start, my.zid_end, &zid);

...
load_conf(char *filename)
{
  ...
  while((line = chomp_line(fp, &line, &line_num)) != NULL){
	...
	*tmpvalue++=0;
    ...
    else if(strmatch(option, "zid-start")){
      my.zid_start = strtoull(value, 0, 10);
    }
    else if(strmatch(option, "zid-end")) {
      my.zid_end = strtoull(value, 0, 10);
    }
    else if(strmatch(option,"gid")) {
	my.gid = strtoull(value, 0, 10);
    }
    else if(strmatch(option,"secret")) {
        my.secret = stralloc(trim(value));
    }
    else{

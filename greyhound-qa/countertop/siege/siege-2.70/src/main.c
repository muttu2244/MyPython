...
static struct option long_options[] =
{
  ...
  { "zid-start",  required_argument, NULL, 'X' },
  { "zid-end",    required_argument, NULL, 'Y' },
};
...
display_help()
{
  ...
  puts("  -X, --zid-start=<zid>   Start zid value for Z-Authorization header" ); 
  puts("  -Y, --zid-end=<zid>     End zid value for Z-Authorization header" ); 
  puts("");
  puts(copyright);
  /**
...
parse_rc_cmdline(int argc, char *argv[])
{
  ...
  while( a > -1 ){
    a = getopt_long(argc, argv, "VhvCDgl::ibr:t:f:d:c:m:H:R:A:X:Y:", long_options, (int*)0);
    if(a == 'R'){
      strcpy(my.rc, optarg);
      a = -1;
    }
...
parse_cmdline(int argc, char *argv[])
{
  int c = 0;
  int nargs;
  while((c = getopt_long( argc, argv, "VhvCDgl::ibr:t:f:d:c:m:H:R:A:X:Y:",   
  ...
  switch(c){
	...
	if(my.delay < 0){
      ...
      case 'H':
        ...
        break; 
      case 'X': 
        my.zid_start = strtoull(optarg, 0, 10);
        printf("zid-start: %s\n", optarg);
        break;
      case 'Y': 
        my.zid_end = strtoull(optarg, 0, 10);
        printf("zid-end: %s\n", optarg);
        break;
  ...
  if(!my.zid_start || !my.zid_end){
    puts("ERROR: zid incorrect or not specified");
    exit(1);
  }
  return;

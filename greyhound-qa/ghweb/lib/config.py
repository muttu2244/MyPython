...
class ConfigService:
	@staticmethod
	def getConfig():
		path = "/apps/%s/current/Storage.yaml" % (Constants.APP_NAMESPACE)
		f = open(path)
	...
	def getDeltaLimits(delta_type):
		...
		for d in deltas:
			...
			if d['type'] == delta_type:
				...
				return {'maxsize': maxsize, 'maxcount': maxcount, 'ttl': ttl, 'keep': keep}
	...
	def getScoreBoardLimits(type):
		...
		for d in deltas:
			...
			if d['type'] == type:
				...
				return {'max': max, 'min': min, 'ttl': ttl, 'maxcount': maxcount}

...
def user_reputation_get(zauth,data):
	url = "http://%s/services/user.reputation.get.php"  % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)] )
...
def user_meta_threshold_get(zauth,data):
	url = "http://%s/services/user.meta.thresholds.get.php" % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)] )
	return curl_web_req(zauth, url, data)

def user_admin_payments_meta_get(zauth,data):
	url = "http://%s/services/user.payments.meta.get.php" % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)] )
        return curl_web_req(zauth, url, data)


def user_history_get(zauth,data):
	url = "http://%s/services/user.history.get.php" % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)] )
	return curl_web_req(zauth , url , data)


def user_blob_revert_golden(zauth,data):
	url = "http://%s//services/user.blob.revert.golden.php" % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)] )
        return curl_web_req(zauth,url ,data)


def user_blob_delete(zauth,blob_type):
	 url = "http://%s/services/user.blob.delete.php/%s" % (ADMIN_URL[random.randrange(0,SERVERS_ADMIN)],blob_type )

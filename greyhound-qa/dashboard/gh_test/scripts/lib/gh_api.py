...
def user_blob_set_empty(zauth, blob_type,fields, cas):
	url = "%s/user.blob.set.php/%s" % (SERVICE_URL[random.randrange(0,SERVERS)] , blob_type)
	add_headers = ['X-Operation: user.blob.set' , "If-Match: %s" % (cas) ]
	return curl_web_req(zauth, url, None, add_headers)

def user_blob_patch(zauth, blob_type,file, cas,checksum):
        url = "%s/user.blob.patch.php/%s" % (SERVICE_URL[random.randrange(0,SERVERS)] , blob_type)
...
def user_blob_deleteDeltas( zauth , delta_ids):
	url = "%s/user.blob.deleteDeltas.php" % (SERVICE_URL[random.randrange(0,SERVERS)] )
...
def user_scoreboard_create(zauth,ival,scoreboard_type=''):
        url = "%s/user.scoreboard.create.php/%s" %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_type)
...
def user_scoreboard_get(zauth,scoreboard_key=' '):
	url = "%s/user.scoreboard.get.php/%s" %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
	add_headers = ['X-Operation: user.scoreboard.get']
	return curl_web_req(zauth,url,{}, add_headers)

def friend_scoreboard_get(fzauth,scoreboard_key=' ' ):
	url = "%s/friend.scoreboard.get.php/%s"  %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
...
def friend_scoreboard_increment(fzauth, scoreboard_key= ' '):
	url = "%s/friend.scoreboard.increment.php/%s"  %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
	add_headers = ['X-Operation: friend.scoreboard.increment']
	return curl_web_req(fzauth, url , {} , add_headers)

def friend_scoreboard_decrement(fzauth, scoreboard_key= ' '):
	url = "%s/friend.scoreboard.decrement.php/%s" %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
	add_headers = ['X-Operation: friend.scoreboard.decrement']
	return curl_web_req(fzauth, url , {} , add_headers)

def user_scoreboard_setmeta(zauth, scoreboard_key, meta_data):
	url = "%s/user.scoreboard.setmeta.php/%s" %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
...
def user_scoreboard_delete(zauth,scoreboard_key= ' '):
	url = "%s/user.scoreboard.delete.php/%s" %(SERVICE_URL[random.randrange(0,SERVERS)] ,scoreboard_key)
...
def get_friend_nofriend_id(zauth, graph_type):
	url = "%s/graph.user.getmembers" %(MQS_URL[random.randrange(0,MQS_SERVERS)] )
